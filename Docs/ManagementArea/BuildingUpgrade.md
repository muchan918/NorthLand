...
> **이슈**: #139 (feature/139-building-upgrade) · #205 (마법 연구소 강화) · #229 (본진 업그레이드) · #444 (되돌리기)
...
> **되돌리기(#444)**: 낮 동안 한 업그레이드와 **주민 증축**은 되돌리기 버튼·**Ctrl+Z**로 되돌릴 수 있다 —
> 타워 배치·합성과 같은 히스토리를 쓴다(§10).
...
| UI | `BuildingInfoUI`(클릭→패널→버튼) | **동일** `BuildingInfoUI`(별 분기). (연금술사의 집은 예외 — 업그레이드 트랙이 아니라 교환소라 **별도 `StorePanelUI`** 를 띄운다, #211) 생산의 "주민당 5→7" 자리(`_amountText`)는 여전히 `building.upgrade.skill_pending`("스킬 강화")이지만, **강화 수치는 업그레이드 버튼 위 별도 줄**(`_skillEffectText`)에 "데미지: 36 → 42" 형태로 표시한다(#375 도입 → #398에서 % 문장형에서 실수치로 개정, 아래 상세) |
...
### 강화 수치 UI 표시 (#375 도입 → #398 실수치 표기)

업그레이드 버튼 **위** 줄(`BuildingInfoUI._skillEffectText`)에 이번 업그레이드로 감전 스탯이 **어떤 값이 되는지**를 현재값 → 도달값으로 보여준다(Lv1 → Lv2 예시).
...
- **#375의 % 문장형("감전 데미지가 기본 대비 40% 증가합니다")에서 #398로 바뀌었다.** %는 기본 대비 누적 배율이라 플레이어가 체감하는 값이 아니었다 — 지금 데미지가 30인지 48인지 모르는 채 "40% 증가"만 읽히고, 감전 베이스 스탯이 화면 어디에도 없어 환산할 방법도 없었다. 표기 규약은 생산 건물의 "주민당 5 → 7"(`RefreshProductionLine`)·보상 카드(`SkillStatsFormatter`)와 같다.
- **배율은 클릭한 건물 SO에서, 베이스 스탯은 `SkillManager`에서** 온다. 절대값을 만들려면 두 출처가 다 필요하다(배율만으로는 절대값이 안 나온다) — #375가 "UI가 베이스 스탯을 읽으면 경영↔전투 결합이 새로 생긴다"며 미뤄 뒀던 그 결합을 **이번에 열었다**: `SkillManager.BaseDamage`/`BaseRadius`/`BaseCooldown`(배율 적용 **전** 원본)과 `static SkillManager.Scale(baseValue, multiplier)`.
  - **곱셈은 `Scale` 하나만 통과한다** — 0/음수 배율 방어(`PositiveOr1`)까지 그 안에 있어서, UI가 곱셈식을 따로 들면 한쪽만 고쳤을 때 "패널엔 36인데 실제론 30"이 조용히 생긴다. 표시와 실효가 같은 식을 지나는 것이 이 결합을 허용한 대가이자 조건이다.
  - ⚠ **문자열은 `SkillManager`가 만들지 않는다.** 매니저는 자기 `_magicLabAsset` 하나만 알기 때문에, 두 번째 스킬 강화 건물이 생기면 **클릭한 건물과 다른 수치**를 보여주게 된다. 배율은 항상 선택된 건물에서 읽는다(PR#378 리뷰의 "컨트롤러와 같은 리스트를 읽어야 한다"와 같은 축).
- 현재 authoring 기준 도달값(베이스 데미지 30 / 범위 6 / 재충전 10 — `GameScene`의 `SkillManager` 인스펙터, WL-015 축):
...
- 문자열 소유가 옮겨졌다: `NorthLand_default`의 문장형 키 3개(`lab.effect.damage`/`.radius`/`.cooldown`)는 **삭제**됐고, `NorthLand_Skills`의 라벨 키 3개(`skills.stat.damage`/`.radius`/`.cooldown`)를 쓴다. 문장이 아니라 "라벨: 값 → 값" 조립이라 **서식 소유자는 로컬라이제이션 문자열이 아니라 `SkillStatsFormatter`**(`BuildSkillDamageLine`/`BuildSkillRadiusLine`/`BuildSkillCooldownLine`)이며, 보상 카드와 화살표(`k_Arrow`)·숫자 서식(`0:0.#`)을 공유한다.
  - **단위("초")만 별도 키**(`skills.stat.unit.second`)인 이유: 라벨에 단위를 붙이면 언어별 표기(초/s/秒)를 코드가 하드코딩하게 되고, 줄 전체를 Smart String("재충전 시간 {0}초 → {1}초")으로 만들면 화살표가 로컬라이즈 문자열 안으로 들어가 `k_Arrow` 단일 출처가 깨진다. 단위만 키로 빼면 둘 다 지킨다.
- 배율 엔트리는 `Skill.UpgradeLevels`를 직접 읽지 않고 **`UpgradeSteps`를 인덱싱한 뒤 `as SkillUpgradeLevel`로 좁힌다**(#375에서 확립, 그대로) — 컨트롤러가 레벨·MAX를 세는 것과 물리적으로 같은 리스트여야 한다(PR#378 리뷰). `UpgradeSteps`는 Castle→Skill→Production→Exchange 중 첫 그룹만 돌려주므로, 두 그룹을 동시에 authoring한 건물에서 "Lv 2/3인데 4행째 수치"가 뜨는 것을 막는다.
  - **현재값 쪽 배율은 `Scaling(nextLevel - 1)`**이고, Lv0이면 null = 배율 없음 = **베이스가 곧 현재값**이다(`SkillManager.RefreshUpgrade`의 "레벨 0/범위 밖 = 배율 1.0"과 같은 규약, `DamageMult`/`RadiusMult`/`CooldownMult`).
- **값이 변하지 않는 스탯은 줄 자체가 빠진다** — 그 레벨에서 안 건드리는 스탯이 "30 → 30"으로 뜨는 것을 `AddStatLine`의 `Mathf.Approximately` 가드가 막는다(%표기 시절 "0% 증가합니다"를 막던 것과 같은 목적, `RewardCardView`가 수치 없는 카드를 통째로 비우는 규약과 동일 계보). 수치가 placeholder(TBD)라 밸런싱 패스에서 한 레벨에 한 스탯만 올리는 순간 필요해진다.
- 재충전은 낮아지는 게 이득이라 값이 **줄어드는** 유일한 줄이다("9초 → 8초"). %표기 시절의 `Pct(..., inverted: true)` 부호 뒤집기는 사라졌다 — 실수치는 방향을 설명할 필요가 없다.
- **최대 도달·본진 레벨 잠금이면 빈 줄**이다(잠금 상태에서 필요한 정보는 잠금 사유 쪽, §9). `Body`의 VerticalLayoutGroup이 `childControlHeight = true`라 빈 줄이면 자리가 접힌다.
- `building.upgrade.skill_pending`은 **삭제되지 않고 폴백으로 남는다** — SO에 해당 레벨이 없거나 **씬에 `SkillManager`가 없으면**(베이스 스탯 출처 부재, `SkillManager.Instance == null`) `UpgradeEffect`가 빈 문자열을 돌려주고, `_amountText`의 "스킬 강화"만 보인다.
...
### 잔여 / TODO
- [ ] **수치 밸런싱**: 레벨 수·레벨당 마나 비용(현재 placeholder 20/40/60), 스킬 강화 배율(현재 placeholder). 배율을 바꾸면 위 UI 표시값이 자동으로 따라간다(별도 작업 불필요). 단 **베이스 스탯(`SkillManager` 인스펙터)을 바꾸면 위 도달값 표도 함께 갱신**해야 한다 — 표시값이 배율만이 아니라 베이스×배율이 됐다(#398).
...
> **상태**: ✅ 구현. 낮 동안 한 **건물 업그레이드**(생산 라인 · 업그레이드 전용/본진)와 **주민 증축**을
> 되돌린다 — 타워 배치·합성(#281)과 **같은 히스토리**에 쌓이므로 "건물 → 타워 → 건물" 순으로 했다면
> 그 역순으로 하나씩 되감긴다.
> **입력**: 되돌리기 버튼 + **Ctrl+Z**(둘 다 `UndoRequest.Submit()`으로 모인다).

### 무엇을 새로 만들지 않았는가

이 이슈의 핵심은 **기능 추가가 아니라 기존 것의 승격**이다. 네 가지를 그대로 재사용한다:

| 재사용한 것 | 원래 용도 | 되돌리기가 쓰는 방식 |
|---|---|---|
| `CommandHistory`(LIFO 20) | 타워 배치·합성 되돌리기(#281) | **같은 스택**에 경영 커맨드를 올린다(새 스택을 만들면 두 스택의 순서를 사용자가 알 수 없다) |
| `ManagementController.Grant` | 타워 배치 되돌리기의 비용 환원(#281) | 그대로. 환원 경로는 여전히 하나다(WL-017) |
| `TryRestoreProductionLine` / `TryRestoreUpgradeBuilding` / `TryRestoreBonusVillagers` | 세이브 로드 복원(#270) | **이전 값으로 되맞추는 수단**. "비용·페이즈 게이트를 타지 않고 상태를 그 값으로 맞춘다"가 되돌리기에 필요한 것과 정확히 같다 |
| `UnassignVillager` | 경영 패널의 배치 −1(#219) | 주민 증축 되돌리기가 상한을 맞추려 한 명을 뺄 때 쓴다 — 퇴장 연출까지 딸려 오므로 새로 만들 것이 없다(아래) |

되돌리기 전용 감소 API(`DowngradeBuilding` 류)를 만들지 않은 근거가 위의 `TryRestore*` 행이다 — 같은 런타임 상태를
쓰는 경로가 둘이 되면 그 둘이 어긋나는 순간을 아무도 못 본다(WL-016의 "상태 소유는 한 곳" 축).

새로 생긴 것은 공통 기반 `ReversibleCommandBase`(상태 기계 + 비용 환원)와 범용 커맨드
`ResourceSpendCommand`(자원을 냈고 수치 하나가 올라갔다 = 경영 조작의 공통 모양) 둘뿐이고, 타워 커맨드
2종도 같은 기반으로 옮겨 **상태 전이가 한 군데서만** 일어나게 했다.

### 등록 위치 — UI가 아니라 컨트롤러

`TryUpgrade` / `TryUpgradeBuilding` / `TryIncreaseVillagers`가 성공한 자리에서 직접 `PushSpendUndo`를 부른다.
그래서 `BuildingInfoUI`·`CastlePanelUI`·테스트 헬퍼(`BuildingsUpgradeHelper`, `BalanceTestPanel`) **어느 호출부도
고치지 않고** 전부 되돌릴 수 있다. UI에서 등록하면 호출부가 늘 때마다 되돌리기가 조용히 빠진다
(`TowerPlacer`가 배치 코어에서 등록하는 것과 같은 판단).

### 순서가 유효성 검사를 겸한다 (LIFO)

본진 레벨이 하위 건물의 실질 Max를 정하므로(§9), **본진을 올리고 → 그 덕에 열린 하위 건물을 올린** 상태에서
본진만 먼저 되돌리면 하위 건물이 상한을 넘은 레벨로 남는다. LIFO가 하위 건물을 먼저 되돌리게 강제해 이
경로를 원천 차단한다 — 타워 되돌리기가 "합성 재료가 타일을 되찾는 순서"를 LIFO로 보장하는 것과 같은 논지다
(`IReversibleCommand` 주석).

### 주민 증축은 배치까지 본다

증축(#227)은 상한을 올리는 조작이라 되돌리기가 **상한만 내리면 안 된다.** 늘린 주민은 그날 바로 배치할 수
있고 그 배치는 히스토리를 거치지 않으므로(`AssignVillager`는 자원을 쓰지 않아 이번 축의 바깥이다), 상한만
내리면 `AssignedTotal > MaxVillagers` — **없는 주민이 계속 생산하는 상태**가 남는다.

그래서 `RevertVillagerGrowth`는 규칙 하나를 더 갖는다:

> 상한 −1. 배치가 새 상한을 넘으면 **가장 많이 배치된 라인에서 1명을 `UnassignVillager`** 한다.

- 라인 선택은 임의지만 **결정적**이다(동수면 낮은 index). 분산 배치한 판을 가장 덜 흐트러뜨리는 선택이다.
  ⚠ **이 선택은 구현 규칙이고 GDD 조항이 아니다** — GDD §5.2가 규정하는 것은 "상한을 넘긴 배치를 남기지
  않는다"는 불변식 하나이므로, 규칙을 "마지막에 배치한 라인" 등으로 바꿔도 GDD는 그대로다.
  ⚠ **빠져나온 주민은 되돌리기로 복구되지 않는다** — 배치는 되돌리기 대상이 아니라(아래 「범위 밖」)
  Ctrl+Z를 더 눌러도 그 주민이 원래 라인으로 돌아가지 않는다. 라인별 생산 효율 차가 벌어지면 이 손실이
  체감될 수 있어, 규칙 재검토가 필요해지는 지점이 바로 거기다.
- **연출을 새로 만들지 않는다** — `UnassignVillager`를 그대로 쓰므로 `VillagerUnassigned`가 발화해 그 건물
  출입 포인트에서 주민 1명이 걸어 나온다(`Resident.md` §3.2). "되돌렸다"가 화면에서 읽히는 경로가 이미 있다.
- 군중 수(`crowdSize − AssignedTotal`)는 `MaxVillagers`와 무관하므로, 유휴 주민이 있는 흔한 경우엔 상한만
  내려가고 화면은 그대로다 — 군중 표시와 어긋날 여지가 없다.
- 뺄 수 없으면(있을 수 없지만) **상한을 내리지 않고 실패로 끝낸다.** `ResourceSpendCommand`가 실패 시
  환원을 건너뛰므로 상태와 지갑이 함께 제자리에 남는다.

### 되돌릴 대상을 다시 찾는다 — index 대조

복원 API는 대상을 **`BuildingID`로 다시 찾는다**(세이브는 index가 없어 그럴 수밖에 없다). 그런데 되돌리기는
등록 시점의 index를 클로저에 쥐고 있으므로, 복원 직전에 **재해소한 index가 그것과 같은지 대조한다**
(`TryRevertUpgradeBuilding` / `TryRevertProductionLine`).

같은 건물 SO가 `_upgradeBuildings`·`_productionBuildings`에 두 번 등록되면(authoring 실수)
`FindUpgradeIndexById`가 첫 일치를 잡아 **엉뚱한 건물이 되돌아간다** — 자원은 환원되는데 레벨은 남의 것이
내려가고 콘솔은 조용하다. SO 유일성은 저장소 전체가 기대는 미가드 전제라(WL-021) 여기서 상속하지 않고 막는다.

⚠ 대조 실패는 **에러 로그**로 남긴다. `CommandHistory.Undo`가 커맨드를 실행 **전에** 스택에서 빼므로 실패한
항목은 재시도되지 않는다 — 조용히 넘어가면 "눌렀는데 아무 일도 없다"로만 보인다. 이 순서 자체를 고치는 축은
WL-148이 따로 본다.
### 범위 밖 — 자원 교환

| 조작 | 왜 아직 안 하는가 |
|---|---|
| 자원 교환(#211) | 받은 자원을 이미 써버렸을 수 있어 되돌리기가 **실패할 수 있는 조작**이 된다. "되돌리기는 항상 성공한다"가 깨지면 눌러도 아무 일이 없는 항목이 스택에 남는다 |
| 주민 배치(#219/#341) | 자원을 쓰지 않아 이번 축의 바깥이고, **반대 조작이 이미 있다**(`ProductionLineView`의 − 버튼). 게다가 낮 루프에서 가장 잦은 조작인데 다중 드롭은 한 제스처가 `AssignVillager`를 인원수만큼 부르므로, 깊이 20을 잡아먹어 정작 되돌리고 싶은 지출이 스택에서 밀려난다 |

되돌릴 수 없다는 뜻이지, 되돌리기가 **엉뚱한 것을 되돌린다**는 뜻은 아니다 — 스택에 안 오르므로 Ctrl+Z는
그 이전의 되돌릴 수 있는 조작을 되돌린다(일반적인 undo 동작).

### 확인 방법 (PlayMode, 수동)

1. 낮에 생산 건물 업그레이드 → 자원이 줄고 Lv가 오른다 → Ctrl+Z → **자원 100% 복구 + Lv 원복**, 패널은 열린 채 갱신된다.
2. 본진 업그레이드 → 타워 배치 → 본진 업그레이드. Ctrl+Z 3회에 **본진 → 타워 → 본진** 순으로 되감긴다.
3. 밤으로 넘어가면 버튼이 비활성이 되고 Ctrl+Z도 먹지 않는다(`CommandHistory.CanUndo`의 페이즈 검사).
4. 업그레이드 21회 후 되돌리기 20회 → 21번째는 되돌릴 수 없다(LIFO 깊이 20, 초과분은 밀려나며 `Commit`된다).
5. 주민 증축(유휴 상태) → Ctrl+Z → **자원 복구 + 상한 −1**, 화면의 군중은 그대로다.
6. 주민 증축 → 늘어난 주민까지 전부 배치 → Ctrl+Z → 상한 −1과 함께 **가장 많이 배치된 건물에서 주민 1명이 걸어 나온다**(배치 −1). 이후 배치 ≤ 상한이 유지된다.
