...
- **주민·식량과 무관**: 확장 자원은 주민 배치·식량과 독립적인 패시브 수입이다. 확보한 영지 수가 늘수록 자원 종류와 총 일일 수급이 함께 늘어난다.
- 구현 시 영향 지점: 라인 목록이 고정 `_resourceAssets[]`가 아니라 **영토 해금으로 동적 증가**(현재 인스펙터 고정 배열은 WL-021 경로로 재작업 예정), 정산부(`HandleNightToDay`)에 확장 자원 **일일 수급 `Add`** 로직 추가(주민 배치와 무관한 자동 수급 라인 유형 신설), 자원 총량 표기의 정본을 생산 패널로 이관((C)).
...
### (C) 자원 총량 표기 — 생산 라인 패널로 이관
자원 보유 **총량 표기의 정본을 우측 생산 라인 패널**로 옮긴다(회의 결정). 패널은 자원 종류당 한 row이며, row 유형에 따라 표기가 갈린다.

| row 유형 | +/- 버튼 | 표기 내용 |
|---|---|---|
| 기본 자원(나무·철·식량) | ✅ (주민 배치) | 현재 보유량 · 배치 주민수 · 예상 생산량 · +/- |
| 미개척 영지 자원(가변) | ❌ | 현재 보유량 · **매일 추가되는 수급량** |
| 마나석 | ❌ | 현재 보유량 |

- **+/- 버튼 규칙**: **주민을 소모(배치)해서 얻는 자원(기본 3종)만** +/- 를 노출한다. 자동 수급형(확장 자원)·상위 자원(마나석)은 +/- 없이 보유량 중심으로 표기.
- **탑 바(top bar)**: 종전 탑 바의 자원 4종 총량 표기는 **당분간 현행 그대로 유지하되 4주차에 제거 예정**(임시). 이번 작업에서 탑 바는 건드리지 않으며, 총량 표기의 정본은 패널이다.
- 구현 영향: `ManagementPanelView`/`ProductionLineView`가 **row 유형별로 렌더 분기**(+/- 유무, 보유량, 수급량/예상 생산량). 마나석은 생산 라인이 아니지만 **표기 목적의 row**로 패널에 포함하고, 실제 적립은 후속 시스템의 지갑 `Add`가 담당((A)·계약 #3).
