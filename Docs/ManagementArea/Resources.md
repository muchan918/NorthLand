...
- 교환 대상은 마나석을 제외한 **7종**(나무·철·식량·금·루비·사파이어·다이아). 행 추가/삭제는 코드 변경 없이 `Exchange.Offers`(SO 리스트) 편집으로 끝난다.
- **역교환(자원 → 마나석)은 없다.** 마나석은 여전히 영토 확장·전투 보상으로만 들어온다.
- 교환비 수치는 SO authoring(밸런싱 TBD). 미개척 영지 자원 4종은 영토 확장 동기를 해치지 않도록 기본 자원보다 확실히 비싸게 잡는다.
- **업그레이드 시 교환 효율이 개선된다**(확정 설계, **#211 미구현**): 지불 마나석은 그대로 두고 받는 자원량에 `Exchange.UpgradeLevels[].GainMultiplier`를 곱한다(예: 마나석 10 → 나무 10 이 업그레이드 후 나무 15). 게이트웨이 쪽 훅(`ManagementController.ExchangeGainAmount`)은 이미 들어가 있어 `UpgradeLevels`가 비어 있는 동안 항등으로 동작한다 — 남은 건 **레벨 테이블 등록 경로**로, BuildingUpgrade.md §8의 '타입 중립 승격'이 선행돼야 한다.
...
- **#3 자원 흐름** (GDD §3.2): 기본 자원(나무/철/식량) = 주민 배치 생산 **또는 영토 확장 보상**, 마나석 = 영토 확장·전투 보상에서만.
  ...
  - **방향 전환(이번 이슈)**: **미개척 영지 자원**은 영지 해금 시 **매일 정산마다 일정량이 자동 수급**된다(GDD §3.2, 주민 배치·식량 소모 모두 없음). 영토 해금이라는 정당한 원천에서 나오므로 계약 위반이 아니다. (직전 '식량 소모 → 확장 자원 변환' 모델은 폐기 — §5.5 (A).) **마나석 → 자원 교환은 연금술사의 집으로 확정**(#211, §3 '자원 교환 경로' — WL-042 해소). 지갑 획득 API를 열지 않고 차감+지급 원자 트랜잭션 하나만 노출했으므로 계약 위반이 아니다. 본진 업그레이드 마나석 소모 등 나머지 우회 경로는 여전히 WL-042 합의 대기.
