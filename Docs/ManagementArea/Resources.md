...
- **페이즈 전환 요청**: 패널 버튼 → `ManagementEndDayConfirmPopup.Request(controller)` → 낮 프로세스 조건
  (① 유휴 주민 없음 `!HasIdleVillagers`, ② 오늘 타워를 배치했음)을 점검해 **둘 다 충족이면 곧장**,
  하나라도 미충족이면 **확인 팝업을 띄우고 [계속] 선택 시** `ManagementController.EndDay()` → `DayNightManager.EndDay()`.
  영토 미확장 경고는 #337에서 삭제됐고, 같은 이슈에서 **타워 미배치 경고가 새로 들어왔다**.
  - **타워 조건은 팝업이 직접 관찰한다** — `ManagementController`가 아니라 `ManagementEndDayConfirmPopup`이
    `Tower.ActiveChanged`를 구독해 "오늘 타워 수가 늘었는가"를 본다(팀 계약 #4 공간 분리 — 경영 모델이
    전투 공간을 알지 않는다). 판정 기준은 `ShouldWarnAboutTowers` 프로퍼티 **한 곳**에 격리돼 있어,
    "무방비일 때만 경고"(`Tower.Active.Count == 0`)나 경고 빈도 축소로 갈아끼울 때 그 줄만 고치면 된다.
...
- ⏳ **주민 시스템**: 주민 **개체** 보유·집계 (GDD §6.1). 여전히 부재 — #227로 늘어난 건 '수'(`MaxVillagers`)뿐이고, 주민 하나하나를 표현하는 엔티티는 없다.
...
2. 낮 종료 버튼 → 유휴 주민이 남아 있거나 **오늘 타워를 배치하지 않았으면** 확인 팝업(해당 항목만 표시),
   [계속]이면 낮→밤 정산(자원이 HUD에 증가). 둘 다 충족이면 팝업 없이 바로 넘어간다.
   버튼은 어느 경우에도 비활성화되지 않는다(#219).
   - 타워 경고는 **매 낮 시작마다 리셋**된다. 합성으로 타워가 새로 생긴 것도 배치로 친다.
   - ⚠ **이어하기 직후 그날의 타워 경고는 실행 순서에 따라 갈린다.** 팝업 `Start()`가 먼저 돌면
     타워 수를 0으로 스냅샷한 뒤 복원이 타워를 등록해 수가 늘므로 '오늘 배치함'으로 잡혀 경고가 뜨지 않고,
     `RunSaveManager.Start()`가 먼저 돌면 이미 등록된 수를 스냅샷하므로 정상적으로 경고가 뜬다.
     이 프로젝트엔 `ScriptExecutionOrder`가 없고 `[DefaultExecutionOrder]`도 `RunBootstrapper`(-1000)
     하나뿐이라 **둘 사이 순서는 보장되지 않는다**. 어느 쪽이든 타워가 실제로 있는 상태라 피해는 없어
     지금은 고정하지 않는다 — 재현성이 필요해지면 실행 순서를 명시적으로 지정할 것(RunSaveManager 소유자 판단).
