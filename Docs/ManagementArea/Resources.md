...
- **페이즈 전환 요청**: 패널 버튼 → `ManagementEndDayConfirmPopup.Request(controller)` → 낮 프로세스 조건
  (유휴 주민 없음 `!HasIdleVillagers` — 영토 미확장 경고는 #337에서 삭제돼 **경고는 주민 미배치 하나뿐**)을
  점검해 **충족이면 곧장**, 미충족이면 **확인 팝업을 띄우고 [계속] 선택 시** `ManagementController.EndDay()` → `DayNightManager.EndDay()`.
...
2. 낮 종료 버튼 → 유휴 주민이 남아 있으면 **확인 팝업**, [계속]이면 낮→밤 정산(자원이 HUD에 증가).
   다 배치했으면 팝업 없이 바로 넘어간다. 버튼은 어느 경우에도 비활성화되지 않는다(#219).
