...
| `LaserAction` (#300) | `NightOnly` | 조준 방향 **사각 범위 안 전원** | 주기 발사(`Interval`) + 자체 충전 |
...
- **`LaserAction`이 세 번째 공격 축인 이유**: 투사체가 없고(히트스캔) 대상을 지속 잠그지도 않는다.
  조준 방향으로 박스 판정 1회를 던져 **범위 안 전원을 동시에** 때린다 — 관통도 연쇄도 아니다.
  대상 탐색은 "어디를 조준할지"만 정하고 **누가 맞을지는 박스가 정한다.** 자기 충전을 직접 소유하며
  근거는 §3.10.
...
| 축 | 부품·저작 위치 | 트리거 | 적용 | 리셋 |
|---|---|---|---|---|
| **원장형** | `RampAction` + `TowerAsset.Ramp` | `Hit`(명중) / `Kill`(처치) | `Owner.Stats`에 소스 1개 | 감쇠 타이머 |
| **대상별** | `BeamAction` + `Beam.LockRamp` | 같은 대상을 잠근 경과 시간 | 그 대상의 피해에만 곱 | **대상이 바뀌면 0** |
| **충전형** | `LaserAction` + `Laser.ChargeRamp` | 마지막 발사 이후 경과 시간 | 레이저 **폭**(배율) + **피해**(절댓값) | **발사 즉시 0** |

**왜 하나로 합치지 않는가.** 세 축이 원장으로 수렴할 수 없는 이유가 각각 다르다.

- **대상별**: 원장은 타워 단위라 ① 목표를 바꿔도 배율이 남고 ② 그 타워의 다른 액션과 DoT까지 함께
  강해진다 — "한 놈을 오래 지질수록 아프다"를 표현할 수 없다.
- **충전형**: **원장에 "폭" 축이 아예 없다.** `TowerStat`은 3축이고 `CombatSpace.TileBuffStat`과 1:1
  대응이라 축을 늘리면 타일 버프까지 번진다. 폭을 기존 축에 매핑하면 의미가 어긋나는데, **같은 문제가
  이미 WL-127로 열려 있다**(슬로우 강도를 넣을 축이 없다, §5.3).
- 반대로 원장형을 액션 국소로 만들면 명중·처치 램프가 액션마다 따로 굴러 정보 패널·사거리 원과 어긋난다.

**공유는 수치 규약(`RampProfile`)까지, 적용은 각자**가 결론이다.

> **충전형이 소유권까지 액션에 두는 것은 함정 하나를 구조적으로 제거한다.** 충전을 `RampAction`의
> 트리거로 뒀다면 리셋 신호가 `Tower.OnFired`이므로 `LaserAction`이 그것을 **직접 발행**해야 하고,
> 잊으면 경고도 예외도 없이 스택이 영영 안 풀린다. 자기가 소유하면 발사와 리셋이 같은 코드 경로다.

**충전 피해가 배율이 아니라 절댓값인 이유**: 배율이면 기여가 `기본값 × 스택 × 계수`라 나중에 기본
공격력을 조정할 때 충전 보상까지 함께 움직인다. 절댓값이면 두 값을 독립적으로 밸런싱할 수 있다.
원장에 새 소스를 만들지 않고 **`Evaluate(stat, baseValue)`의 기본값에 더해서** 넘긴다 — 그 대신
충전분도 타일 버프·오라 배율에 함께 증폭되며, 이것이 상한 산정의 기준 조합이다.

> ⚠ **밸런싱에서 진짜 증폭원은 폭 × 피해의 곱이다.** 총량이 `덮은 적 수(폭) × 적당 피해`라 두 축이
> 같이 오르면 곱이 된다 — 절댓값이 적당 피해를 선형으로 억제해도 이건 남는다. 폭 상한을 보수적으로
> 잡는 쪽이 안전하다.
...
- 레이저(#300): 사거리·발사 주기는 원장을 타고, **충전 폭은 축이 없어서 타지 않는다.** 충전 피해는
  소스를 만들지 않고 `Evaluate`의 **기본값에 더해** 넘긴다 — 아래 ② 슬로우 강도와 같은 "축 부재"
  사례이므로 축을 신설할 때 함께 검토할 것(WL-127)
- **미연결 2건**:
...
| 14차 (#300 4번째 타워) | **§3.5를 5종 → 6종**(`LaserAction` — 투사체 없고 대상 잠금도 없는 세 번째 공격 축, 조준 방향 사각 범위 즉시타). **§3.10에 충전형 램프 축 추가** — 세 축(원장형/대상별/충전형)이 원장으로 수렴할 수 없는 이유가 각각 다르다는 것을 명시: 대상별은 "대상이 바뀌면 리셋"을, 충전형은 **"폭" 축 자체가 원장에 없다**(WL-127과 같은 축 부재). 충전 소유권을 액션에 둔 것이 `Tower.OnFired` 발행 누락 함정을 구조적으로 제거하는 근거, 충전 피해가 배율이 아니라 절댓값인 근거(기본 공격력과 독립 밸런싱), 폭 × 피해가 곱으로 작용한다는 밸런싱 경고를 함께 기록. §5.3에 레이저의 원장 커버리지(사거리·주기는 타고 폭은 축이 없어 안 탄다) 추가 |
