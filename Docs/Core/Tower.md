...
4. `OnWaveEnd`(가상, 기본 no-op)는 웨이브 종료(밤→낮) 시 호출된다 — "이 웨이브 동안만" 유효한 상태를
   버리는 자리다(#300). **호스트가 부른다**: `Tick`의 페이즈 게이팅과 같은 이유로 액션이 직접
   `DayNightManager`를 구독하지 않는다. 호스트는 이벤트가 아니라 **이미 매 프레임 읽는 페이즈 값의
   전이**를 본다 — ① 게이팅과 신호원이 하나로 유지되고(WL-044 축) ② `Instance`가 아직 null인 시점에
   조립된 타워(씬 선배치·테스트 씬)가 구독을 놓쳐 **그 타워만 영구히 통지를 못 받는** 무증상 실패가
   없으며 ③ 구독 해제 누락 경로가 없다. `ActivePhase`와 **무관하게** 호출된다 — 밤에만 도는 액션도
   웨이브 종료는 들어야 한다
...
| `BeamAction` (#298) | `NightOnly` | 사거리 내 최대 `MaxTargets`기 (**지속 잠금**) | 주기 피해(`TickInterval`) |
| `RampAction` (#300) | **`Always`** | 자기 타워(원장) | 트리거 이벤트 + 감쇠 타이머 |
...
### 3.10 성장(램프업) — 축이 둘인 이유 (#300)

전투 실적으로 타워가 스스로 강해지는 축. **지금까지 타워 스탯을 바꾸는 소스는 전부 외부에서 왔다**
(타일 버프·오라·플레이어 스킬·보스 봉인) — 이것이 자기 실적이 원장에 얹히는 첫 소스다.

수치 규약은 부품 하나([`RampProfile`](../../Assets/Scripts/CombatSystem/Tower/RampProfile.cs))가
소유하지만 **적용 지점은 둘로 갈린다.**
...
남고 ② 그 타워의 다른 액션과 DoT까지 함께 강해진다 — "한 놈을 오래 지질수록 아프다"(단일 인페르노)를
표현할 수 없다. 반대로 원장형을 대상별로 만들면 명중·처치 램프가 액션마다 따로 굴러 정보 패널·사거리
원과 어긋난다. **공유는 수치 규약까지, 적용은 각자**가 결론이다.

**원장형이 기존 액션을 한 줄도 고치지 않는 이유**: `AttackAction.Damage`/`Interval`,
`BeamAction.DamagePerTick`/`TickInterval`, DoT 수치가 **이미 전부** `Owner.Stats.Evaluate`를 통과한다.
원장에 소스를 얹으면 공격·빔·DoT·정보 패널·선택 사거리 원이 자동으로 따라온다.

**트리거 두 종의 신호원**

- `Hit` → `Projectile.DamageDealt`(기존 static 이벤트). ⚠ 히트스캔(`BeamAction`)은 이 이벤트를
  발행하지 않으므로(설계 의도, [TowerAddGuide §6](TowerAddGuide.md)) **"빔 + 명중 램프"는 조용히
  스택이 0에 머문다** → `TowerAsset.OnValidate`가 조합을 경고한다.
- `Kill` → `Enemy.Killed`(#300 신규). `Die()`의 `isDying` 게이트 뒤에서 1회 발행되며, **본진 도달
  이탈은 `Die()`를 거치지 않아 킬로 세지지 않는다**(`HandleRouteCompleted`가 바로 Destroy). DoT 처치는
  `StatusEffectHandler`가 원 소스를 실어 보내므로 귀속되고, 스킬 처치는 소스가 `null`이라 귀속되지 않는다.

#### 성장은 웨이브를 넘기지 않는다

두 축 모두 웨이브 종료(밤→낮)에 초기화된다 — `TowerAction.OnWaveEnd()`(§3.3 규약 4번).
그래서 **`DecaySeconds = 0`은 "영구"가 아니라 "웨이브 동안 유지"**를 뜻한다.

경계를 두는 근거 둘: ① 성장이 Run 내내 누적되면 밸런싱이 후반 한 점에 묶인다 ② 이어하기(#270)가
스택을 저장하지 않아 "저장 후 재개하면 약해지는" 불일치가 생긴다. 웨이브 안으로 가두면 둘이 함께
사라진다(저장할 상태가 없어진다).

> `BeamAction`도 이 훅을 구현한다 — 램프 때문만이 아니라 **낮에 빔이 켜진 채로 남는 문제**(#298부터
> 존재) 때문이다. `NightOnly`라 낮에는 `Tick`이 아예 돌지 않아 잠금 정리도 빔 표시 갱신도 멈추고,
> 밤 마지막 프레임의 `LineRenderer.enabled = true`가 그대로 굳는다.
...
| **성장(램프업)** | `TowerAction.SourceId` (호스트 ID ^ 액션 타입명) | 지속형 — 감쇠·웨이브 종료로만 해제 |
...
#### ① 에피소드 시작 기준 천장 — 티어별 스턴 지속 대응

원래 규칙 ①은 "스턴 중 재적용 **무시**"였다. 티어별로 스턴 지속이 다른 스턴 타워(1티어 0.7s /
2티어 1.0s)가 생기면서 그 규칙이 틀린 답을 냈다 — 둘이 거의 동시에 맞히면 먼저 도착한 1티어가
슬롯을 잡고 2티어 명중이 **통째로 버려져** 총 0.7초가 됐다.

지금은 스턴 중에 들어온 재적용을 버리지 않고 `ExtendStun()`으로 넘겨, 종료 시각을
**`에피소드 시작 + 이번 지속`까지만** 끌어올린다. **지금 시각 기준이 아닌 것이 핵심이다** —
시작 기준이라 명중이 반복될수록 후보가 계속 줄어들어 천장을 넘지 못한다.

| 상황 | 결과 |
|---|---|
| T1(0.7) t=0, T2(1.0) t=0.05 | 후보 `1.0−0.05=0.95` > 남은 `0.65` → 채택. **총 1.0초** |
| T2(1.0) t=0, T1(0.7) t=0.05 | 후보 `0.7−0.05=0.65` < 남은 `0.95` → 무시. **총 1.0초** |
| T1만 0.1초 간격 도배 | 후보 `0.7−t`가 항상 남은 시간 이하 → 전부 무시. **0.7초 고정** |

따라서 한 에피소드 길이의 상계는 **저작된 스턴 지속의 최댓값**이고, 소프트락 방어는 그대로 유지된다.

⚠ 이 불변식은 "대상당 스턴 슬롯이 하나"에 의존한다. `ExtendStun`이 들어온 `effectId`가 아니라
`stunSource` 슬롯을 갱신하는 이유다 — 티어별로 `effectId`를 갈라 슬롯이 둘이 되면 두 스턴이
합집합으로 이어져 천장이 사라진다. `SharedEffectId`를 인스턴스별로 바꾸려는 논의
([TowerRedesign.md](TowerRedesign.md) §12 #1)는 이제 이 불변식과 함께 검토해야 한다.

⚠ 늦게 도착한 상위 티어는 손해를 본다. 2티어가 `t=0.6`에 오면 `0+1.0−0.6 = 0.4초`만 더 받는다
(단독이면 1.0초). "비슷한 시점"을 전제한 의도된 트레이드오프다 — 피하려면 소스별 합집합으로
가야 하는데 그건 천장이 사라지는 방향이다.

#### ② 면역 창은 저작 값이다 (#441)

`StunStatus.ImmunityWindow`(기본 0.4)가 소유하고, `ApplySlow`가 선택 인자로 받는다. 값은
**종료 시각을 결정한 스턴의 것**을 `EndStun`까지 들고 간다 — 에피소드 도중 더 긴 스턴이 천장을
끌어올리면 그 스턴의 창으로 교체된다(위 표의 1·2행이면 2티어의 창). 만료 시점에 조회하면 그 사이
다른 부여자가 값을 바꿨을 때 어느 스턴의 규칙인지가 흐려진다. `StatusEffectHandler.stunImmunityWindow`
...
⚠ **규칙 ①의 천장은 지우면 안 된다.** 밤 종료 조건이 몬스터 전멸이므로(`MonsterSpawn`) 영구 정지는 곧
밤이 끝나지 않는 것이다. 면역 창을 0으로 두는 것도 같은 방향의 위험인데, 그 타워가 대상을 스스로
죽이지 못할 때(딜을 걷어낸 순수 유틸 스턴 타워) 드러난다.

⚠ `TowerAsset.OnValidate`의 두 경고는 **에셋 1개 = 티어 1개** 기준이다. 서로 다른 티어가 섞였을 때의
상호작용(2티어 스턴이 1티어의 창을 덮어쓰는 조합)은 SO 단위 검증 범위 밖이라 잡히지 않는다.

> ⚠ **이 static이 상한의 근거는 아니다.** 게이트는 `ApplySlow`([:151](../../Assets/Scripts/CombatSystem/StatusEffect/StatusEffectHandler.cs))에 있고,
> `stunActive` / `stunImmuneUntil` / `stunEpisodeStart` 셋 다 **핸들러(대상) 인스턴스 필드**이며,
> `slows` 딕셔너리 조회보다 **앞에** 있다. 따라서 `effectId`가 타워별로 갈려도 두 번째 스턴은 같은
> 자리에서 `ExtendStun`으로 흡수되지 새 에피소드를 열지 못한다. static인 것은 **중복 방지일 뿐**이고,
> 상한은 전적으로 대상 쪽 두 규칙에서 나온다.

---

## 타워 등급

타워 도감은 타워를 다음 세 등급으로 분류한다.

- Normal: 일반 타워
- Rare: 희귀 타워
- Legendary: 전설 타워

현재 등급은 도감의 표시 및 정렬을 위한 임시 분류 기준이다.

등급에 따른 등장 확률, 웨이브 보상, 상점 가격, 합성 확률 등 게임플레이 규칙은 아직 확정되지 않았다. 해당 시스템에서 등급을 사용하려면 별도의 기획 합의가 필요하다.

현재 `TowerRarity` 값은 `TowerAsset`에 저장되어 있다. 다만 타워별 스칼라 데이터는 CSV에서 관리한다는 프로젝트 원칙에 따라, 등급 시스템이 확정되면 `TowerTable.csv`와 `TowerData`로 이전한다.

신규 타워의 기본 enum 값이 `Normal`로 처리될 수 있으므로, 새 타워를 추가할 때 등급을 명시적으로 검토해야 한다.
...
| 13차 (#300 구현) | **§3.10 성장(램프업) 신설** — 축이 둘(원장형 `RampAction` / 대상별 `Beam.LockRamp`)인 이유, 하나로 합칠 수 없는 이유(원장은 타워 단위라 "대상이 바뀌면 리셋"을 표현할 수 없다), 트리거 두 종의 신호원(`Projectile.DamageDealt` / 신규 `Enemy.Killed`)과 귀속 규칙(경로 이탈은 킬이 아니다·DoT는 귀속·스킬은 미귀속). **성장은 웨이브를 넘기지 않는다** — §3.3에 생명주기 규약 4번(`OnWaveEnd`) 추가, 호스트가 이벤트가 아니라 페이즈 값의 전이를 보는 이유 3가지 명시. §5.2에 성장 소스키 + 타워당 램프 1개 제약, §5.3에 램프 축 커버리지 추가. **§3.5를 3종 → 5종으로 정정** — #298의 `BeamAction`이 표에 누락돼 있었다(그 이슈에서 이 문서가 갱신되지 않음). 빔이 낮에도 켜진 채 남던 #298 버그를 `BeamAction.OnWaveEnd`로 해소한 것도 기록 |
