...
더 읽기 어렵다」). 그 결과 **본진이 깎이는 소리가 전부 카메라에 종속됐고** 그것이 WL-229가 됐다.
...
**WL-230와의 관계.** ③이 「화면 밖에서 본진 피격을 모른다」를 임계 경보로 닫는다. 임계 방식 자체가
WL-229가 제안한 형태다. 남은 것은 `BaseHealthBar`의 피격 순간 연출(바 플래시)이며 UI 소유라

**WL-230와의 관계.** ③이 「화면 밖에서 본진 피격을 모른다」는 부분을 닫는다. 남은 것은 *매 피격
경고가 아니라 임계 경보로 격상할지*와 `BaseHealthBar`의 피격 순간 연출이며, 그쪽은 UI 소유라
N0WST4NDUP 합의가 선행이다.
