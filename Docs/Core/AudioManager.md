...
- `warningMinInterval`(기본 1.2초)은 이제 주 게이트가 아니라 **자기 소리 겹침 방지 하한**이다.
  ...
  이 하한에 막힌 통과는 그냥 버린다 — 1초 전에 이미 울렸으므로 "위험하다"는 전달됐고, 큐에 쌓아
  뒤늦게 울리면 상황과 어긋난 시점에 경보가 난다.
...
**WL-229와의 관계.** ③이 「화면 밖에서 본진 피격을 모른다」를 임계 경보로 닫는다. 임계 방식 자체가
WL-229가 제안한 형태다. 남은 것은 `BaseHealthBar`의 피격 순간 연출(바 플래시)이며 UI 소유라

**WL-229와의 관계.** ③이 「화면 밖에서 본진 피격을 모른다」는 부분을 닫는다. 남은 것은 *매 피격
