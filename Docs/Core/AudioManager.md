...
| 경로 | 볼륨 제어 | 성격 |
|---|---|---|
| `CombatSfxPool` | ✅ 매 프레임 `GetEffectiveVolume(Sfx)`를 곱한다 | 전투 위치 기반 스킬·타워·몬스터 효과음 |
...
- 몬스터 전투음: 공격음 `Low`(클립 스로틀 0.08초) · 돌진 충돌음 `Normal` · 자폭 폭발음 `High`. 클립 주인은 `EnemyAsset`이다(§6.4)
- **클립 단위 스로틀**(`Play`의 `minRepeatInterval`, §6.4): 같은 클립이 지정 간격 안에 다시 요청되면 보이스를 잡지 않는다. 소리의 주인이 개체로 흩어진 소비처(몬스터 평타)를 위한 것이고, 실시간 기준이며 씬 전환에서 장부를 비운다. ⚠ 보이스 획득 **전에** 걸러야 한다 — 뒤에서 거르면 탈취를 이미 돌린 요청이 울리던 소리만 뺏는다
- ⚠ **본진 경고음은 이 풀이 아니라 2D다**(§6.4). 「본진이 깎였다」는 카메라와 무관하게 들려야 하므로 `Sfx.BaseDamaged()` → `AudioManager.PlaySfx` 경로다
...
셋 다 `CombatSfx.Play(..., priority: Low)`로 나가므로 상한에서 스킬음보다 먼저 회수된다.
⚠ **본진 경고음은 비교 대상이 아니다** — 그쪽은 이 풀을 쓰지 않는 2D 경로라(§6.4) 보이스 상한을
두고 경쟁하지 않는다. 우선순위는 `CombatSfx` 안에서만 의미가 있다.
...
- ⚠ **몬스터 전투음과 음색이 겹치지 않게 한다**(§6.4). 자폭 폭발음과 돌진 충돌음은 본진 HP가 크게
  깎이는 **위험 신호**인데, 타워가 0.75초마다 같은 음색을 내면 플레이어가 그것을 일상음으로 학습해
  정작 반응하지 않게 된다. 실제로 캐논이 한 번 그 상태였다.
  ⚠ **타워음·몬스터음이 같은 풀·같은 감쇠를 타므로 구분할 단서가 음색뿐이다.** 본진 경고음만 2D로
  경로가 갈리고, 그 셋은 서로 음색으로만 갈린다.

### 6.4 몬스터 전투음 · 본진 경고음 (#540 후속)

| 축 | 저작 위치 | 발화 지점 | 경로 | 성격 |
|---|---|---|---|---|
| 몬스터 공격음 | `EnemyAsset.AttackSfx` + `AttackSfxVolume` | `Enemy.TryAttack` | `CombatSfx` `Low` + 클립 스로틀 0.08s | 사건 · 원샷 |
| 몬스터 돌진 충돌음 | `EnemyAsset.ImpactSfx` + `ImpactSfxVolume` | `EnemyImpactTargetAction` | `CombatSfx` `Normal` | 사건 · 원샷 |
| 자폭 폭발음 | `EnemyAsset.SelfDestruct.ExplosionSfx` + `ExplosionSfxVolume` | `Enemy.Detonate` | `CombatSfx` `High` | 사건 · 원샷 |
| **본진 경고음** | `SfxBank.BaseDamaged` | `PlayerBase.TakeDamage` | **`AudioManager.PlaySfx`(2D)** | 사건 · 원샷 · **디바운스** |

**타격음의 주인은 때리는 쪽이다.** 타워가 `TowerAsset.Attack.FireSfx`를 들고 자폭병이
`SelfDestruct.ExplosionSfx`를 드는 것과 같은 규칙이다(§5.4). 본진은 타격음을 소유하지 않는다.

⚠ **이 절은 두 번 뒤집혔다. 되돌리려면 아래 근거를 먼저 읽을 것.**
① 처음에는 자폭음이 2D였다(「본진 HP 10%는 위험 신호라 화면 밖에서도 들려야 한다」).
② #565가 그것을 `CombatSfx`로 옮기고 본진에 위치 기반 피격음을 붙였다(「그림 없이 폭음만 나는 쪽이
더 읽기 어렵다」). 그 결과 **본진이 깎이는 소리가 전부 카메라에 종속됐고** 그것이 WL-230이 됐다
(등재 당시 번호는 WL-229였는데 main의 #570이 그 번호를 먼저 점유해 밀렸다).
③ 지금은 **축을 둘로 갈랐다** — 위치음은 가해자가, 「본진이 깎였다」는 2D 경고음이 맡는다.

**한 소리가 두 역할을 겸할 수 없다는 것이 ③의 근거다.** 타격음은 *어디서 무엇이 때렸는가*를 알리므로
위치·감쇠가 있어야 하고, 경고음은 *본진이 깎였다*를 알리므로 카메라와 무관해야 한다. 요구가 정반대라
한 소리로 묶으면 둘 중 하나가 반드시 어긋난다. ①은 후자만, ②는 전자만 만족했다.

**부수 효과 셋.**
- 고블린 칼질·박쥐 물기·탱크 주먹이 **각자의 소리**를 갖는다. 예전에는 전부 같은 클립이었고
  가르려면 받는 쪽에 특례를 얹어야 했다.
- **몬스터가 병사를 때릴 때도 소리가 난다.** 예전에는 본진만 소리를 소유해 그쪽이 무음이었다.
- 본진 경고음 클립이 `SfxBank`(메인 저장소 `Resources`)로 옮겨져 **중첩 저장소 배선이 이 축에서
  사라졌다.** `PlayerBase`의 직렬화 필드로 두면 씬의 그것이 `Assets/Imported`의 `Gate_02.prefab`이라
  메인 저장소만 받은 환경에서 조용히 무음이 된다. 뱅크가 배제하는 것은 "타워 발사음처럼 각자의 SO가
  이미 클립을 드는 소리"이고(§5.4) 본진은 SO가 아니라 런당 하나뿐인 씬 오브젝트라 그 경우가 아니다.

**클립 단위 스로틀(`CombatSfx.Play`의 `minRepeatInterval`).** 소리의 주인이 개체로 흩어지면 창구도
흩어진다 — 개체마다 타이머를 둬도 **서로를 모르기 때문에** 성문에 8마리가 붙은 순간 8개가 동시에
터진다. 풀은 보이스 상한만 지키므로 청감상 뭉치는 것은 막지 못한다. 그래서 풀이 **클립을 키로**
거른다. 개체 기준이면 위 문제가 그대로고, 위치 기준이면 같은 지점의 서로 다른 소리까지 함께 잘린다.
`SfxBank.Cue.LastPlayedFrame`이 같은 발상을 프레임 단위로 이미 쓰고 있고 이쪽은 그 초 단위 확장이다.
- 평타는 0.08초(초당 최대 12회) — 개별 타격은 살아 있고 뭉침만 걷힌다.
- 돌진에는 걸지 않는다. `tank` P1은 1회 한정 패턴이라 자기끼리 겹칠 일이 없고, 걸어 두면 놓칠 위험만 생긴다.
- ⚠ **보이스를 잡기 전에 거른다.** 뒤에서 거르면 탈취 판정을 이미 돌린 뒤라, 스로틀에 막힐 요청이
  화면 안에서 울리던 소리를 뺏고서 자기는 재생하지 않는 조합이 생긴다.
- ⚠ 기준은 `Time.unscaledTime`이다. 오디오 뭉침은 배속과 무관한 실시간 현상이라, 스케일 시간을 쓰면
  2배속에서만 창이 늘어나 그 배속에서 소리가 성기어진다.

**공격음을 `TryAttack` 안에서 내는 이유.** 와인드업(#452)이 이미 그 호출을 **애니메이션의 타격
프레임**으로 밀어 놓았다. 스윙 시작 시점에 내면 모션보다 소리가 앞선다. 근접은 타격 순간, 원거리는
발사 순간이 되며 한 필드가 "이 몬스터가 공격하는 소리" 하나를 뜻한다 — 원거리의 착탄음이
필요해지면 그때 축을 가른다(타워 `ImpactSfx`가 선례).

**본진 경고음은 매 피격이 아니라 HP 임계를 아래로 통과할 때 1회 울린다.** 사이렌은 「지금 위험하다」는
**상태** 신호라 드물게 울려야 무게가 산다. 한 방이 본진 HP의 5~25%(잡몹 10 ~ 탱크 50 / 200)여서 매
피격에 물리면 공성 중 거의 연속으로 울리고, 연속으로 울리는 사이렌은 정보가 아니라 밤의 배경음이 된다
— 특히 자폭병은 웨이브당 설계된 이벤트라(규약 ④, W7·W11·W15에 11마리) 런 내내 11번 울렸다.
개별 타격의 피드백은 가해자의 위치음이 이미 담당하므로 경고가 그것을 겸할 이유가 없다.

- 저작은 `PlayerBase.warningHpThresholds`(기본 `0.75 / 0.5 / 0.25 / 0.1` → 본진 200 기준 150·100·50·20 HP).
  런당 최대 4회다.
- **한 방에 임계 둘을 넘어도 1회만 울린다.** 통과 개수를 한꺼번에 올려 두므로 다음 피해가 같은
  임계로 다시 울리지 않는다(탱크 램 50 = 25%가 그런 경우다).
- ⚠ **통과 판정을 「만족하는 임계의 개수」로 센다.** 비율이 내려갈수록 그 개수는 단조 증가하므로
  **배열 정렬 순서에 의존하지 않는다.** 인덱스나 값으로 추적하면 정렬 전제가 생기고, 저작자가
  순서를 섞어 넣으면 조용히 틀린다.
- ⚠ **세이브 복원에서 장부를 동기화한다**(`SyncWarningThresholds`). HP 40%로 이어하기를 하면 75%·50%가
  이미 지나간 상태인데 장부가 0이면 복원 직후 첫 피격에 그 둘을 "새로 통과"한 것으로 보고 사이렌이
  울린다. 증상이 "이어하기 하면 맞을 때 경보가 이상하게 난다"라 원인에서 멀다.
- ⚠ **본진 회복이 들어오면 그 경로에서도 같은 함수를 불러야 한다**(WL-206의 「낮 시작 본진 회복」).
  안 부르면 회복 후 다시 내려갈 때 경보가 재무장되지 않아 조용히 지나간다.
- `warningMinInterval`(기본 1.2초)은 이제 주 게이트가 아니라 **자기 소리 겹침 방지 하한**이다.
  `PlaySfx`는 `PlayOneShot`이라 동시재생 상한이 없어서, 임계 둘을 짧은 간격으로 연달아 통과하면
  사이렌 두 벌이 겹쳐 볼륨이 튄다. **클립 길이 이상으로 잡는다**(현재 클립 1.10초).
  ⚠ **하한에 막히면 장부(`announcedThresholds`)를 올리지 않고 돌아간다 — 순서가 핵심이다.**
  장부를 먼저 찍고 창을 검사하면 막힌 통과가 **울리지도 않은 채 알린 것으로 기록돼 영영 사라진다.**
  위험한 시나리오가 정확히 그것이다: 75% 경보 직후 0.5초 만에 25%·10%로 연쇄 하락하면
  (탱크 램 50~90 한 방이면 임계 둘을 건너뛴다) **가장 절박한 경보가 조용히 씹힌다.**
  장부를 미뤄 두면 다음 피해가 같은 통과를 다시 시도해 창이 지나는 즉시 울리므로, 최대 하한만큼
  늦을 뿐 잃지 않는다. 피해가 멎었다면 위급 상황도 지난 것이라 울리지 않는 편이 맞다.
  「새 임계면 창을 무시하고 울린다」로 가지 않은 이유: 같은 임계로는 애초에 다시 오지 않으므로
  그 조건이 곧 "항상 무시"가 되어 창이 사라지고, 사이렌 두 벌이 겹쳐 볼륨이 배로 튄다
  (`PlaySfx`는 `PlayOneShot`이라 상한이 없고 현재 클립 peak가 -0.2dBFS라 두 벌이면 풀스케일에 닿는다).

**피해 종류를 보지 않는다.** 자폭·돌진·평타가 모두 같은 규칙을 탄다. 예전에는 자폭을 억제하고 돌진에
디바운스 우회를 뒀는데, 그 둘은 「매 피격마다 울린다」는 전제 위의 보정이었다 — 임계 방식에서는 큰
피해가 **자연히** 임계를 넘고(탱크 램 25%는 사실상 항상 넘는다) 작은 피해는 넘을 때만 울리므로
종류별 특례가 필요 없다.

⚠ 그래서 그 특례를 위해 만들었던 **`DamageKind` 축을 함께 걷어냈다**(`DamageInfo`는 `Amount`·`Source`
둘로 돌아갔다). 읽는 곳이 없는 축을 남기면 다음 사람이 "누가 쓰나"를 다시 추적하게 된다.
받는 쪽에서 사건 종류가 다시 필요해지면 그때 넣는다 — 호출부 두 곳(`Enemy.Detonate`,
`EnemyImpactTargetAction`)에 선택 인자를 얹는 비용이라 되돌리기가 싸다.
- ⚠ 그때도 **`Source`를 캐스팅해 되묻는 방식은 쓰지 말 것.** `Source is Enemy && IsBoss` 같은 술어는
  「자폭 가능한 적」·「보스인가」이지 「이번 피해가 무엇인가」가 아니라서, 조합이 늘면 조용히 틀린다
  (돌진은 `Source`가 `null`이라 되묻는 방법 자체가 없다). 종류를 아는 것은 가해자뿐이므로
  가해자가 실어 보내야 한다.
- ⚠ 이름에 `Boss`를 박지 말 것. `EnemyImpactTargetAction`은 보스 전용 노드가 아니라 충돌 피해
  노드다 — 돌진하는 일반 몹이 생기면 그쪽도 같은 사건을 낸다(`EnemyType`이 역할을 겸직해 생긴
  WL-207과 같은 형태). 현재 이 노드를 쓰는 것은 `TankBossBehavior` 하나다
  (`ogre_king`의 `MidBossBehavior`에는 0건).
⚠ **`ExplosionSfx`가 비면 자폭이 소리 없이 터진다.** 자폭병은 평타 경로를 타지 않아 `AttackSfx`도
읽히지 않으므로 대신 나 주는 소리가 없다 — 경고음은 임계를 넘을 때만 울리므로 자폭 한 번이 경고를
부르지 않을 수 있고, 부른다 해도 그것은 「본진이 위험하다」이지 「무언가 폭발했다」가 아니다.
`EnemyAsset.OnValidate`가 저장 시점에 경고한다.

**실제로 깎인 값이 0이면 경고하지 않는다.** HP가 바닥나 클램프된 잉여 피해와, 데미지 0으로 저작된 적
(자폭병의 `Melee.Stat.AttackDamage`가 그렇다)이 여기 걸린다.

**경고를 `TakeDamage` 한 곳에서 내는 이유.** 평타·자폭·돌진이 전부 지나는 단일 창구다. 구독
컴포넌트나 `InGameCue`로 빼면 프리팹·씬에서 빠졌을 때 에러 없이 조용히 무음이 된다 — 발사음을
`Tower.RaiseFired`에 둔 것과 같은 축이다(§6.3).

**WL-230과의 관계.** ③이 「화면 밖에서 본진 피격을 모른다」를 닫는다 — 경고음이 2D라 카메라와
무관하게 들린다. **「매 피격이 아니라 임계 경보로」도 이 절에서 결정됐다**(위 임계 문단). 둘 다
WL-230이 제안한 형태다.

남은 것은 `BaseHealthBar`의 피격 순간 연출(바 플래시) 하나이며, UI 소유라 N0WST4NDUP 합의가 선행이다.

⚠ **원장(WL-230)의 서술이 현재 코드보다 낡았다.** 「본진 피격음도 같은 풀에 붙어 두 소리 모두 화면
밖에서 무음」이라고 적혀 있는데 그것은 ②의 상태다 — 지금은 ③이라 본진 소리가 2D 임계 사이렌이다.
원장을 갱신할 때 이 절을 근거로 삼을 것.
...
- [x] **몬스터 전투음 · 본진 경고음** — 축을 둘로 갈랐다(§6.4). 위치음은 가해자(`EnemyAsset`)가,
      「본진이 깎였다」는 2D 경고음(`SfxBank.BaseDamaged`)이 맡는다. 클립 단위 스로틀도 이때 들어왔다.
      ⚠ **이 항목은 두 번 뒤집혔다** — 「자폭음은 2D」 → 「전부 위치음」 → 「축 분리」. 되돌리려는
      생각이 들면 §6.4의 ①②③를 먼저 읽을 것. 한 소리로 두 역할을 겸하려는 시도가 두 번 실패했다.
      ⚠ **실질 상한은 1.0이다.** `AudioSource.volume`이 0~1로 클램프되므로 화면 중앙·설정 100%에서는
      `volumeScale > 1`이 잘린다 — `[Range(0,2)]`의 윗칸은 감쇠가 먹은 뒤(gain < 1)에만 쓰인다.
- [ ] **몬스터별 `AttackSfx` 클립** — 필드만 있고 **9종 전부 비어 있다**(자폭병은 평타 경로를 타지
      않으므로 대상 외). 비면 조용히 무음이라 게임은 깨지지 않는다. 배선되면 고블린 칼질·박쥐 물기가
      갈리고, 그때 클립 RMS를 아처 기준선(§6.3)에 맞춰 `AttackSfxVolume`을 잡는다
