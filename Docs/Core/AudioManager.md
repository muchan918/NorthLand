...
| 경로 | 볼륨 제어 | 성격 |
|---|---|---|
| `CombatSfxPool` | ✅ 매 프레임 `GetEffectiveVolume(Sfx)`를 곱한다 | 전투 위치 기반 스킬·타워·본진 효과음 |
...
- 본진 피격음·자폭 폭발음: 둘 다 `High` 단발. 본진 쪽에만 디바운스가 붙고, 자폭 피해에는 피격음이 억제된다(§6.4)
...
- ⚠ **자폭병 폭발음·본진 피격음과 음색이 겹치지 않게 한다.** 그 둘은 본진 HP가 깎이는 **위험 신호**인데,
  타워가 0.75초마다 같은 음색을 내면 플레이어가 그것을 일상음으로 학습해 정작 반응하지 않게 된다.
  실제로 캐논이 한 번 그 상태였다.
  ⚠ **셋 다 같은 풀·같은 감쇠를 타게 된 뒤로 이 규약이 더 중요해졌다**(§6.4). 예전에는 자폭음만 2D라
  경로 자체가 달라 음색이 비슷해도 어느 정도 갈렸는데, 지금은 **구분할 단서가 음색뿐이다.**

### 6.4 본진 전투음 — 피격음 · 자폭 폭발음 (#540 후속)
...
**자폭 폭발음은 2D 원샷에서 여기로 옮겨왔다.** 예전에는 `AudioManager.PlaySfx`로 나가서 카메라가 본진에
없어도 같은 크기로 울렸고, 근거는 "본진 HP 10%가 날아가는 위험 신호라 화면 밖에서도 들려야 한다"였다.
그림 없이 폭음만 나는 쪽이 더 읽기 어렵다는 판단으로 뒤집었다. **이 문서의 다른 곳에 남아 있던
「자폭음은 2D다」 서술은 전부 이 절 기준으로 무효다.**

**우선순위가 `High`인 이유.** 타워 셋(발사·착탄·빔)은 전부 `Low`라 상한(32)에 닿으면 먼저 회수된다.
본진이 깎이는 것은 런의 패배 조건에 직결되는 사건이라, 개틀링 발사음에 밀려 잘리면 **가장 중요한
소리만 사라진다.** 스킬 시전·착탄음과 같은 급이다.

**자폭 순간에는 폭발음만 난다 — 본진 피격음은 억제된다.** 둘 다 같은 프레임·같은 지점에서 나므로
겹치면 뭉치고, 그 자리의 피드백은 폭발음이 이미 다 한다. 자폭 아닌 모든 피해(평타·보스 충돌)는
평소대로 피격음을 낸다.

가르는 축은 `DamageInfo.Kind`(`DamageKind.Attack` / `SelfDestruct`)이고 `Enemy.Detonate`만 후자를 넘긴다.
⚠ **`Source is Enemy`를 캐스팅해 "자폭병인가"를 되묻는 방식은 쓰지 않는다.** 그 술어는 「자폭 가능한
적」이지 「이번 피해가 자폭」이 아니라서, 자폭병이 평타도 하게 되는 날 조용히 틀린다. 종류를 아는 것은
가해자뿐이므로 가해자가 실어 보낸다.
...
**디바운스는 피격음에만 있다.** 밤 후반에는 본진에 붙은 근접 몹이 여러 마리라 같은 프레임에 피해가
여러 번 들어오고, 그대로 두면 한 번에 여러 보이스를 물어 소리가 뭉치면서 풀 상한까지 갉는다.
자폭은 웨이브당 몇 번뿐이라 이 축이 필요 없다.
⚠ **기준이 `Time.unscaledTime`인 것이 의도다.** 오디오 뭉침은 배속과 무관한 실시간 현상이라,
`Time.time`을 쓰면 2배속에서만 창이 함께 늘어나 그 배속에서 소리가 성기어진다.

**실제로 깎인 값이 0이면 울리지 않는다.** HP가 바닥나 클램프된 잉여 피해와, 데미지 0으로 저작된 적
(자폭병의 `Melee.Stat.AttackDamage`가 그렇다)이 여기 걸린다 — 소리만 나고 체력바는 그대로인 상태가
"맞았는데 안 깎인다"로 읽힌다.

**구독 컴포넌트로 빼지 않았다.** 본진 피격은 모든 피해 경로(평타·자폭·보스)가 지나는 `TakeDamage`
한 곳에서 나야 하는 보편 소리라, 컴포넌트로 두면 프리팹에서 빠졌을 때 에러 없이 조용히 무음이 된다 —
발사음을 `Tower.RaiseFired`에 둔 것과 같은 축이다(§6.3).

⚠ **배선 위치에 함정이 있다.** 씬의 `PlayerBase`는 `Assets/Personal/SUNGSOO/Prefabs/PlayerBase.prefab`이
아니라 **`Assets/Imported/@NorthLand/Prefabs/Props/Gate_02.prefab`**이다(전자는 개인 WIP 씬 전용).
`Assets/Imported`는 중첩 git 저장소라 **클립 배선이 메인 저장소 커밋에 안 들어온다** — 그쪽을 따로
커밋하지 않으면 다른 팀원 환경에서 필드가 빈 채로 와서 조용히 무음이 된다.
그래서 `PlayerBase.WarnIfHitSfxMissing`이 `Awake`에서 1회 경고한다(WL-040의 「§4 계약 등재 + null 시
1회 경고」 처방). **재생 시점이 아니라 부팅 시점인 것이 의도다** — 본진 피격은 밤이 한참 진행돼야
처음 일어나므로 재생 경로에 걸면 발견이 그만큼 늦다(`CursorSet.WarnIfArtMissing`과 같은 자리).
...
- [x] **본진 전투음** — 피격음 신설 + 자폭 폭발음의 2D → `CombatSfx` 이관(§6.4).
      ⚠ **원래 계획은 "피격음은 2D여야 한다"였고 그것을 뒤집었다.** 근거였던 "화면 밖에서도 들려야 하는
      위험 신호"가, 그림 없이 소리만 나는 쪽이 더 읽기 어렵다는 판단으로 무효가 됐다. 되돌리고 싶어지면
      §6.4를 먼저 읽을 것 — 두 소리를 갈라 놓으면 자폭 한 번에 감쇠되는 소리와 안 되는 소리가 섞인다.
      남은 것은 레벨의 귀 검증이다 — `hitSfxVolume` 0.8은 클립 RMS(-23.5 dBFS)를 아처 기준선
      (실효 -25.3dB)에 맞춰 계산만 한 값이고 아직 들어보지 않았다.
