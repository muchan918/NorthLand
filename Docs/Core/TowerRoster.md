...
| `single_inferno_tower` | 단일 인페르노 | 합성 전용 | 멀티인페르노×1 + 스나이퍼×1 (`Recipe_SingleInfernoTower`) | #300 신규 — 한 대상만 지지되 조준 유지 시간만큼 피해가 커진다(8 → 32 DPS / 6초). **합성 산물을 재료로 쓰는 첫 사례**(다단 합성) |
| `charging_cannon_tower` | 차징 캐논 | 합성 전용 | 캐논×3 (`Recipe_ChargingCannonTower`) | #300 신규 — 사각 레이저 즉시타. 쏘지 않고 참을수록 **폭이 굵어지고 공격력이 오른다**(발사 시 초기화). 재료를 캐논×3으로 잡은 이유는 원안(캐논×1+스나이퍼×1)이 `Recipe_KillStackTower`를 부분집합으로 포함해 두 버튼이 함께 뜨기 때문 |
...
- **성장형 4종(#300) 밸런싱 미확정**: 수치는 전부 구현자 추정이며 팀 사인오프가 없다(WL-015 축). 확인된 지적:
  **단일 인페르노의 상한 피해(32 DPS)가 낮고 사거리(20)가 짧다** — 재료가 멀티인페르노+스나이퍼인
  다단 합성 산물인데 스나이퍼(20 DPS/사거리 36)와 견줘 이득이 작다. 램프업(+8%×10, 감쇠 2초)·
  킬스택(+15%×8)의 폭도 함께 재검토 대상. 기능 검증은 완료됐고 수치만 남았다.
  차징 캐논(사거리 28 / 폭 2→4.5 / 피해 18→78 / 만충 10초)은 **폭 × 피해가 곱으로 작용**하는 유일한
  타워라 상한 산정이 특히 민감하다 — 폭 상한을 보수적으로(×2.25) 잡아둔 상태이며, 충전분이 타일 버프·
  오라 배율에 함께 증폭되는 것을 기준 조합으로 봐야 한다(`Docs/Core/Tower.md` §3.10).
