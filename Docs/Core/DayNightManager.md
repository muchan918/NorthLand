...
| `BgmCue.cs` | (#361) `OnDayToNight`/`OnNightToDay` 양쪽 구독 — 낮/밤 BGM 크로스페이드 요청 + 전환 스팅어 1회 재생. **전환 순간에만** 울리므로 `Start`의 초기 트랙 지정에는 스팅어가 딸려 나오지 않는다. `Start`에서 구독, `OnDestroy`에서 해제. 실제 재생은 `AudioManager`(`DontDestroyOnLoad`)가 하고 이 컴포넌트는 씬의 클립 배선만 소유한다 — 정본 `Docs/Core/AudioManager.md` |
