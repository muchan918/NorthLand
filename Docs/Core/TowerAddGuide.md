...
| 체인(연쇄) — 빔 | `HitscanAttackAction` · `ChainRadius`/`MaxChainTargets`/`ChainDamageFalloff` |
...
⚠ **"체인 + 효과"는 지금 못 만든다** — 체인은 명중 효과(`Effects`)를 걸지 않는다(#252 확정 결정,
근거는 [Tower.md](Tower.md) §3.7.1). 저작하면 저장 시점에 경고가 뜨고 런타임에는 무시된다.
필요해지면 기획 결정이 선행한다.

부품 재고: 액션 4종(투사체 공격 / **히트스캔 공격** / 버프 오라 / 디버프 오라) ·
명중 2종(`Single`/`Area` — 투사체 전용) · 비행 2종(`Homing`/`Ballistic`) ·
효과 4종(`Burn`/`Poison`/`Slow`/`Stun`). 각각의 동작은
[Tower.md](Tower.md) §3.5(액션) · §3.7(비행·명중) · §3.7.1(전달 방식) · §3.8(효과).

⚠ **공격 액션은 둘 중 하나만 담는다** — `AttackAction`과 `HitscanAttackAction`을 함께 담으면 같은
공격 수치로 투사체와 빔이 모두 나가 DPS가 두 배가 된다(저장 시 경고).
...
- [ ] `Impact` — `Single` / `Area`(+`SplashRadius`). **투사체(`AttackAction`) 전용** — 히트스캔에서는 읽히지 않는다
- [ ] 체인 — `ChainRadius` / `MaxChainTargets`(최초 대상 포함) / `ChainDamageFalloff`(홉마다 곱, 감쇠 없음은 1) /
      `BeamPrefab`(빔 외형. 비우면 코드가 기본 빔을 만든다). **`HitscanAttackAction`을 담은 타워만 읽는다**.
      이쪽을 쓸 때 `Attack`의 `ProjectilePrefab`·`Flight`는 **비워둔다**(읽히지 않으므로 경고가 뜬다)
...
| ❌ | `haste_tower` | **미마이그레이션.** 삭제된 `TowerType`·`Single:`/`Area:` 래퍼 키가 아직 남아 있어 오독을 부른다 |
| ⚠ | `lightning_tower` | 스키마는 평탄화 완료(#252 병합)이고 체인 수치도 기입됐지만 **`TowerPrefab` 참조가 깨져 있다**(프리팹이 저장소에 없음, WL-001). 체인 저작 예시로만 참고하고 복제 원본으로는 쓰지 말 것 |
...
| `프리팹에 공격 액션이 있는데 공격 수치가 비었습니다` | [3.5](#35-so-수치-기입) `Attack` |
| `공격 수치를 적었는데 프리팹에 공격 액션이 없습니다` | [3.3](#33-타워-프리팹) `Actions` |
| `AttackAction과 HitscanAttackAction이 함께 있습니다` | [3.3](#33-타워-프리팹) — 전달 방식은 하나만 |
...
| `HitscanAttackAction이 있는데 MaxChainTargets가 …입니다` (`ChainRadius`/`ChainDamageFalloff` 0도 동일) | [3.5](#35-so-수치-기입) 체인 |
| `히트스캔 타워인데 Attack.ProjectilePrefab/Flight가 저작돼 있습니다` (`Impact`도 동일) | [3.5](#35-so-수치-기입) — 비워둘 것 |
| `히트스캔 타워에 명중 효과 …개가 저작돼 있습니다` | 체인은 효과를 걸지 않는다([§1](#1-코드가-필요한가)) |
