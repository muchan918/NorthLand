...
파일이 없으면 기본 설정을 생성해 저장한다. JSON 파싱·검증 실패로 손상 파일로 판정되면 기존
`settings.json`을 `settings.corrupt.<UTC timestamp>.json`으로 격리하고 기본 설정을 새로 저장한다.
손상 백업은 최신 3개만 유지하며, 복구 성공 여부는
`GameSettingsService.WasRecoveredFromCorruption`으로 조회할 수 있다.

지원하지 않는 상위·과거 버전 또는 파일 IO·구버전 재기록 실패는 손상으로 취급하지 않는다.
이 경우 원본 파일을 이동하거나 덮어쓰지 않고 현재 실행에서만 기본값을 사용하며 설정 저장을 차단한다.
언어와 슬롯 선택 변경은 메모리에는 반영되지만 다음 실행에는 유지되지 않는다.
...
- [ ] 손상된 `settings.json`이 `settings.corrupt.<UTC timestamp>.json`으로 격리되고 기본 설정이 새로 생성된다.
- [ ] 손상 설정 백업은 최신 3개만 유지된다.
- [ ] 설정 복구 성공 후 언어·카메라·그래픽·사운드 설정을 다시 저장할 수 있다.
- [ ] 지원하지 않는 상위·과거 버전과 IO 실패 파일은 이동하거나 덮어쓰지 않는다.
