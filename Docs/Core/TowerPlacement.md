...
- **타일 버프 수치 미리보기** (현재 풋프린트의 양수 보정값을 고스트 상단에 현재 로케일로 표시)
...
#### 9.1.1 타일 버프 수치 미리보기 (구현)

배치 중 현재 풋프린트가 받는 타일 버프를 계산해 고스트 상단에 텍스트로 표시한다.

- 표시 대상은 공격력·공격속도·사거리·오라 반경의 **양수 보정값**이다.
- Flat 보정은 `+값`, Percentage 보정은 `+값%` 형식으로 표시한다.
- 라벨은 한국어 문자열을 직접 소유하지 않고 `TowerStatsFormatter.StatLabel`과 기존 `game.tower.*` 키를 사용한다. 따라서 배치 전·후 UI가 같은 명칭을 사용하고 현재 로케일을 따른다.
- 증가 수치는 `UiPalette.PositiveHex` 색상으로 표시한다.
- 위치는 파티클 렌더러를 제외한 고스트의 `Renderer.bounds` 상단을 기준으로 한다. Renderer가 없으면 고스트 위치를 사용한다.
- Renderer 목록은 고스트별로 한 번 캐시하고, bounds는 풋프린트가 바뀔 때만 다시 계산한다. 카메라 이동·줌에 대응하기 위한 화면 좌표 갱신만 매 프레임 수행한다.

잔상 방지를 위해 다음 상황에서는 패널을 숨긴다.

1. 첫 스냅 전
2. 앵커가 없거나 표시할 양수 버프가 없을 때
3. 커서가 배치 표면을 벗어날 때(`OnSurfaceHoverChanged(false)`)
4. 배치가 확정·취소·교체되어 세션이 종료될 때

카메라 뒤에 있거나 Canvas 좌표 변환에 실패한 경우에도 방어적으로 숨긴다.

필수 UI 배선이 누락되면 초기화를 실패로 고정하고 오류를 세션당 한 번만 출력한다. 외부 `Show` 호출이 계속 들어와도 초기화를 반복하거나 null 필드에 접근하지 않는다.
...
- [x] 배치 중 현재 풋프린트의 타일 버프 수치가 고스트 상단에 현재 로케일로 표시되고, 앵커·표면 이탈 및 세션 종료 시 잔상 없이 숨겨진다.
