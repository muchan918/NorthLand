...
> **상태**: 배치 코어 · **SO 게이트웨이 · 자원 차감 · 타일 버프 · 등장 연출 구현 완료** · 낮 전용 게이팅만 미구현(§8)
...
>
> 다만 **§9.3.2 계약은 임시가 아니다** — 그게 이 연출이 에셋 교체를 견디는 유일한 장치다.
...
| 항목 | 현재 값 | 근거 |
| --- | --- | --- |
| 수렴 / 등장 / 링 지속 | 0.45s / 0.28s / 0.38s | 감으로 정함. 미검증 |
| 알갱이 크기 | 풋프린트 × 0.15 (=2.25) | 게임 줌 70·1080p에서 17.4px |
| 알갱이 화면 하한 / 상한 | `orthoSize × 0.017` / 풋프린트 × 0.4 | 줌 300에서도 보이게 / 칸을 뒤덮지 않게 |
...
| --- | --- |
| 컴파일 | ✅ |
| 파생 수치 (9개 대상, 40배 스케일 범위 + 피벗 오프셋 + Renderer 없음 폴백) | ✅ 편집 모드 |
| 정지 프레임 룩 (실제 게임 줌 70 · 1920×1080) | ✅ 편집 모드 |
| **움직임** (수렴 이징 · 소용돌이 · 오버슈트 · 링) | ❌ **미검증** — `UniTask`+`Time.deltaTime`이라 플레이 모드 필요 |
| 줌 70~300 전 구간 실사용 | ❌ 미검증 |
...
**연출(§9.3) — 전부 임시라 아래는 "할 일"이라기보다 "열려 있는 결정"이다**
- **아트 방향 확정 시 룩 전면 재검토.** 색·형태·지속 시간 어느 것도 고정이 아니다. 유지해야 하는 것은 §9.3.2 계약뿐.
- **플레이 모드 검증** — 움직임 미확인(§9.3.4).
- **줌 70~300 실사용 확인** — 화면 하한/상한이 실제로 맞는지.
- **타워 에셋 교체 시 회귀 확인** — 크기가 크게 다른 큐브 몇 개로 재생해 보면 된다(계약상 코드 수정은 필요 없어야 한다). **이 확인이 실패하면 계약이 깨진 것이다.**
