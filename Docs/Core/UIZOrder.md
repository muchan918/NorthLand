...
|---:|---|---:|---|
| 월드 (3D) | `EnemyStateUILayer` | `40` | 몬스터 머리 위 체력바. 코드가 만드는 **World Space** Canvas 하나를 살아 있는 모든 몬스터가 공유한다. 입력을 받지 않는다 — ⚠ 아래 「World Space는 이 표의 총순서에 속하지 않는다」 참고 |
...
`EnemyStateUILayer`는 **World Space**라 아예 다른 축에 있다. 3D 지오메트리로 그려지므로 지형·몬스터에 가려지고(ZTest), **`sortingOrder` 값과 무관하게 모든 Overlay 캔버스 아래에 놓인다.** 즉 체력바 `40` < 선택 사각형 `50`이라는 관계는 **결론만 맞고 근거가 아니다** — 40을 600으로 바꿔도 HUD 위로 올라오지 않는다.
...
`EnemyStateUILayer`(#447)도 런타임에 전용 Canvas를 만드는 읽기 전용 오버레이이며, `GraphicRaycaster` 없음·`raycastTarget` 전부 off는 같다. 다른 점은 두 가지다.

- **World Space다.** 바가 월드 좌표를 따라다니므로 Overlay가 아니라 World Space Canvas이고, 이 표에서 유일하게 3D 공간에 사는 항목이다.
- **살아 있는 모든 몬스터가 이 Canvas 하나를 공유한다.** `SelectionBoxView`는 리빌드 범위를 가두려고 Canvas를 **나눴지만**, 체력바는 반대로 **합쳐야** 한다 — 몬스터마다 Canvas를 두면 Canvas끼리는 배칭되지 않아 드로우콜이 마릿수만큼 붙고(#447 이전 상태), 바가 몬스터 트랜스폼에 매달려 프리팹 루트 스케일에 끌려간다(제거 전 월드 폭 실측 3.00~10.00, 편차 3.3배). 합치면 배경·필·눈금이 전부 흰 텍스처라 **전 몬스터 합계 1 배치**가 된다.

명세는 `Docs/Core/EnemyStateUI.md`.
