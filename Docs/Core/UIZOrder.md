...
`MonsterHealthBarLayer`(#447)도 런타임에 전용 Canvas를 만드는 읽기 전용 오버레이이며, `GraphicRaycaster` 없음·`raycastTarget` 전부 off는 같다. 다른 점은 두 가지다.
...
명세는 `Docs/Core/MonsterHealthBar.md`.
