...
| KeyboardManager (단축키 입력)               | n0wst4ndup | `Assets/Scripts/GameManager/KeyboardManager`                         | 단축키 입력의 단일 창구(#444). `MouseManager`의 형제지만 **아는 것이 없다** — 기능 쪽이 `KeyboardManager.Bind(Key, KeyModifier, handler, label, exactModifiers)`로 자기 단축키를 등록하고 매니저는 눌림만 감지해 부른다(단축키가 늘어도 이 파일은 안 바뀐다). 바인딩 목록은 static이고 펌프(MonoBehaviour)는 `AfterSceneLoad` 자가 부팅 + `DontDestroyOnLoad`라 **씬 배선이 없다**(`ResidentDragCoordinator` 계보). 텍스트 입력 중(`TMP_InputField`/`InputField` 포커스)에는 삼킨다. **보조키 판정은 바인딩별로 고른다(#537, WL-201 해소)**: 기본 `exactModifiers: true`는 정확히 일치라 Ctrl+Shift+Z가 Ctrl+Z를 겸하지 않고, `false`는 "요구 보조키 포함, 나머지 무시"다. ⚠ **무보조키 단축키를 둘 중 어느 쪽으로 등록할지는 아직 팀 합의가 없다(WL-219)** — 이 게임에서 Shift는 그룹 선택으로 쥔 채 조작하는 키라 정확 일치는 "가끔 안 먹는" 단축키가 되지만, 반대로 `false`는 Shift+숫자·Ctrl+Q 같은 조합을 나중에 다른 기능에 줄 수 없게 만든다. ⚠ **인스턴스 메서드를 등록하면 `Unbind`는 호출부 책임이다**(목록이 static이라 파괴된 오브젝트를 붙든 채 남는다) — 소유자 기반 자동 해제는 미구현(WL-199 PARTIAL). 현재 등록: **Ctrl+Z → `UndoRequest.Submit`**(static, 앱 수명, 정확 일치), **Space → `GameSpeedController.ToggleSpeed`**(인스턴스, `OnEnable`/`OnDisable` 대칭 + `exactModifiers: false`), **Q → `SkillButtonView.HandleShortcut`**(#501 감전 스킬 조준, 인스턴스, `Awake`/`OnDestroy` 대칭), **Digit1~9 → `BuildingShortcutBar.FocusEntry`**(항목 개수만큼, 인스턴스, `OnDestroy` 대칭). ⚠ **뒤 둘은 기본값(정확 일치)으로 등록돼 있어 Shift·Ctrl·Alt 중 하나라도 쥐면 발화하지 않는다** — 그룹 선택으로 Shift를 쥔 채로는 Q도 숫자키도 안 먹는다는 뜻이고, **그것이 의도인지 함정인지가 WL-219의 미결 사항이다.** 정책이 정해지기 전까지 두 호출부를 임의로 바꾸지 말 것 |
| GameSpeed (배속·일시정지, #190/#537)        | n0wst4ndup | `Assets/Scripts/UI/GameSpeedController.cs`, `Assets/Scripts/UI/UIEffects/SpeedBoostEffect.cs` | 전역 `Time.timeScale`을 쓰는 **유일한 자리**(`ApplyTimeScale`) — 게임 진행을 멈춰야 하는 모달은 직접 건드리지 말고 이쪽에 요청한다(`Docs/Core/UIZOrder.md`). **배속은 1↔2 토글 하나다(#537)**. 그 전에는 1/2/4배 버튼 3개였고 각 버튼이 씬 UnityEvent로 서로 다른 메서드를 불러 "지금 켜진 배속"이 버튼 색과 `CurrentSpeed` 두 곳에 있었다 — 지금은 버튼도 스페이스바도 **`ToggleSpeed()` 하나**를 부르고 표시는 그 결과를 따라가므로 표시와 실제가 갈릴 자리가 없다(`UndoRequest.Submit`이 버튼과 Ctrl+Z를 한 진입점으로 묶은 것과 같은 이유). 🗑 **4배속 폐기** — 보스 BT 예고·준비 모션이 전부 스케일드 타임이라 4배에서 예고 `Duration` 0.5초가 0.125초로 뭉개져 인지 자체가 불가능했다(WL-119). **일시정지는 이유(`GamePauseReason`) 단위 참조 계수**다: `Reward`/`Settings`/`ResultDecided`/`Cutscene`/`Tutorial` 중 하나라도 남아 있으면 `timeScale`은 0이고, 리워드 패널과 설정 창이 겹쳤을 때 먼저 닫힌 쪽이 나머지의 정지를 풀지 못한다. 정지 중에도 `CurrentSpeed`는 유지된다(정지는 배속 위에 겹치는 상태이지 1배로 되돌리는 것이 아니다). ⚠ **정지 중 `ToggleSpeed`는 무시한다** — 버튼은 `CanvasGroup`으로 이미 잠기므로 단축키만 열어 두면 모달을 닫는 순간 아무도 누른 기억이 없는 배속이 된다. 활성 표시는 버튼(`UICanvas/GameSpeed/SpeedBtn`) 주위를 도는 별 파티클(`BoostVfx` = `UIParticle` + `VFX_X2Active`)이고 **`SpeedBoostEffect`가 `Play`/`Stop` 두 신호만 받는다**(배속 상태를 스스로 판단하지 않는다). ⚠ 파티클은 **unscaled 강제**(§6 기준) + **끌 때 남은 별까지 즉시 `Clear`** — `orbs` 수명이 5초라 방출만 멈추면 배속을 끈 뒤에도 별이 돌아 "아직 2배인가"로 읽힌다. ⚠ `UIParticle`은 `autoScalingMode = Transform`이라 **자기가 붙은 오브젝트의 `localScale`을 덮어쓴다** — 버튼에 직접 붙이면 버튼이 커지므로 전용 자식(`BoostVfx`)에 둔다. 파티클 프리팹은 `Assets/Imported/@NorthLand/Prefabs/UI/Particle/VFX_X2Active.prefab`으로 **별도 저장소**라 미동기화 시 연출만 조용히 사라진다(WL-040과 같은 축) |
...
| Localization                                | n0wst4ndup | `Assets/Scripts/Localization/LocalizationHelper.cs`, `Assets/Localization/*`(String Table 컬렉션), `Assets/Scripts/Test/LocalizationTest.cs` | String Table 8종(`NorthLand_default`/`NorthLand_buildings`/`NorthLand_Enemies`/`NorthLand_Towers`/`NorthLand_Territories`/`NorthLand_Rewards`/`NorthLand_Skills`/`NorthLand_Tutorial`, ko-KR/en-US/ja-JP) 구축. 본진 효과 문구는 **레벨별 키 규약** `castle.effect.lv{n}`(n=도달할 표시 레벨)를 쓴다 — 무엇이 해금되는지가 레벨마다 달라 코드는 키만 조립하고 문구는 테이블에서 authoring한다. Building/Enemy/Resource/Tower CSV 표시 문자열은 키로 이관 완료(WL-013 해소, PR#126 — 신규 `poison_tower` 행 포함). `LocalizationHelper`(static 동기 pull 헬퍼) 신설 — 호버 툴팁 등 '호출 시점 1회' 풀 경로 전용, 지속형 표시는 `LocalizeStringEvent`/`LocalizedString.StringChanged` 사용. 튜토리얼 단계 SO는 키를 저장하고 현재 표시 중인 팝업·말풍선은 로케일 변경 시 재조회한다. 전투 공간(TowerInfoUI) 표시 배선은 후속(#102) |
...
| WaterShader (경영 수면 룩·접촉 거품, #145) | muchan | `Assets/Shaders/Water` | `WaterURP_Ortho.shader` + `Water_Ortho.mat`이 정본. 수면은 `_ColorDeep` 단색이고, 거품은 깊이 버퍼로 복원한 씬 월드 위치와 수면의 상대 높이로 접촉을 판정한다. 화면 깊이 차이를 접촉 판정에 사용하면 오소그래픽 카메라에서 떨어진 오브젝트의 실루엣을 물 접촉으로 오인하므로 **재도입 금지**(#145). 밤 틴트는 `DayNightLightingController`가 노출된 수면 색 프로퍼티에 MPB로 적용한다. 상세 `VisualLookPipeline.md` §3.3.1 |
...
| Command (되돌리기 커맨드·히스토리, #263/#281/#444) | muchan · n0wst4ndup(#444) | `Assets/Scripts/Command`(계약·공통 기반·히스토리·요청 진입점), `Assets/Scripts/GameManager/MouseManager/TowerPlacement`(타워 구현체 2종), `Assets/Scripts/UI/TowerPanel/TowerUndoButtonView.cs` | 낮 동안 **자원을 소모한 조작**을 되돌린다 — 타워 배치·합성(#281)에 **건물 업그레이드**(생산 라인 + 업그레이드 전용/본진)와 **주민 증축**(#444)이 **같은 스택으로** 합류했다(건물→타워→건물을 눌린 역순으로 되감는다). `IReversibleCommand` 4단 트랜잭션 + 공통 기반 `ReversibleCommandBase`(상태 기계·비용 환원) + static `CommandHistory`(LIFO 20, 씬 배선 없음). 요청 해석은 `UndoRequest.Submit()` 하나로 모여 **되돌리기 버튼과 Ctrl+Z가 같은 경로**를 쓴다. 주민 증축 되돌리기는 **배치까지 본다** — 상한을 넘게 되면 가장 많이 배치된 라인에서 1명을 `UnassignVillager`한다(없는 주민이 계속 생산하는 상태 방지). **자원 교환(#211)·주민 배치는 범위 밖** — 교환은 받은 자원을 이미 썼을 수 있어 "되돌리기는 항상 성공한다"가 깨지고, 배치는 자원을 쓰지 않고 반대 조작(− 버튼)이 이미 있으며 다중 드롭이 깊이 20을 잡아먹는다. 되돌릴 수 있는 것은 "방금 한 조작"뿐이라 **임의 철거 경로는 여전히 없다**(`Tower.md` §6 #1 미해소). Redo 없음. 명세 `Docs/Core/TowerPlacement.md` §7·§8, `Docs/Core/TowerMerge.md` §9.3, `Docs/ManagementArea/BuildingUpgrade.md` §10 |
...
| CursorFeedback (상태별 커서 그림) | n0wst4ndup | `Assets/Scripts/GameManager/MouseManager/Cursor`(`CursorController`/`CursorSet`/`ICursorHint`), `Assets/Resources/ScriptableObjects/CursorSet.asset` | 하드웨어 커서(`Cursor.SetCursor` + `CursorMode.Auto`)를 상호작용 상태에 따라 교체한다. `OutlineInteractionDriver`와 같은 계보 — MouseManager는 통지만 하고 그림 결정은 구독자가 한다. **MouseManager 신규 통지 3종**: `OnPointerPressedChanged`(좌버튼 `isPressed` **변화**만, UI·모드 무관한 순수 입력 통지 — `wasReleasedThisFrame`을 쓰면 포커스 상실 시 뗌이 유실돼 눌린 그림에 갇힌다, WL-143과 같은 논리) · `OnModeChanged`+`CurrentMode`(`SetMode` 단독 발행, 동일 모드 재진입은 통지 안 함 — `CancelInteractions`가 `SetMode(Idle)`을 두 번 때린다) · `OnTargetSurfaceChanged`+`IsOverTargetSurface`(#517 후속 — 배치·조준 중 고스트가 지금 그려지는가. 「숨김」과 「안 됨」을 가르는 값이며 갱신 창구·발행 순서·마스크 소유는 §2에 등재). 대상별 커서는 **대상이 스스로 답한다**(`ICursorHint`, `IOutlineKindFilter`와 같은 확장 규칙): `BuildingTooltipSource`=Door · `TowerGroupSelectable`=Inspect · `ResidentSelectable`=Hand. ⚠ 세 구현 모두 `IHoverable`과 **같은 컴포넌트**여야 한다(`TryGetComponent`가 GameObject당 하나만 잡음). **상태는 두 축이다** — 종류(`CursorKind`, 우선순위 `CameraPan > UIButton > Placing/SkillAiming/Blocked > ResidentDrag > 호버(Building·Tower·Resident) > Default`) × 눌림(불리언). 어휘가 그림(Door/Inspect/Hand)이 아니라 **대상 기준**인 것은 아트 파일명(`Building-Hover-32` 등)과 1:1로 맞추기 위해서다 — 도메인 이름이 들어와도 원칙 2는 유지된다(값을 고르는 것은 대상 자신이고, 컨트롤러가 분기하는 것은 **모드 3종과 「대상 표면 위인가」**뿐이다 — `MouseManager.IsOverTargetSurface`, §2 등재). 눌림이 별도 종류가 **아닌** 이유: "건물 위에서 누르면 열린 문"처럼 눌렸을 때의 그림이 무엇을 가리키느냐에 따라 다르므로, 전역 눌림 1장으로는 표현되지 않는다. 그래서 뱅크의 칸마다 `Normal`/`Pressed` 두 장을 갖고, `Pressed`가 비면 `Normal`로 폴백한다(=그 종류는 눌림 연출 없음). 텍스처는 자가 부팅 컨트롤러라 인스펙터 대신 Resources SO가 든다(`SfxBank` 선례). ⚠ 하드웨어 커서 제약: **애니메이션·파티클 부착·스케일 불가**, 텍스처는 `Texture Type = Cursor` 필수(아니면 `"not CPU accessible"`로 실패 → 컨트롤러가 사전 검사해 경고). **칸별 `Hidden`을 켜면 그 상태에서 커서를 감춘다**(`Cursor.visible=false`) — ⚠ **켜진 칸은 `Placing`·`SkillAiming` 둘이며 실가동이다**(#517 후속). 한 번 껐다가 다시 켠 자리다 — 2026-08-27에 껐던 이유는 "고스트·인디케이터는 **커서가 타일 위일 때만** 그려져 타일 밖에서는 대신할 것도 없이 커서까지 사라진다"였고, 그 빈자리를 **새 칸 `Blocked`(「안 됨」)**이 메우면서 숨김을 전제가 성립하는 구간에만 걸 수 있게 됐다. 가르는 값은 `MouseManager.IsOverTargetSurface` 하나다(§2 등재). **`Hidden`을 다른 칸에 켜려면 같은 질문을 먼저 통과할 것**: 커서가 사라진 자리를 무엇이 대신하며 그것이 **항상** 거기 있는가. ⚠ **커서가 UI 위면 숨김을 되살린다** — 배치 중에도 취소 버튼을 겨냥해야 하기 때문이다. (예전 근거였던 "고스트는 `overUI`면 갱신을 멈춘다"는 **사실이 아니다** — `UpdatePlacement`는 `overUI`를 보지 않고 레이를 쏘므로 고스트는 패널 뒤 타일을 맞혀 계속 따라다닌다. 되살리는 진짜 근거는 **겨냥해야 하는 대상이 UI 쪽**이라 고스트가 그 자리에서 커서를 대신하지 못한다는 것이다.) 이 판정(`EventSystem.IsPointerOverGameObject()`)만 `LateUpdate`에서 1회/프레임 폴링하며, **`_overUI`가 쓰이는 곳은 「막는 자리 둘」**이다 — 숨김의 예외와 「안 됨」의 예외(「안 됨」은 UI 위에서 나오지 않는다: "여기엔 못 놓는다"는 **지도**에 대한 말이라 패널 위에서는 거짓말이 된다). 그래서 `_overUI`만 바뀌어도 **종류가 바뀔 수 있어** 예전의 "숨김만 갱신" 지름길은 제거됐다. 숨기는 종류여도 **그림은 계속 세팅한다** — 안 그러면 배치 직전 그림(문·돋보기)이 남아 UI 위에서 뜬다(숨김 칸은 비어 있어 `Default`로 폴백된다). ⚠ **"UI 위"가 아니라 "누를 수 있는 UI 위"에서만 그림이 바뀐다**(`UIButton`) — "UI에 올라가면 무조건 바뀜"은 좌우 한 쌍의 손 그림까지 넣었다가 "오히려 헷갈린다"로 걷어냈다(2026-08-27, `UI-Left-32`/`UI-Right-32` 삭제됨). **그 형태로 되살리지 말 것**. 판정은 `EventSystem.RaycastAll` 맨 앞 히트의 `Selectable`+`interactable`이며, 이것만 새 레이캐스트라 `IsPointerOverGameObject()`가 참일 때만 돈다(버퍼 재사용, 무할당). `CameraPan`은 **우버튼을 직접 폴링하지 않고** `CameraController2.IsDragging`(이번에 공개)을 읽는다 — 직접 읽으면 배치·조준 취소용 우클릭까지 팬으로 오인하고 "UI 위에서 시작한 우클릭은 팬이 아니다"라는 카메라 규칙을 재구현하게 된다. 같은 맥락으로 `CameraController2.Update`의 `Result != Playing` 조기 반환에 `_isDragging=false`를 추가했다(패널 분기와 달리 빠져 있어, 우버튼을 쥔 채 판이 끝나면 커서가 '쥔 손'으로 굳었다). `Cursor.visible`은 전역 값이라 **표시/숨김은 이 컨트롤러가 단독 소유**한다는 전제다. `CameraController2`는 씬 소유라 못 찾으면 **120프레임 백오프** 후 재탐색한다(`ResidentDragCoordinator`와 같은 값·같은 이유 — 이 컨트롤러는 `DontDestroyOnLoad`라 카메라가 없는 씬에도 상주한다, WL-002). 현재 배선: `Default`(Default-32/Default-Click-32, 손끝 14,5)·`UIButton`(Default-Hover-32, 13,2)·`CameraPan`(Default-Grab-32, 13,2)·`Resident`(Default-Hover-32, 13,2)·`Building`(Building-Hover-32/Building-Click-32, 핫스팟 16,16 — 두 그림 알파 bbox가 같아 눌러도 안 튐)·`Tower`(Tower-Hover-32, 12,12 = 렌즈 중심)·`ResidentDrag`(Resident-Drag-32, 16,8)·`Placing`·`SkillAiming`(**둘 다 비어 있음 — 숨김이라 그림이 필요 없다**. UI 위에서 되살아날 때만 폴백을 타고 `Default`가 뜬다)·`Blocked`(Default-No-32 = 빨간 금지 표지, 16,16). ⚠ 미해결 2종은 `CursorFeedback.md` §6에 있다: `CameraPan` 핫스팟 (13,2)가 주먹 실루엣(y 5–27) 밖 — 손 그림 값이 딸려 온 것으로 보임 / `Resident`와 `UIButton`이 같은 그림이라 주민 위와 버튼 위가 구분되지 않음. (종전의 `Placing` 핫스팟 (0,0) 건은 그 칸이 숨김으로 바뀌며 소멸했다.) 아트는 `Assets/Imported/@NorthLand/UI/Cursor`(32×32, 파일명 `대상-상태-크기`, **별도 저장소 — §4 「커서 아트 동기화 계약」**, 미동기 시 `WarnIfArtMissing`이 1회 경고) · 클릭 파티클 보류. 정본 `Docs/Core/CursorFeedback.md` |
...
| Camera (쿼터뷰 카메라 · 줌) | sunjin1222 · n0wst4ndup | `Assets/Scripts/Camera/CameraController2.cs`, `Assets/Scripts/Camera/CameraVisibility.cs`, `Assets/Scripts/Camera/ZoomDrivenVisibility.cs` | 정본 씬은 `CameraController2` 단독(구 `CameraController.cs`는 미사용 잔존, WL-023). 이동(WASD·우드래그·미니맵)과 줌(휠 → `CinemachineCamera.Lens.OrthographicSize`, 씬 범위 30~150)을 소유하며 `Mouse`/`Keyboard.current`를 직접 폴링한다 — MouseManager의 '입력 단일 창구' 계약 **밖**이다(WL-023·WL-073). **#138에서 `OnZoomChanged` 발행이 추가돼 줌 소비 축이 둘이 됐다**: `PixelationZoomBinder`(매 `LateUpdate` 폴링, `[ExecuteAlways]`라 편집 모드에서도 돌아야 해 이벤트로 이관 불가 — **의도된 병존**)와 `ZoomDrivenVisibility`(push + 붙을 때 1회 pull). 발행이 `ZoomMouseWheel` 인라인이라 `ApplyZoom` 격리 seam은 아직 없다(WL-024). **#390에서 `ZoomTo`/`UpdateTargetZoom`(SmoothDamp, `LateUpdate`)이 붙어 발행처가 둘로 늘었고, seam이 없는 탓에 렌즈 대입+발행이 그대로 복제됐다** — WL-024가 예고한 비용이 실제로 발생한 지점이다. **자동 모션은 수동 입력에 양보한다** — 이동은 WASD·우드래그가 `CancelMinimapMove`로, 줌은 휠이 `isZooming = false`로 각각 끊는다. **예외로 튜토리얼 강제 포커스(`_tutorialFocusActive`) 중에는 연출이 완료될 때까지 수동 카메라 입력보다 자동 모션을 우선한다.** ⚠ **양보를 빠뜨리면 증상이 "느려짐"이 아니라 "잠김"이다**(#390 실측) — `Update`에서 바꾼 배율을 같은 프레임 `LateUpdate`가 되돌리고, 굴리는 동안은 SmoothDamp가 목표에 닿지 못해 종료 조건도 안 걸린다. `CameraVisibility`(static)는 프러스텀 가시성 질의의 공용 창구로, 평면을 **프레임당 1회만** 계산해 캐시한다(질의 주체가 스포너 1 + 비행 중 N대). **우드래그 팬 상태를 `IsDragging`(읽기 전용)으로 공개**한다 — 유일한 소비처는 커서 그림(`CursorFeedback.md`)이고, 우버튼을 직접 폴링하면 배치·조준 취소용 우클릭까지 팬으로 오인하기 때문이다. 이때 `Update`의 `Result != Playing` 조기 반환에 `_isDragging=false`를 함께 넣었다(바로 위 패널 분기에는 있는데 이쪽에만 빠져 있었다 — 우버튼을 쥔 채 판이 끝나면 플래그가 켜진 채 남는다) |
| Audio (볼륨 3채널 · BGM 재생/전환, #361) | n0wst4ndup | `Assets/Scripts/GameManager/AudioManager.cs`, `Assets/Scripts/CombatSystem/CombatSfx.cs`, `.../SoundCue.cs`+`TitleCue.cs`+`InGameCue.cs`(씬 큐), `.../SfxBank.cs`+`Sfx.cs`(공용 효과음 뱅크), `.../UiClickSfx.cs`+`UiClickSfxIgnore.cs`(버튼 클릭 전역 훅)` | **AudioMixer를 쓰지 않는다** — 채널이 Master/BGM/SFX 3개뿐이고 더킹·스냅샷 요구가 없어 믹서 에셋 + 그룹 배선 + dB 변환 비용이 이득보다 크다고 판단했다. 대신 `AudioManager`가 볼륨 값을 소유하고 **자기가 소유한** `AudioSource.volume`에 곱해 넣는다(`실효 = Master × 채널`, 음소거면 0). SFX는 화면 전역 2D 원샷(`PlaySfx`), 겹치지 않는 전용 소스(`PlaySfxExclusive`), 전투 위치 보이스 풀(`NorthLand.Combat.CombatSfx`) 세 경로다. 전투 풀은 16개 사전 생성·최대 32개이며 화면 좌표와 오쏘 80~160 줌 감쇠, 스테레오 팬, 재생 중 SFX 설정 반영을 담당한다. 소비처는 낮↔밤 전환 스팅어 2 + 공용 효과음 10(튜토리얼 Bubble·Popup 안내음 포함). **겹치면 안 되는 소리를 위한 두 번째 경로 `PlaySfxExclusive`가 생겼다** — 전용 소스 1개로 직전 것을 끊고 처음부터 재생하며 볼륨을 매 프레임 다시 곱한다(재생 중 슬라이더 반영). 소비처는 주민 증가음(9.46초 징글)과 결과창 승리/패배 스팅어(#538) **2개**이고, 소스가 하나라 이 경로끼리도 서로를 끊는다 — 둘은 동시에 울릴 수 없는 화면이라 지금은 무해하다. 클립·볼륨은 씬이 아니라 `SfxBank` SO 하나가 소유하고 정적 `Sfx`가 진입점이다(§2 Audio) — 버튼 클릭음처럼 **모든 씬에서 같은 소리**는 씬 큐에 두면 새 씬에서 조용히 누락되기 때문이다(WL-180과 같은 축). ⚠ 뱅크의 범위는 **주인 없는 소리**뿐이고 타워 발사음류는 넣지 않는다. 부팅은 `GameSceneManager`와 같은 `RuntimeInitializeOnLoadMethod(BeforeSceneLoad)` + `DontDestroyOnLoad`(씬 배치 없음 — 씬 파일 병합 충돌을 만들지 않기 위함). BGM은 `AudioSource` 2개 크로스페이드이고, **페이드 가중치와 실효 볼륨을 분리해 매 프레임 다시 곱한다**(합치면 페이드 중 슬라이더 조작이 목표 볼륨을 덮어쓴다). 나가는 소스는 스왑 시점 가중치(`outgoingWeight`)를 기억한다 — 안 그러면 페이드 도중 트랙을 다시 요청했을 때 `fadeProgress` 리셋으로 최대 볼륨까지 튀었다 내려온다. 페이드는 **`unscaledDeltaTime`** — 설정 패널을 여는 것 자체가 `GamePauseReason.Settings` 정지(`timeScale` 0)라 scaled면 얼어붙는다. 배속(2·4배)에서 `pitch`는 건드리지 않는다. **"어떤 곡을 틀지"는 매니저가 모른다** — `DontDestroyOnLoad`라 인스펙터 배선을 가질 수 없어, 클립 배선과 낮/밤 구독은 씬의 `SoundCue` 계층(`SoundCue` abstract 베이스 → `TitleCue` / `InGameCue`)이 맡는다. 매니저가 씬마다 죽는 `DayNightManager`를 재구독할 일이 없는 대신 **씬마다 큐가 하나씩 있어야 한다**는 계약이 붙는다 — 큐가 없는 씬은 직전 씬 BGM을 그대로 끌고 간다(WL-180 실사고: 밤 게임오버 → 타이틀에서 밤 BGM 루프). 그래서 `TitleCue`는 트랙 에셋이 없는 지금도 배치돼 있고 `titleClip`이 비면 `StopBgm()`을 부른다 — 빈 클립을 `PlayBgm`에 넘기면 매니저가 무시해 직전 트랙이 살아남기 때문이다. 영속화는 `LocalizationManager`(`SelectedLocale`) 선례를 따라 **`PlayerPrefs`** 6키(`MasterVolume`/`BgmVolume`/`SfxVolume` + `*Muted`), 기본값 Master 1.0 · BGM 0.5 · SFX 0.8, `Save()` flush는 종료·포커스 상실 시점에만. **에셋**: `Assets/Imported/@NorthLand/Sound`(BGM 2 + 전환 스팅어 2) — ⚠ **별도 저장소라 클립·`.meta`(임포트 설정) 변경이 본 저장소 diff에 보이지 않고, 미동기화 시 에러 없이 소리만 사라진다**(WL-040과 같은 축). 임포트 설정은 기본값에서 조정했다: **BGM은 `Streaming`+`loadInBackground`**(`DecompressOnLoad`면 두 트랙이 30.2MB+32.1MB로 풀리는데 크로스페이드가 둘을 동시에 문다), 전환 스팅어는 `DecompressOnLoad`+`preload`. 플랫폼 오버라이드 없이 `defaultSettings`가 PC·Mobile 양쪽에 적용된다. Vorbis quality는 100% 유지(청감 tradeoff라 미결). ⚠ **임포트 설정에 클립별 게인이 없다** — 특정 클립이 크면 파일을 다시 내보내거나 재생 배율을 곱하는 수밖에 없다(`.meta`의 `normalize`는 모노 다운믹스용이라 게인이 아니다). 전환 스팅어는 피크가 -0.8/-1.0 dBFS로 꽉 차 있어 `BgmCue.stingerVolume`(코드 기본 0.35 ≈ -9dB, 정본 씬 0.4)로 눌러 재생한다. **매니저 밖에서 직접 `AudioSource`를 소유하는 경로는 `ResidentVoice`와 `CombatSfxPool` 둘뿐이며 둘 다 매 프레임 `GetEffectiveVolume(Sfx)`를 곱한다**(WL-179 PARTIAL). 주민은 상태를 따라가는 장기 소스, 전투는 짧은 위치 효과음 풀이므로 서로 흡수하지 않는다. **✅ 공용 효과음 10본**: 버튼 클릭 · 패널 오픈 · 튜토리얼 Bubble · 튜토리얼 Popup · 타워 설치 · 거절 · 건물 업그레이드 · 주민 증가 · 되돌리기 · 다시 실행(클립만, 기능 미구현). 되돌리기음은 `UndoRequest.Submit` 한 곳에서 난다 — 버튼과 Ctrl+Z가 같은 진입점이라 단축키에만 소리가 빠지는 사고가 구조적으로 생기지 않는다(`CommandHistory.Undo`가 `bool`을 반환하도록 바꿔 `CanUndo` 조건을 두 벌로 만들지 않았다). ⚠ **같은 큐는 한 프레임에 한 번만 난다**(`Sfx.ClaimFrame`) — 타워 1회 클릭에 `Tower.OnSelected`가 두 번 불려(MouseManager + `TowerMergeCoordinator.RefreshPanel`) 패널음이 두 겹으로 크게 들렸다. 그쪽은 "표시가 idempotent라 무해"가 의도된 설계라 호출부를 줄이지 않고 소리 쪽에서 걸렀다(WL-087 회귀 방지). ⚠ **자기 결과음을 내는 버튼은 `UiClickSfxIgnore.ApplyTo`로 공용 클릭음에서 뺀다** — 클릭음은 누를 때, 결과음은 뗄 때 나서 겹쳐 들린다(주민 증가·업그레이드·합성 후보·되돌리기). 인스펙터가 아니라 소리를 배선한 코드 옆에서 붙여 씬 병합 충돌을 만들지 않는다. ⚠ **6본 전부 `Assets/Imported/CartoonSFX` 팩 파일의 복사본이다**(해시 대조 확인) — 팩 원본이 남아 있어 편집이 되돌려지는 대신, **팩 원본은 절대 편집하지 않는다**(다른 데모 씬이 참조). ⚠ `SFX_Castle_ResidentIncrease`만 `.meta`까지 복사돼 **GUID가 충돌**했고, 복사본이 팩 원본의 GUID를 가져가 Unity가 팩 원본에 새 GUID를 발급했다(미해결, AudioManager.md §4.5.1). 클립을 가져올 땐 `.wav`만 복사할 것. **"소리가 게임과 안 붙는다"의 원인은 앞쪽 무음이 아니라 포락선이었다** — 디지털 무음은 5~18ms로 무시 가능했고, 진짜 지연은 `Tower_Install`이 **140ms에 걸쳐 부풀어 오르는 슬라이드**라 클릭보다 늦게 얹힌 것이었다(앞 75ms를 잘라 RMS 피크를 0.14s→0.06s로 당겼다). 절단면 클릭음은 3ms 페이드인으로 막는다. 업그레이드·주민 증가는 `Try*`의 **반환값**으로 성공/실패를 갈라 성공음/거절음을 낸다 — 두 버튼 모두 자원 부족에도 눌리는데(비활성화는 최대 레벨에만 걸린다) 그 경로가 여태 로그만 남기고 조용했다. **설치음은 `TowerPlacer.PlaceTower` 한 곳이 일반 배치와 합성 결과 배치를 함께 덮는다**(합성이 결과 타워를 같은 배치 경로로 놓는다). **거절음을 위해 `PlacementRequest.OnRejected`를 신설했다** — 그 전까지 무효 클릭은 발화하는 신호가 하나도 없었다. 거절음은 배치 반려 · 합성 재료/코스트 부족 · 주민 증가 실패 셋이 클립 하나를 공유한다(합성 후보 버튼의 표시 조건이 **재료 매칭뿐**이라 자원이 모자라도 버튼이 떠 있고, 그게 유일하게 자주 도달하는 실패 경로다). ⚠ 클립 레벨이 균일하지 않다 — `CantPlace`가 나머지보다 peak 6~8dB 높아 뱅크의 클립별 볼륨 없이는 거절음만 튄다. **✅ 타워 전투음(#540)**: `TowerAsset`이 클립을 소유하는 세 축(발사 `Attack.FireSfx` · 착탄 `ImpactSfx` · 빔 루프 `Beam.LoopSfx`+`BeamStage.LoopSfx`)을 추가하고 전부 `CombatSfx` 풀 `Low`로 흘린다 — 새 재생 인프라 0. 배선 16종(투사체 14 = 발사 12·착탄 2, 빔 2)이고 오라 타워는 제외했다. 발사음을 `RaiseFired` 창구에 둔 이유는 구독 컴포넌트 방식이면 새 타워를 만든 사람이 잊었을 때 그 타워만 조용히 무음이 되기 때문이다(`UiClickSfx` 전역 훅과 같은 축). 착탄음은 착탄 파티클과 **같은 구독**을 타 트리거가 갈리지 않는다(WL-208). ⚠ 클립 14본은 `Assets/Imported/@NorthLand/Sound/Towers`(별도 저장소)라 본 저장소 diff에 안 보인다. **미착수**: 주민/전투 화면 감쇠 계산 공용화, 타이틀 BGM 클립(`TitleCue`는 배치돼 있고 클립만 비었다), UI 호버 사운드(클릭과 빈도가 달라 디바운스를 함께 정해야 한다), 팬파레가 BGM에 묻히는 문제(파일 정규화 +3dB + 뱅크 볼륨 1.0으로 6dB 올렸다 — 부족하면 리미팅 또는 BGM 더킹), GUID 충돌 정리. 정본 `Docs/Core/AudioManager.md` |
...
| GameSceneManager (씬 전환 · Run 진입 핸드오프) | 미정(muchan918·sunjin1222 공동 편집) | `Assets/Scripts/GameManager/GameSceneManager.cs` | `RuntimeInitializeOnLoadMethod(BeforeSceneLoad)`로 부팅하는 `DontDestroyOnLoad` 싱글톤(씬 배치 없음 — `AudioManager`와 같은 패턴). 씬 이름은 인덱스가 아니라 정본 상수(`TitleScene`/`GameScene`/`LoadingScene`, WL-028)로 로드한다. **게임 씬 진입 5경로가 `EnterGameScene()` 단일 통로로 모이고 로딩 씬을 경유한다**(#442) — 로딩 씬이 Build Settings에 없으면 경고 후 옛 직행 경로로 폴백한다. 씬 판정은 **두 개**다: `IsTitleScene`(타이틀인가) / `IsGameplayScene`(게임 씬인가). 로딩 중에는 **둘 다 거짓**이라 서로의 부정이 아니며, 다른 매니저의 존재 여부로 씬을 추정하지도 않는다. 새 게임·이어하기·시드 지정 진입이 전부 여기를 지난다. 타이틀에서 역직렬화가 끝난 이어하기 `RunData`와 직접 입력한 마스터 시드를 다음 게임 씬까지 **일회성 핸드오프 데이터**로 임시 보관하며, `TryConsume*` 성공 시 즉시 참조와 플래그를 지워 다음 Run에 새지 않게 한다. 파일 IO·버전 판별·역직렬화는 `RunSaveLoader`, 게임 상태 복원은 `RunSaveManager`의 책임이며 이 매니저는 세이브 내용을 해석하지 않는다. |
| Loading (로딩 화면 · 부팅 워밍업, #442) | n0wst4ndup | `Assets/Scripts/GameManager/Loading`, `Assets/Scenes/LoadingScene.unity` | GameScene 진입 시 한 프레임에 몰리던 부팅 비용(실측 976.77ms)을 커튼 뒤로 옮긴다. 전이는 `TitleScene → LoadingScene(Single) → GameScene(**Additive**) → SetActiveScene → LoadingScene 언로드`다. **Additive여야 하는 이유**: `Single`로 올리면 활성화 순간 커튼이 함께 파괴돼 부팅 프레임이 그대로 노출된다. **게임 씬 카메라를 끄지 않는 이유**: 꺼 두면 URP RenderGraph 최초 컴파일·렌더 JIT(실측 444.75ms)가 커튼 뒤에서 못 일어나고 커튼을 걷은 직후 터진다 — 커튼은 카메라가 아니라 Screen Space - Overlay 캔버스(`sortingOrder = UILayer.LoadingCurtain`, `Docs/Core/UIZOrder.md` §3)가 덮는 것으로 만든다. `BootWarmup`은 **씬과 무관하게 미리 끝낼 수 있는 것만** 앞당긴다(Localization 초기화·String Table, `DataTableManager` 접촉, `Resources.LoadAll<TowerAsset>` 실측 586ms, 공유 셰이더·절차 텍스처) — 씬이 있어야 성립하는 타일 Instantiate·주민 스폰은 커튼 뒤 프레임 분산의 몫이지 워밍업이 아니다. 전부 멱등이라 로딩을 두 번 지나도 두 번째는 거의 공짜다. 준비 완료 판정은 `CombatMapInitializer.IsInitialized` 폴링(2절)이고, **실패 시 탈출 경로가 있다** — 예외·전체 타임아웃(60초) 어느 쪽이든 `Recover()`가 커튼을 즉시 걷고 게임 씬으로 넘긴다(게임 씬이 아예 못 올라왔으면 옛 직행 경로로 폴백). ⚠ 마스코트 애니메이션 클립·컨트롤러는 별도 저장소(`NorthLand-Imported`)의 `@NorthLand/UI/`에 있어 미동기화 시 마스코트만 사라진다(WL-160과 같은 축). 명세 `Docs/Core/LoadingScene.md` |
| EnemyStateUI (몬스터 상태 UI · HP 눈금 + 상태이상 아이콘, #447 · #502) | n0wst4ndup | `Assets/Scripts/UI/HealthUI`, `Assets/Resources/UI/EnemyStateUI.prefab` | 바 **폭은 전 몬스터 공통 고정**이고 HP가 많을수록 눈금이 촘촘해진다(LoL 방식) — 얇은 줄 100 HP · 굵은 줄 1000 HP. 칸당 HP가 공통이라 **남은 칸 수 = `CurrentHp` ÷ 단위**가 되고, `CurrentHp` 절댓값으로 판정하는 「체력 높은/낮은 적」 조준 정책과 화면이 어긋나던 문제가 해소된다(빌드 6 노트 :137). **보스도 같은 바를 쓴다** — 굵은 줄 덕에 `tank` 2600도 같은 폭에 들어가므로 보스 전용 HP UI를 만들지 않는다(`BossDesign.md` 확정). 단위 계산의 기준은 SO 저작값이 아니라 **실효 HP**(= SO × 웨이브 배율, 보스는 배율 제외)다. 바는 몬스터 프리팹에 **없다** — `Enemy.Spawned`(static)를 받아 씬 배선 없는 `EnemyStateUILayer`가 런타임에 붙이고, 살아 있는 전 몬스터가 **World Space Canvas 하나**를 공유해 드로우콜 합계 1이 된다(마리당 캔버스는 배칭이 안 된다). 부착 높이는 렌더러 바운즈 상단에서 스폰 시 1회 계산하므로 프리팹에 앵커도 두지 않는다(WL-055 해소 — 손튜닝 스케일 오버라이드 0건, 제거 전 월드 폭 실측 편차 3.3배). 눈금은 텍스처가 아니라 `Graphic.OnPopulateMesh` 메시라 칸 수와 무관하게 선 두께가 고정이다. **상태이상은 아이콘으로 낸다**(#502) — 종류별로 프리팹에 미리 배치해 두고 `SetActive`로만 켜므로 런타임 생성이 0이고, 꺼진 아이콘은 `HorizontalLayoutGroup`에서 빠져 남은 것들이 다시 모인다(정렬 코드 없음). UI가 읽는 창구는 `StatusEffectHandler.ActiveKindMask` 하나이며, **DoT 항목에 `EffectKind`를 새로 보관하게 한 것**이 그 전제다 — `effectId`가 `HashCode.Combine(baseId, kind)`라 종류를 되돌릴 수 없었다. 마스크는 캐시하지 않는다(만료·사망·덮임·전량해제 네 경로에서 어긋날 자리가 생긴다). 명세 `Docs/Core/EnemyStateUI.md` |
| Tutorial (튜토리얼 진행·진입, #408·#479·#488·#490·#526·#545) | muchan | `Assets/Scripts/Tutorial`, `Assets/Prefabs/Tutorial`, `Assets/Scenes/GameScene.unity` | `TutorialController.Steps`의 44개 등록 단계 + 진행 컨트롤러 + 오버레이 + 완료 조건 추상화(`[SerializeReference]`)가 정본 `GameScene`에서 동작한다. #545에서 시작 인트로·단계 사이 연출 지연·전투 공간 소개와 `CameraFocusCondition` 기반 강제 카메라 이동을 추가했고, 후반부에는 핵심 기능 소개 종료 뒤 Undo·배속·타워 공격 설정·타워 도감을 설명하는 팝업 구간이 이어진다. 전체 튜토리얼 여부의 단일 정본은 `TutorialMode.IsActive`이며, 정식 진입은 씬 로드 전에 활성화해 전용 웨이브·초기 자원·적 HP·스킬 쿨다운을 함께 적용한다. 팝업은 전체 화면 투명 패널로 다른 Canvas 입력을 막고, 행동 단계는 Flags 기반 `TutorialInputGate`를 UI와 실제 시스템 진입점 양쪽에서 검사한다. 카메라 연출은 사용자 입력과 게임 시간을 막은 상태에서도 `unscaledDeltaTime`으로 이동한 뒤 다음 팝업으로 진행한다. 말풍선은 문장 길이에 맞춰 크기를 계산하고, Bubble·Popup 최초 표시에는 `SfxBank`의 전용 안내음을 1회 재생한다. 슬롯별 완료 상태·다시 보기 Run 삭제·도감 차단·본진 재사용·완료/스킵 규칙은 기존 계약을 유지한다. 절차·44개 등록 단계 설정 정본은 `Docs/Core/Tutorial.md` |

`OutlineInteractionDriver`·`ResidentSelectionCoordinator`·`ResidentDragCoordinator`·`CursorController`는 런타임에 자가 부팅되는 전역 소비처다. 씬 오브젝트에 중복 부착된 인스턴스는 호스트 오브젝트를 지우지 않고 `Destroy(this)`로 중복 컴포넌트만 제거한다. 세 소비처의 누락 경고는 `GameSceneManager.IsGameplayScene`일 때만 **발화**하며(#442에서 `!IsTitleScene`에서 교정), 경고 latch는 `HandleSceneLoaded`에서 씬 단위로 리셋한다. 개인 테스트 씬에서는 진단을 유지한다.

⚠ **가드를 고를 때는 "언제 검사하는가"를 먼저 본다 — 전부 `IsGameplayScene`으로 바꾸면 안 된다.** 위 세 소비처는 `DontDestroyOnLoad`라 매 프레임 재평가하므로, 로딩 중(둘 다 거짓)에는 조용하고 `SetActiveScene` 이후에 발화한다. 반면 `BuildingFeedback`은 **`OnEnable`에서 딱 한 번** 검사하는 씬 컴포넌트인데, 그 시점은 GameScene이 Additive로 올라온 직후이자 `SetActiveScene` **이전**이라 `IsGameplayScene`이 아직 거짓이다 — 여기서 `IsGameplayScene`으로 바꾸면 경고가 영구히 소실된다. 그래서 `BuildingFeedback`은 `!IsTitleScene`이 맞고, 의도적으로 그대로 둔다.
...
- `GameSceneManager.Instance.LoadMainMenu()` / `LoadManageSpace()` — 타이틀 ↔ 게임 씬 전환. 씬 이름 상수는 정본 고정이라 개인 사본을 지목하지 않는다(WL-028). `LoadManageSpace()`는 남아 있는 이어하기 데이터와 직접 입력 시드를 지운 뒤 새 Run으로 진입한다. 시드·이어하기 진입은 Run/Seed·RunSave 절 참고.
- `GameSceneManager.IsTitleScene` / `IsGameplayScene` / `GameSceneName` — 씬 문맥 판정의 단일 출처. `DayNightManager` 등 다른 시스템의 존재 여부로 씬을 추정하지 않는다.
  ⚠ **두 판정은 서로의 부정이 아니다**(#442). 로딩 씬에서는 **둘 다 거짓**이다. "타이틀이 아니면 게임플레이다"라고 쓰면 로딩 씬이 그 가드를 통과해, 게임 씬에서 진짜로 필요할 때 써야 할 **일회성 경고를 로딩 씬이 미리 태워 버린다**(#442에서 `OutlineInteractionDriver`·`ResidentDragCoordinator`·`ResidentSelectionCoordinator` 3종에서 실제로 발생, WL-145 계열). 게임 씬 구성물의 누락을 알리는 경고는 `IsGameplayScene`을 쓴다.
  ⚠ 단, **한 번만 검사하는 코드는 예외**다 — 근거와 판단 기준은 §1 표 아래 문단 참고.
- `GameSceneManager.QuitGame()` — 에디터에서는 플레이 종료, 빌드에서는 `Application.Quit`.

### GameSpeed (배속 · 일시정지)

- `GameSpeedController.Instance` — **씬 스코프 싱글톤**(`DontDestroyOnLoad` 아님). 소비 측은 매번 조회하고 null을 견뎌야 한다.
- `SetPaused(GamePauseReason, bool)` — **게임 진행을 멈추는 유일한 정식 경로.** 모달 UI는 `Time.timeScale`을 직접 건드리지 않는다(`Docs/Core/UIZOrder.md`). 이유 단위 **참조 계수**라 하나라도 남아 있으면 정지이고, **켠 쪽이 끄는 것**이 계약이다 — 리워드 패널과 설정 창이 겹쳤을 때 먼저 닫힌 쪽이 나머지의 정지를 풀지 못하게 하기 위함이다. 소비처: `WaveRewardSelectionUI`(`Reward`), `SettingUI`(`Settings`), `TutorialController`·`TutorialRelayUI`(`Tutorial`), 자기 자신(`GameManager.OnResultDecided` → `ResultDecided`). `Cutscene`은 값만 있고 소비처가 없다.
- `ToggleSpeed()` — 1배 ↔ 2배. **배속 버튼(`SpeedBtn.onClick`)과 스페이스바가 공유하는 단 하나의 진입점**이라 표시와 실제 배속이 갈릴 자리가 없다. ⚠ **일시정지 중에는 무시된다**(버튼이 `CanvasGroup`으로 잠기는 것과 짝).
- `CurrentSpeed` / `IsBoosted` / `IsPaused` — 상태 pull. 정지 중에도 `CurrentSpeed`는 유지된다(정지는 배속 위에 겹치는 상태이지 1배로 되돌리는 것이 아니다). `IsPaused`는 시간이 멎었는지 알아야 하는 비동기 대기가 쓴다 — `CombatMapMonsterConnector.WaitForSpawnDelayAsync`가 unscaled로 세면서 정지 중에만 누적을 멈춘다.
- `SpeedBoostEffect.Play()` / `Stop()` — 배속 활성 표시(버튼 주위를 도는 별 파티클). **배속 상태를 스스로 판단하지 않는다** — 상태가 두 곳에 생기면 표시와 실제가 어긋날 수 있어, 신호를 받기만 한다.
...
- `CombatMapInitializer.IsInitialized` — 전투맵 초기화 완료 여부. **`LoadingFlow`가 "게임 씬 준비 완료"를 판정하는 근거**라(#442) 통합 계약이 됐다. 초기화 시점이 `Start`에서 옮겨 가면 로딩 커튼이 준비 전에 걷히므로, 이 값이 참이 되는 시점을 바꿀 때는 `Docs/Core/LoadingScene.md` §5.3을 함께 본다. 초기화 담당이 없는 씬에서는 `LoadingFlow`가 기다리지 않고 넘어간다.
...
- `NorthLand.Combat.CombatSfx.Play(AudioClip, Vector3, bool loop = false, float volumeScale = 1, CombatSfxPriority priority = Normal)` — #522 전투 위치 SFX 공용 진입점. 16개 사전 생성·최대 32개 보이스 풀, 화면 좌표 및 오쏘 80~160 줌 감쇠, 스테레오 팬, SFX 설정 실시간 반영. 핸들은 슬롯 세대로 재사용 오정지를 막는다. 상한에서는 화면 밖 무음·낮은 우선순위·오래된 보이스 순으로 회수한다. 타워 전투음 세 축(발사·착탄·빔 루프)도 이 API로 나간다(#540, `AudioManager.md` §6.3).
- `TowerAsset.Attack.FireSfx` / `TowerAsset.ImpactSfx` / `TowerAsset.Beam.LoopSfx`(+`BeamStage.LoopSfx`) — #540 타워 전투음 세 축. 각각 볼륨 배율 필드가 짝으로 붙는다. **클립 주인은 SO다**(`SfxBank`는 주인 없는 소리 전용, §5.4). 발화는 `Tower.RaiseFired`(발사) / `AttackAction`의 `Projectile.Impacted` 구독(착탄, 파티클 `ImpactVfx`와 같은 통지·같은 `isFresh` 필터라 스플래시·부메랑에서 배수로 늘지 않는다) / `BeamAction.UpdateLoopSfx`(빔, **타워당 1개**·단계는 대상 최대 진행도 기준). ⚠ 빔 루프 정지는 `HideAllBeams`에 있다 — `Dispose`·`OnWaveEnd` 두 경로를 한 자리에서 덮지 않으면 낮 내내 소리가 흐른다. ⚠ 저작 규약(클립 길이 <= 공격 간격, 어택 <= 0.03초, 발사음에 폭발감 금지, 자폭병 폭발음과 음색 분리)은 `AudioManager.md` §6.3.
- `NorthLand.Combat.CombatSfx.IsAudible(Vector3)` — #540. 그 위치의 소리가 지금 들리는가(화면 안 + 줌 감쇠 > 0). **루프 소비처 전용 계약이다** — 단발음은 들리지 않아도 곧 끝나므로 그냥 재생해 두면 풀이 회수하지만, 루프는 화면 밖에서 `LastGain`이 계속 0이라 **탈취 1순위**가 되고 뺏길 때마다 다시 잡으면 포화 상태에서 매 프레임 재획득이 반복돼 **화면 안에서 들리던 소리를 60fps로 잘라 낸다.** 그래서 루프를 새로 잡는 쪽은 `Play` **전에** 이것을 확인하고, 들리지 않으면 걸지 않는다(`BeamAction.UpdateLoopSfx`가 그 형태다). 화면에 들어오면 다음 프레임에 자연히 켜지므로 청감 손실이 없다.
- **일시정지 중 루프 정지는 풀이 소유한다**(#540) — `CombatSfxPool.Update`가 `GameSpeedController.IsPaused`를 보고 `loop` 보이스만 `Pause`/`UnPause` 한다. **소비처가 따로 막지 않는다**(`SkillField`는 코드 한 줄 없이 이 정책을 받는다). 단발음은 정지 중 소진되는 편이 자연스러워 대상에서 뺐다. ⚠ 새 루프 소비처를 만드는 쪽이 알아야 할 두 가지: `AudioSource.Pause()`가 `isPlaying`을 false로 만들지만 **풀이 회수 판정 앞에서 걸러 주고**, `CombatSfxHandle.IsPlaying`도 정지 중 **true를 답한다**(false면 매 프레임 도는 소비처가 루프가 죽은 줄 알고 정지 화면에서 새 보이스를 잡는다).
- `AudioManager.StopSfxExclusive()` — 위 경로로 울리는 소리를 즉시 정지. ⚠ **씬 전환에 반드시 필요하다** — 이 경로는 소스가 살아 있는 한 계속 울려서, 정지 주체가 없으면 9.5초짜리 팡파레가 타이틀 복귀 뒤에도 끝까지 들린다(WL-180과 같은 축). 정지 주체는 `SoundCue.Awake`이며, BGM과 마찬가지로 **씬 쪽에 둬서** 타이틀 복귀 경로 4개와 무관해진다(WL-205).
- `AudioManager.PlaySfxExclusive(AudioClip, float volumeScale = 1f, float startTime = 0f)` — 2D 전용 소스 1개. **직전 것을 끊고 처음부터 재생**하므로 연타에 겹치지 않고, `PlayOneShot`이 아니라 `clip`+`Play()`라 **매 프레임 볼륨을 다시 곱한다**(재생 중 슬라이더 반영). ⚠ 소스가 하나뿐이라 **이 경로의 소리들끼리도 서로를 끊는다** — 소비처는 주민 증가음 + 결과창 승리/패배 스팅어 **2개**. 길거나(수 초) 연타 가능한 버튼에 물린 소리 전용이다. **`startTime`(#538)은 타격이 클립 중간에 있는 스팅어를 화면 연출에 맞추는 인자다** — 결과창 승리음은 실제 타격이 0.66초 지점이라 그대로 틀면 로고 착지 한참 뒤에 소리가 난다(근거·포락선은 `AudioManager.md` §5.6). 기본값 0이라 기존 호출부는 영향 없고, `time`은 반드시 `Play()` **전에** 넣는다(뒤에 밀면 워밍업이 한 프레임 샌다).
- `TowerMergeCoordinator.CanAffordMerge(TowerRecipe)` / `OnAffordabilityChanged` — 후보 버튼의 **활성 여부**(표시는 재료, 활성은 코스트). 판정은 `TowerFusionController.CanAfford`가 하고 코디네이터는 파사드로 감싼다 — **뷰가 경영 시스템을 직접 부르지 않게** 하고(계약 #6) 코스트 규칙을 `TryFuse`와 한 곳에 둔다. 이벤트는 `ManagementController.OnChanged`를 받아 다시 낸 것 — 패널이 열린 채 되돌리기로 자원이 환불되면 표시가 낡는다(WL-209).
- `Sfx`(정적 클래스) — 주인 없는 공용 효과음의 유일한 호출 진입점: `ButtonClick()` / `PanelOpen()` / `TutorialBubbleOpened()` / `TutorialPopupOpened()` / `TowerInstalled()` / `Rejected()` / `InsufficientResources()` / `Blocked()` / `SkillOutOfCharges()` / `BuildingUpgraded()` / `ResidentIncreased()` / `Undone()` / `Redone()`(호출부 없음). 튜토리얼 안내음 둘은 `TutorialOverlay`의 Bubble·Popup 루트가 **비활성→활성**으로 바뀔 때만 나며, 열린 패널의 Localization 갱신과 닫힘에는 반복하지 않는다. ⚠ **건물 사건(업그레이드·주민 증축)의 성공음은 호출부가 아니라 `ManagementController.OnBuildingAction` 구독(`InGameCue`)에서 난다** — 같은 사건의 파티클(`BuildingFeedback`)이 이미 그 이벤트를 쓰므로 트리거를 한 벌로 모은다. 실패는 이벤트로 오지 않아 거절음만 호출부 몫이다(WL-208). **같은 큐는 한 프레임에 한 번만 재생된다** — 선택 표시 소유자가 둘이라 `OnSelected`가 한 클릭에 두 번 불리는 구조를 소리 쪽에서 흡수한다.
- `UiClickSfxIgnore.ApplyTo(Component)` — 그 버튼을 공용 클릭음에서 제외한다(중복이면 무시). **자기 결과음을 내는 버튼은 반드시 부른다** — 클릭음은 누를 때, 결과음은 뗄 때 나서 겹쳐 들린다. 소리를 배선한 코드 바로 옆에서 부르는 것이 규약이다(인스펙터 배선 금지 — 씬 병합 충돌 + 원인 추적성).
- `CommandHistory.Undo()` → **`bool`**(실제로 되돌렸는가). 호출부가 피드백을 붙일 때 `CanUndo`를 다시 검사하지 않게 하려고 낸다 — 같은 조건을 두 곳에 두면 조용히 어긋난다. 히스토리는 연출을 모른다: 소리는 `UndoRequest.Submit`이 낸다(되돌림 → 되돌리기음, 되돌릴 것 없음 → 거절음, 배치 고스트 취소 → 되돌리기음). 클립·볼륨은 `SfxBank`(`Assets/Resources/ScriptableObjects/SfxBank.asset`)가 소유하고 `Resources.Load`로 1회 집어온다. **씬 배치도 부팅도 없다**(WL-002에 자가 부팅을 더하지 않으려 정적 클래스). `SoundId` enum·딕셔너리를 두지 않는다 — 소리마다 이름 있는 메서드라 오타가 컴파일 에러로 잡힌다. ⚠ **범위는 "주인이 없는 소리"뿐** — 타워 발사음처럼 클립 주인이 이미 있는 소리는 뱅크에 넣지 않는다(각자의 SO가 들고 직접 넘기는 방향, AudioManager.md §7).
- `UiClickSfx`(자가 부팅, 씬 배치 0) — 좌클릭이 눌린 프레임에 `EventSystem.RaycastAll` 1회 → 최상단 히트에서 부모로 올라가 첫 `Selectable`을 찾고, `Button`/`Toggle`이고 `IsInteractable()`이면 클릭음. **버튼별 배선을 하지 않는 것이 목적**이다(`onClick.AddListener` 사용 파일 30개 + 런타임 생성 버튼 다수 → 하나씩 붙이면 누락이 무음으로만 드러난다). 탐색 순서가 EventSystem의 대상 선정(`ExecuteHierarchy`)과 같아 "소리만 나고 버튼은 안 눌림"이 생기지 않는다. 제외는 `UiClickSfxIgnore`(버튼 또는 패널 루트에 부착).
- `IDisabledClickFeedback.OnDisabledClick()` — #550. **비활성(회색) 버튼이 눌린 프레임에 `UiClickSfx`가 호출한다.** `Button.interactable == false`면 `onClick`이 아예 발화하지 않고 위 전역 훅도 `IsInteractable()`에서 빠지므로, **회색 버튼은 어느 소리 경로도 지나지 않는다** — 눌러도 아무 일도 안 일어난 것처럼 보인다. 무슨 소리를 낼지는 **버튼 자신이 판단한다**(`ICursorHint`와 같은 계보 — 전역 훅이 "이건 스킬 버튼인가"를 분기하기 시작하면 버튼 종류마다 조건이 하나씩 박힌다). 구현하지 않은 버튼은 조용히 넘어가고, 배선은 0이다. ⚠ **타입 필터(`Button`/`Toggle`)와 `UiClickSfxIgnore` 제외를 지나지 않는다** — 공용 클릭음이 아니라 버튼 자기 소리이므로 자기 규칙으로 낸다(`TowerUndoButtonView`가 `ApplyTo`로 클릭음을 뺀 상태에서도 이 피드백은 살아 있다).
  - ⚠ **구현 제약: 소리·연출만. 게임 상태를 바꾸지 말 것**(툴팁·패널 열기도 상태다). 이것은 취향이 아니라 **`UiClickSfx`가 존재할 수 있는 근거**다 — 팀 계약 #1 넷째 예외의 조건이 "게임 상태를 바꾸지 않는다" 하나이고 "이 예외를 근거로 삼으려면 상태 불변을 먼저 만족해야 한다"고 못 박혀 있는데, 이 메서드는 그 좌클릭 폴링 경로 위에서 실행된다. 상태를 바꿔야 하는 반응이 필요하면 이 훅이 아니라 `MouseManager` 라우팅으로 간다.
  - 현재 구현체 **6개**: `SkillButtonView`(충전 소진 → `Sfx.SkillOutOfCharges`, 연타 흡수용 0.7초 게이트) · `TowerUndoButtonView`(되돌릴 것 없음 → `Sfx.Rejected`) · `TowerButtonView`(자원 부족 → `Sfx.InsufficientResources`, 타워 팔레트·합성 후보 양쪽이 이 한 곳을 지난다) · `StoreOfferRow`(지불 자원 부족 → 〃). 다섯째·여섯째는 `BuildingInfoUI`·`CastlePanelUI`(업그레이드·주민 증가 비용 부족 → 〃)로, **버튼 위가 아니라 패널 루트에 있다** — 아래 부모 탐색 덕에 닿는다. 둘째는 **아래 `CommandHistory.Undo` 항목이 계약으로 실은 「되돌릴 것 없음 → 거절음」이 버튼 경로에서 도달 불가였던 것**을 메운다(Ctrl+Z로만 들렸다).
  - ⚠ **구현체는 비활성 사유를 스스로 계산하지 않는다.** 회색이 된 이유가 하나가 아닐 수 있고(타워 칸은 미해금·자원·튜토리얼 셋), 사유에 따라 낼 소리가 다르다 — "자원을 모으라"는 안내는 미해금·튜토리얼 제한에 붙으면 거짓말이 된다. 그래서 **게이트를 계산한 패널이 사유를 함께 넘기고**(`SetSelectable(selectable, blockedByCost)` / `SetInteractable(canExchange, blockedByCost)`) 뷰는 그 값으로 소리를 낼지만 정한다. 뷰가 다시 계산하면 판정이 두 벌이 되고, 게이트가 늘 때 한쪽만 고치면 "못 쓰는데 엉뚱한 소리가 난다"로 샌다.
  - ⚠ **사유를 만드는 방식이 두 가지이고 안전도가 다르다.** 타워 쪽(`TowerSelectPanelView`·`TowerMergePanelView`)은 회색 조건을 구성한 AND 항을 그대로 재조립해 `blockedByCost`를 만든다 — 항이 늘면 두 줄을 나란히 고치게 된다. 업그레이드 쪽(`BuildingInfoUI`·`CastlePanelUI`)은 컨트롤러가 사유를 돌려주지 않아(`CanUpgrade*`는 bool 하나) **비용 외 사유를 걷어낸 나머지를 비용으로 추정**하므로, 그 안에 모르는 게이트가 늘면 **컴파일 에러 없이** 거짓 안내음이 된다. 실제로 튜토리얼 건물 화이트리스트(`ManagementController.IsUpgradeAllowed`)가 이렇게 누락돼 무료 구간에서 「자원을 모으라」가 났고, 그 게이트를 `public`으로 열어 직접 검사하게 해 막았다. **컨트롤러에 업그레이드 게이트를 추가할 때 이 두 술어를 함께 고칠 것** — 구조 자체는 WL-232.
  - ⚠ **훅은 눌린 `Selectable`을 인자로 받는다**(`OnDisabledClick(Selectable pressed)`). 패널 하나가 자기 버튼 여럿을 대표해 구현할 수 있고 — `CastlePanelUI`는 주민 증가·업그레이드 둘이다 — 막힌 사유가 버튼마다 달라 어느 쪽이 눌렸는지 모르면 낼 소리를 정할 수 없다. 버튼 자신이 구현체인 경우는 이 인자를 보지 않는다.
  - ⚠ **훅 탐색은 눌린 `Selectable`에서 부모로 올라간다**(`GetComponentInParent`, `UiClickSfxIgnore`와 같은 규칙). 버튼이 위젯의 루트가 아닌 경우가 있기 때문이다 — `StoreOfferRow`는 행 루트에 있고 `Btn_Exchange`는 그 자식이라, 버튼 오브젝트만 보면 구현체를 못 찾는다. 자기 GO의 구현체가 먼저 잡히므로 기존 구현체의 거동은 그대로다.
- `SkillManager.IsOutOfCharges` — #550. 지금 못 쓰는 이유가 **충전 소진 하나뿐**인가(= "기다리면 쓸 수 있다"). 안내음의 조건이며, 낮 페이즈·게임 종료로 막힌 상태는 **의도적으로 제외한다** — 그때는 기다려도 충전이 차지 않아(밤 진입 시 만충 리셋) 같은 소리가 거짓 안내가 된다. `CanCast()`와 공통 게이트를 `GatesOpen` 한 벌로 공유하므로 게이트가 늘 때 한쪽만 고쳐 갈라지지 않는다. ⚠ **"쿨다운"이라 부르지 않는다** — `GDD.md` §5.6이 "감전은 쿨다운이 아니라 **충전(탄약)**을 쓴다"로 확정했고 `cooldown`은 **재충전 간격**을 가리키는 내부 이름으로만 남아 있다. `IsRecharging`도 아니다(그 이름은 `charges < MaxCharges`인 동안 — 시전 가능한 상태에서도 — 참이어야 맞는 말이 된다).
- ⚠ **패널 오픈음은 패널이 아니라 클릭하는 쪽에 있다**(`BuildingInfo.ShowOnly` / `Tower.OnSelected`). `OnEnable`에 걸면 ① `CastlePanelUI`·`StorePanelUI`가 씬 로드 시 켜진 채 시작했다가 `Awake`에서 스스로 닫히고 ② `PhasePanelSwitcher`가 낮/밤마다 하단 패널을 토글해 둘 다 오발한다. 같은 대상 재클릭은 `MouseManager.Select`의 중복 제거에 걸려 울리지 않고(패널이 이미 열려 있음), 박스·Shift 다중 선택은 `Select`를 거치지 않아 겹쳐 쌓이지 않는다.
- `PlacementRequest.OnRejected`(선택, null 허용) — **놓을 수 없는 곳을 클릭했을 때** `MouseManager`가 부른다(무효 타일 + 맵 밖·하늘). 이게 없던 동안 무효 클릭은 아무 신호도 없이 삼켜져 "클릭이 안 먹은 것"처럼 보였다. 사유는 넘기지 않는다 — 소비처가 거절음 하나뿐이라, 사유별 반응이 필요해지면 인자를 붙인다.
  - ⚠ **스킬 조준 쪽의 같은 상황은 이 창구를 쓰지 않는다** — 요청 타입이 다르고(`SkillTargetRequest`) 소비처가 소리뿐이라, `MouseManager.UpdateSkillTargeting`이 `Sfx.Blocked()`를 직접 부른다(#550). 그 조건 `!overUI && leftPressed && tile == null`은 커서 쪽 `CursorController.ShowBlocked`(`!_overTargetSurface && !_overUI`)와 **같은 술어**라, 「안 됨」 커서가 떠 있는 순간에만 「안 됨」 소리가 난다. ⚠ 배치 모드의 표면 밖 클릭은 **여전히 이 `OnRejected`를 지난다** — 두 클립이 청감상 거의 같아 갈지 않았다(`AudioManager.md` §5.4). 클립이 갈리면 그 분기를 `Sfx.Blocked()`로 옮긴다.
- `PlacementRequest.OnSurfaceHoverChanged(bool)`(선택, null 허용) — **커서가 배치 표면 위인지 바뀔 때** `MouseManager`가 고스트를 켜고 끄는 **바로 그 자리에서** 함께 부른다. ⚠ **호출 순서가 API의 일부다**: `true`는 그 프레임의 `Snap` **뒤**, `false`는 `Snap`이 돌지 않은 프레임에 온다 — `true` 시점에는 요청 측 미리보기가 이미 이번 프레임 위치로 갱신돼 있다는 뜻이라, 켜는 일을 `Snap`에 맡기고 `false`만 처리하는 구현이 성립한다. 통지를 `Snap` 앞으로 옮기면 그런 소비처가 조용히 깨진다. ⚠ 표면을 벗어나면 `Snap`이 아예 호출되지 않아 요청 측에는 갱신 기회가 없다 — 이 신호가 없던 동안 커서를 타일 밖으로 빼면 **고스트만 사라지고 풋프린트 하이라이트·사거리 원이 마지막 타일에 잔류**했다. 통지는 상태가 바뀔 때만 간다. `TowerPlacer`는 `false`만 처리한다: 켜는 일은 표면 위에서 매 프레임 도는 `Snap`(풋프린트 크기·앵커 유무를 아는 유일한 자리)이 하고, 콜백은 `Snap` **뒤에** 실행되므로 거기서 또 켜면 여분 쿼드까지 되살아난다. 셀 하이라이트 쿼드와 사거리 원은 **꺼진 채로 생성**한다(첫 스냅 전 월드 원점 노출 방지).
...
- `TowerInfoUI.Instance.ShowInfo(TowerData, ITargetingSelector = null, ITowerStatRowSource = null)` / `HideInfo()` — 전투 공간 타워 정보 패널.
  ...
  **조준 전환 행은 여전히 스냅샷**이다 — 그쪽은 원장이 아니라 정책 인덱스라 이 축을 타지 않는다.
  ...
  값이 하나뿐인 서술(`TowerStatRowData.Note` — 연발·성장 등).
  **라벨 규칙 3가지**(#536 리뷰에서 확정):
  ① **오라 반경은 사거리와 라벨을 나눈다**(`game.tower.aura_radius`). 줄 하나였을 때는 "같은 닿는 거리 개념"으로 공유했지만, 행 기반에서는 한 타워가 두 축을 동시에 낼 수 있어(`Tower`가 설계 목표로 적은 공격+오라 하이브리드) 같은 라벨의 행이 둘 생긴다. 배치 **전** 툴팁(`TowerInfoFormatter.BuildStats`)도 같은 라벨을 쓰도록 맞췄다 — 거기서는 "AttackAction이 없다"가 아니라 **"오라 액션이 있다"**로 판정해야 한다(빔 타워가 전자에 걸려 오라로 오분류된다).
  ② **지속 피해 행은 효과 이름을 쓴다**(화상·중독). `DoT`로 고정하면 합성 툴팁의 「계승: 중독」과 이름이 갈리고, 화상+중독을 함께 가진 타워에서 같은 라벨의 행이 둘 뜬다.
  ③ **빔 DPS도 `Stat` 행이다** — 피해와 틱 간격이 각각 원장 축(AttackDamage·AttackSpeed)을 타므로 실제로 버프를 받는다. 동시 타격 대상 수는 값이 아니라 **라벨**에 붙인다(`DPS ×3`): 값 칸은 서식 후 문자열 비교로 화살표를 가르므로 접미사가 남으면 기본값과 실제값이 영원히 달라 보인다.
...
- `Enemy.CurrentHp` / `MaxHp` / `event Action<float,float> OnHpChanged` /
  **`static event Action<Enemy> Spawned`**(#447), `PlayerBase.CurrentHp` /
  ...
  창구다(`PlayerBase.OnBaseSpawned`와 같은 idiom, `Docs/Core/EnemyStateUI.md` §4).
...
- `MonsterSpawnWaveProvider.TryGetWave(int, out IReadOnlyList<MonsterSpawnEntry>)` /
  ...
  거친 뒤 각 그룹의 `EnemyAsset`과 출현 수량을 UI에 제공한다. 일반 몬스터는 저작 순서를 유지하고
  보스 그룹은 실제 스폰 규칙과 동일하게 마지막에 배치한다. 랜덤 스폰 순서는 미리보기에 노출하지 않는다.
  같은 `EnemyAsset`이 다시 등장해도 합산하지 않고 별도 항목으로 유지한다. `WaveMonsterCount`는
  `EnemyAsset Asset`과 `int Count`를 담는 읽기 전용 값이며, 원본
...
- `MouseManager.Instance.ActiveCamera`(#466) — 입력 판정이 실제로 쓰는 카메라. **`Camera.main`과 같다는 보장이 없다**(`SetCamera`로 갈아 끼울 수 있고, 지금 둘이 같은 것은 씬 로드마다 `Camera.main`을 넣어 주기 때문이다). 커서에서 광선을 쏘는 시스템은 이 값을 써야 한다 — `Camera.main`을 따로 보면 그 시스템과 드롭·선택 판정이 **서로 다른 카메라 기준**이 되어 보고 있던 대상과 판정되는 대상이 갈린다(WL-023 계열).
...
- `MouseManager.Instance.IsOverTargetSurface` / `event OnTargetSurfaceChanged(bool)`(#517 후속) — **배치·조준 중 고스트·인디케이터가 지금 그려지고 있는가.** 판정은 두 모드가 다르지만(배치=`_placementMask` 히트, 조준=그 위의 `CombatMapTileView`) 같은 이름으로 나가는 이유는 소비처가 알고 싶은 것이 하나이기 때문이다 — "지금 커서를 대신할 것이 화면에 있는가". 유일한 소비처는 커서 그림이며 이 값으로 「숨김」(`Placing`/`SkillAiming`)과 「안 됨」(`Blocked`)을 가른다(CursorFeedback.md §2). 갱신 창구는 `SetOverTargetSurface` **하나**이고 `SetPlacementPreviewVisible`/`SetSkillIndicatorVisible`/`SetMode`가 그리로 흘린다. `OnModeChanged`와 같은 규약 — 이벤트는 변화만 싣고 초기 상태는 프로퍼티를 직접 읽는다.
  - ⚠ **`PlacementRequest.OnSurfaceHoverChanged`와 발행 순서가 API의 일부다.** 둘은 같은 사실을 나르지만 역할이 다르다(이쪽=전역 통지·커서, 그쪽=**그 요청의** 미리보기). `SetPlacementPreviewVisible` 한 자리에서 **이 이벤트가 먼저** 나간다 — 그쪽은 "상태가 바뀔 때만"이라 조기 반환에 걸리는데 이쪽은 그 앞이라 걸리지 않는다(`BeginPlacement`가 `_placementPreviewVisible`을 통지 없이 false로 놓는 자리가 있어 두 값이 어긋날 수 있고, 그때 이 통지까지 삼켜지면 커서가 이전 세션 상태에 갇힌다). 조준 쪽에는 요청 콜백이 없어 **배치만 두 채널**이다
  - ⚠ **「대상 표면」의 정의는 매니저가 든 전역 마스크 하나(`_placementMask`)다 — 요청별이 아니다.** 정본 씬 값이 `m_Bits: 8`(레이어 3 `Tile`)이라 실질적으로 **전투 타일 전용**이고, "경영 공간 위 = 「안 됨」"이 지금 성립하는 것은 공간을 판정해서가 아니라 경영 지면이 그 마스크에 안 걸리기 때문이다(지면 콜라이더 부재 WL-210, Terrain은 Default 레이어 WL-047). 경영 공간 배치(#27)가 같은 마스크로 열리면 **배치 내내 「안 됨」**에 고정된다 — **WL-215**
  - ⚠ **범위는 배치·조준 두 모드뿐**이다. 세 번째 모드(유닛 끌기 등)에 주려면 `SetMode`의 내리는 조건을 **함께** 늘린다 — 빼먹으면 그 모드는 값을 내려 줄 사람이 없어 모든 프레임이 「안 됨」이 된다
  - **불변식: 이 값 == 「고스트가 켜져 있는가」.** 그래서 `Begin*` 둘 다 고스트를 **비활성으로** 만들어 둔다(위치를 잡기 전에 켜면 월드 원점에 보인다 — 조준 쪽은 이 방어가 빠져 있다가 #517 후속에서 배치와 같은 형태로 맞췄다). ⚠ **`Begin*`에서 이 값을 미리 참으로 맞추지 말 것** — 커서는 숨는데 고스트는 아직 비활성이라 화면이 통째로 빈다(`Hidden`을 되돌렸던 그 상태). 요청 측 미리보기를 함께 켜는 우회도 막혀 있다: `TowerPlacer`는 사거리 원·셀 하이라이트를 `BeginPlacement`가 **돌아온 뒤에** 만든다
...
- `Tower.Stats`(`TowerStats`, `NorthLand.Combat`) — **이 타워의 스탯 modifier 단일 원장.** 타일 버프·
  ...
  **원장 축 커버리지(오라 타워 포함)**: 공격 타워는 AttackDamage/AttackRange/AttackSpeed, 오라 타워는
  ...
  않는다"며 `AttackRange` 축을 공유했는데, 같은 증가량을 공유하면 기본 반경이 다른 두 축의 실효 면적이
  과도하게 갈린다. 그래서 사거리 버프 타일은 공격 타워의 `AttackRange`에 Flat **+2.5/+5/+15**,
  버프·디버프 오라 타워의 `AuraRadius`에 전용 Flat **+2/+5/+10**을 적용한다. 두 축 모두 Flat이지만
  단계별 증가량은 독립적으로 저작하며, 다중 셀 오라 타워가 여러 사거리 타일을 점유하면 가장 높은 단계
  하나만 적용한다. **축만 갈렸고 단일 출처는 유지된다**: 판정(`ApplyDebuff`)·선택 원·장판
...
- `TowerPlaceCommand`(#281, 순수 C#, **#444에서 `ReversibleCommandBase` 파생**) — 배치의 되돌리기. `OnExecute`=**인수(adopt)만 한다(부작용 없음)** / `Commit`은 할 일이 없어 재정의하지 않는다 / `OnUndo`=(확정본이면)`ClearSelection`→`Release()`→`Destroy`이고 **실지불 비용 환원은 기반 클래스가 한다**.
...
- `ReversibleCommandBase`(#444, 추상, `Assets/Scripts/Command/`) — `IReversibleCommand` 4단 상태 기계(`Pending`→`Executed`→`Confirmed`→`Committed` / `Undone`)와 **실지불 비용 환원**을 담은 공통 기반. 구현체는 `OnExecute`/`OnConfirm`/`OnCommit`/`OnUndo(bool wasConfirmed)`만 채운다 — **상태 전이는 여기서만** 일어난다. 승격 이유: 되돌릴 수 있는 조작이 셋(배치·합성·경영)이 되면서 같은 가드가 셋으로 갈라지고, "`Commit`은 `Confirmed`만 받는다" 같은 규칙 하나가 어긋나면 밤 진입이 진행 중 세션의 재료를 먼저 파괴한다.
  ⚠ `OnUndo`의 **반환값은 "비용을 환원해도 되는가"**(= 상태 복원 성공)다. false면 환원하지 않는다 — 상태는 그대로인데 자원만 돌아오면 무료 업그레이드가 된다(WL-057 계열).
- `ResourceSpendCommand`(#444, sealed, `Assets/Scripts/Command/`) — "자원을 냈고 수치 하나가 올라갔다" 모양의 조작 전부를 담는 범용 커맨드. `(management, paid, Func<bool> revert, label)`을 받아 **인수만** 하고, 되돌릴 때 `revert()`를 부른다. 경영 업그레이드가 이걸 쓴다.
  ⚠ 되돌리는 방법으로 **세이브 복원 API(`ManagementController.TryRestore*`)를 그대로 재사용한다** — 그쪽이 이미 "비용·페이즈 게이트를 타지 않고 상태를 그 값으로 맞춘다"라서, 되돌리기 전용 감소 API를 따로 열면 같은 런타임 상태를 쓰는 경로가 둘이 되고 그 둘이 어긋나는 순간을 아무도 못 본다.
- `UndoRequest.Submit()`(#444, static, `Assets/Scripts/Command/`) — "되돌려라" 요청의 단일 진입점. **되돌리기 버튼과 Ctrl+Z가 같은 이 경로**를 쓴다(고스트를 들고 있으면 그것부터 치우고 끝낸다). 밤·빈 스택 판정은 `CommandHistory.CanUndo`에 맡기고 중복하지 않는다. Ctrl+Z 등록도 여기서 한다(`KeyboardManager.Bind`) — **단축키를 원하는 쪽이 자기 단축키를 등록한다**.
  ⚠ **선택 해제는 여기서 하지 않는다** — 무조건 풀면 건물 업그레이드를 되돌릴 때 방금 올린 건물의 패널이 닫혀 되돌아간 레벨을 볼 수 없다. 무엇이 파괴되는지 아는 `TowerPlaceCommand.OnUndo`가 `wasConfirmed`일 때만 푼다(`ResidentSelectionCoordinator`의 "주민이었을 때만 푼다"와 같은 판단).
- `KeyboardManager.Bind(Key, KeyModifier, Action, string label = null, bool exactModifiers = true)` / `Unbind(...)`(#444, static, `Assets/Scripts/GameManager/KeyboardManager/`) — 단축키 등록/해제. 같은 (키+보조키+핸들러)는 중복 등록되지 않는다. **보조키 판정은 바인딩별로 고른다(#537)** — 기본 `exactModifiers: true`는 정확히 일치, `false`는 "요구 보조키 포함, 나머지 무시"다(`Binding.AcceptsModifiers`, `KeyModifier` 주석). ⚠ **무보조키 단축키를 어느 쪽으로 등록할지는 미결이다(WL-219)** — 새 무보조키 바인딩을 추가하기 전에 그 항목을 먼저 볼 것.
  ⚠ 바인딩 목록은 **플레이 세션마다 비우지 않는다** — 등록도 `[RuntimeInitializeOnLoadMethod]`라 초기화 순서가 보장되지 않아 리셋이 그 프레임의 등록을 지울 수 있다. 대신 중복 검사가 "도메인 리로드 없이 플레이"의 이중 등록을 막는다. 인스턴스 메서드를 넘겼다면 대상이 사라질 때 `Unbind`할 것(목록이 static이다).
...
- `TowerMergePanelView`(#183, UI) — 합성 패널. **코디네이터만 참조**(파사드). 상단 선택 리스트 + 하단 후보 버튼(레시피당 1개 미리 생성 후 기본 숨김, `CanMerge`면 `SetActive`, onClick→`RequestMerge`). 카탈로그=`TowerRecipeCatalog.All`(=`Resources.LoadAll<TowerRecipe>("ScriptableObjects/TowerRecipes")`, lazy 1회 — 폴더 투입만으로 후보에 오른다. 구 인스펙터 배열 `TowerRecipe[] _recipes`는 폐기). **후보 버튼 표시 = 결과 타워 아이콘 + 이름**을 프리팹(`TowerButton.prefab` — 배치 팔레트와 같은 것)의 `TowerButtonView.Set`으로 채운다(#445): 예전엔 `GetComponentInChildren<TMP_Text>()`로 라벨만 채워 아이콘 슬롯이 빈 칸이었다. `SetLocked`는 부르지 않는다 — 합성 후보에 해금 개념이 없고 프리팹의 `TowerLockOverlay`는 `m_IsActive: 0`이라 조용하다(`TowerMergeTargetSlot`의 같은 판단). ⚠ **버튼 순서 비결정적** — `Resources.LoadAll`이 순서를 보장하지 않아 구 배열이 주던 결정적 순서가 사라졌고 아직 정렬을 안 넣었다(`TowerMerge.md` §5 ⚠)
- `TowerMergeTargetIndex.RecipesUsing(string towerId)`(순수 static) — **재료→그 재료를 쓰는 레시피들**. 합성 패널의 `CanFuse`(선택 집합→레시피)와 도감의 `recipeByResult`(결과→재료)에 이어 **세 번째 조회 방향**이며, 타워 1개 선택 시 정보 패널에 "상위 타워"를 띄우는 데 쓴다. 재료 집계는 `TowerFusionMatcher.BuildRequired`를 그대로 쓴다(§6 단일 출처 — 재구현 금지). `Result` 없는 레시피는 제외. 카탈로그가 런타임 불변이라 lazy 1회 구축. 명세 `TowerMerge.md` §8.5
- `TowerDisplayName.Of(TowerAsset)` / `.EnsureData(TowerAsset)` — **타워 표시 이름 단일 출처**. `TowerID` → `TowerData.NameKey` → `NorthLand_Towers` 로컬라이즈, 폴백은 `TowerID` → SO 파일명 → `"?"`. `EnsureData`가 런타임 전용 `TowerAsset.Data`(에셋 미직렬화) 채움을 한 곳에서 책임진다. 합성 패널·도감(`FusionTowerCodexUI`)·정보 패널 후보 행·툴팁 헤더 4곳이 각자 구현하던 것을 수렴시킴(폴백이 갈려 같은 타워가 화면마다 다르게 불렸다)
- `TowerAsset.IsUnlocked(int currentWave)` — **타워 해금 판정 단일 출처**(#504). `currentWave >= UnlockWave` 한 줄이며 **씬을 모른다**. 소비처 둘: `TowerSelectPanelView`의 버튼 게이트(자물쇠·회색 처리)와 `TowerTooltipView`의 해금 웨이브 줄 — 각자 비교하면 게이트는 잠갔는데 툴팁은 열린 것처럼 보인다. 별도 static 헬퍼를 두지 않고 SO의 메서드로 둔 이유는 `UnlockWave`와 같다(게이트와 그 기준값을 한 곳에). ⚠ **웨이브를 SO가 스스로 끌어오지 않는다** — 초안은 `IsUnlocked(DayNightManager = null)`로 인자가 비면 `Instance`를 봤는데, 그러면 (a) 데이터 SO가 씬 수명 오브젝트에 의존해 asmdef 격리 시 역방향 엣지가 되고(WL-120·WL-152), (b) 같은 메서드가 push(패널)·pull(툴팁) 두 입력을 받아 판정 입력이 갈리고, (c) 씬 없이 EditMode 검증이 불가능했다. 웨이브 조달은 `DayNightManager.CurrentWaveOrMax`(정적 조회창) 하나로 모았다
- `DayNightManager.CurrentWaveOrMax`(static) — 웨이브 게이트용 조회창. `Instance`가 있으면 `CurrentWave`, 없으면 `int.MaxValue` = **"웨이브 제한 없음"**(타워 테스트 씬 등 permissive 규약의 단일 출처 — 호출부마다 적으면 화면별로 갈린다). 비활성 매니저는 없는 것으로 취급되는데 그런 오브젝트는 `OnDayStart`도 발행하지 않아 어차피 웨이브가 오르지 않는다(게이트만 잠그면 영구 잠금이 된다). 캐시 금지 — 세이브 복원(`TryRestoreState`는 페이즈 이벤트를 발행하지 않는다) 후 잠긴 채 남는다
- `TowerMergeTargetSlot.Set(Sprite, string)`(UI) — 정보 패널 "합성 후보" 블록의 한 칸. **정본 프리팹(`@NorthLand/Prefabs/UI/TowerTargetSlot.prefab`)은 아이콘만 그린다 — 이름 칸은 의도적으로 없다**(정보 패널 폭이 좁아 이름 배너를 넣으면 칸이 커져 블록이 패널을 밀어낸다). **이름·역할·스탯·재료는 호버 툴팁이 낸다** — `TowerInfoUI`가 `TowerTooltipSource`를 런타임 부착하므로 프리팹 배선 불필요, 단 `Slot/Img_Bg`의 `Raycast Target`을 켠 채로 둬야 호버가 잡힌다(`Button`을 떼도 남는다). `_name`은 이름 칸을 가진 변종용 **선택 슬롯**이라 정본에서는 미할당이 정상이고, `Set`의 `displayName`은 그때 버려진다 — ⚠ 그래도 호출부가 `TowerDisplayName.Of`를 계속 넘기는 것은 그 호출의 `EnsureData` 부수효과가 **툴팁이 읽을 `Data`를 채우기** 때문이다(지우면 툴팁이 `TowerID`로 떨어진다). 프리팹은 `TowerButton.prefab`을 복제해 `Button`(누를 수 없는 칸)·`TowerButtonView`·`TowerLockOverlay`·배너 서브트리를 떼고 이 컴포넌트를 붙인 것. `TowerButtonView`를 그대로 안 쓰는 이유는 그쪽 `SetLocked`·해제 연출이 팔레트의 **해금** 개념과 한 몸이고 정보 패널엔 잠금이 없어서. ⚠ 프리팹이 **별도 저장소 소속**이라 미동기 시 블록이 조용히 사라진다 → `TowerInfoUI.HasMergeSlotWiring`이 1회 경고, 동기화 계약은 §4
- `TowerButtonView.SetSelected(bool selected, bool anySelected)`(UI, #563) — 배치 팔레트 칸의 **"이 칸의 타워로 배치 모드에 들어와 있는가"** 표시. **상태의 정본은 패널 하나**(`TowerSelectPanelView._selectedTower`)이고 버튼은 `RefreshButton`에서 "선택된 타워 == 이 칸"을 받아 그리기만 한다 — `SetLocked`·`SetSelectable`과 같은 축이다(버튼이 스스로 기억하면 세이브 복원처럼 갱신이 이벤트 없이 오는 경로에서 표시와 상태가 갈린다, #424). **표시는 파티클과 스케일 두 겹이다** — 선택된 칸이 `_selectedScale`(기본 1.1), 배치 세션 중인 **나머지 칸**이 `_unselectedScale`(기본 0.9)배가 되고, 아무도 고르지 않은 평상시엔 전부 저작 스케일 그대로다. 인자가 둘인 것이 이 **세 번째 상태** 때문이다 — `selected == false` 하나로는 "남이 골렸다"와 "아무도 안 골랐다"가 구별되지 않아 배치 모드에 들어가기도 전에 팔레트 전체가 쪼그라든다. 판단은 여기서도 패널이 한다(`_selectedTower != null`). ⚠ 배율의 기준은 `Vector3.one`이 아니라 **프리팹 저작 스케일**이다(`CacheAuthoredScale` — 절대값을 박으면 칸에 스케일을 준 프리팹에서 첫 선택 때 크기가 조용히 바뀐다. `CacheIconColor`가 `Color.white`를 상수로 박지 않는 것, 도감 `FusionTowerEntry.originalScale`과 같은 규약). ⚠ **스케일 적용은 파티클 배선 가드보다 앞이다** — 아래 두 이탈(`_selectedEffect` 미배선 / 중복 호출)은 파티클만의 사정이라, 뒤에 두면 별도 저장소 미동기 환경에서 스케일까지 함께 죽는다. `localScale`은 `LayoutGroup`이 보지 않아 칸 자리가 밀리지 않는 대신 커진 칸이 이웃 위로 삐져나오는데, 형제 순서를 올려 앞에 그리면 그 순서가 곧 배치 순서라 칸 위치가 바뀐다 — 겹침은 그룹의 `spacing`으로 푼다. 해제는 `TowerPlacer.BeginTowerPlacement(so, onEnded)`의 **종료 통지 한 가닥**으로만 오며 확정·취소·합성 가로채기(`TowerFusionController`)·씬 전환·밤 진입(`PhasePanelSwitcher`)이 전부 그 길로 모인다 — 패널이 각 경로를 알 필요가 없다. ⚠ **선택 표시는 `SetSelectable`과 별개 축이다** — 배치 도중 자원이 줄어 칸이 회색이 돼도 "지금 이걸 놓는 중"은 세션이 끝날 때까지 남는다. ⚠ **`keepPlacing`(연속 배치)이면 확정으로 세션이 끝나지 않아 표시도 유지된다**(`MouseManager`가 `KeepPlacingAfterConfirm`일 때 `CancelPlacement`를 건너뛴다) — 회귀가 아니라 세션이 실제로 살아 있는 것이다. ⚠ 파티클은 **unscaled 강제**(§6 기준, `SpeedBoostEffect`와 같은 근거 — 2배속에서 혼자 빨라지거나 설정·튜토리얼 정지에서 얼어붙으면 안 된다) + **끌 때 `StopEmittingAndClear`, 켤 때 `Clear` 선행**(패널째 비활성화되는 밤 전환은 `Stop` 경로를 타지 않아 잔상이 남는다). 표시 상태(`activeSelf`)가 곧 직전 값이라 별도 플래그를 두지 않는다(`SetLocked`와 같은 규약). ⚠ 서브트리(`Slot/Img_Bg/Fx_Selected`)와 머티리얼이 **별도 저장소 소속**이라 미동기 시 조용히 사라진다 → `EnsureSelectedEffectWired`·`CacheSelectedParticles`가 각각 1회 경고, 동기화 계약은 §4
- `TowerTooltipView.Show(TowerAsset, RectTransform anchor, string extraStatsLine = null, TowerRecipe recipe = null)`(UI) — 타워 전용 호버 툴팁. **코스트 슬롯은 "무엇이 나가는가"를 말하며, `recipe`를 받으면 자원이 아니라 재료 타워를 낸다**(#445): 합성은 결과 SO의 `Cost`(합성 전용 타워는 비어 있다)를 지불하지 않고 **재료 타워 + `recipe.ExtraCost`** 를 낸다. 재료 집계는 `TowerFusionMatcher.BuildRequired`, 이름은 `TowerDisplayName.Of`, '재료' 라벨은 `game.merge.materials`(`NorthLand_default`) — 세 규칙 모두 기존 단일 출처를 그대로 쓴다. 아이콘도 같은 `TowerAsset.Icon`을 그린다(미할당이면 슬롯 off — 스프라이트 없는 `Image`는 폴백이 깔아둔 회색 판으로 그려져 아이콘처럼 보인다). 공급자는 `TowerTooltipSource.Init(TowerAsset, TowerRecipe recipe = null)`(배치 팔레트 버튼·정보 패널 합성 후보 칸)과 `TowerMergeCandidateHover`(합성 패널 후보 버튼, `extraStatsLine`으로 계승 줄도 함께 넘긴다). **아직 잠긴 타워면 코스트 아래 전용 슬롯(`UnlockWave`, 패널 마지막 자식)에 `해금 웨이브: n`이 붙는다**(#504, 판정 `TowerAsset.IsUnlocked` · 서식 `TowerStatsFormatter.BuildUnlockWaveLine`) — 22px Bold·붉은색(1, 0.42, 0.38)으로 스탯(18 연청)·코스트(18 노랑)와 갈라 둔다: 이 줄은 타워의 성능이 아니라 **지금 쓸 수 없다는 상태**를 말하므로 같은 크기·색이면 스탯 한 줄로 읽힌다. 해금되면 슬롯을 접어(`SetActive(false)`, `_descText`와 같은 규약) 원래 툴팁·원래 높이로 돌아간다. 시그니처는 안 바뀌었다(툴팁이 스스로 판정한다). ⚠ **`recipe`가 있으면 이 줄을 내지 않는다 — 합성 결과 SO의 `UnlockWave`는 읽지 않는 값이기 때문이다.** 합성 타워의 출현 시점은 결과 SO가 아니라 **재료의 해금 웨이브**로 만든다("재료 중 가장 늦게 열리는 것" — `TowerRoster.md` §해금 사다리, 게이팅 손잡이는 직접배치 7종의 `UnlockWave` 하나로 유지). 그래서 결과 SO 13종이 전부 `UnlockWave = 1`인 것은 우연이 아니라 규약이고, 합성 패널에도 해금 게이트가 없다(`TowerMergePanelView`가 `SetLocked`를 부르지 않는 것과 같은 전제). 툴팁이 그 값을 읽으면 게이트가 없는 화면에 경고만 뜨는 어긋남이 되므로 표시 경로도 같은 규약을 따른다(코스트 슬롯이 `recipe`로 갈라지는 것과 같은 축)
...
- `ResidentSpawner` — 인원(`crowdSize − AssignedTotal`)과 밤낮 출입의 소유자.
  ...
  - `bool TryCarry(Resident)` / `ReleaseCarried(Resident, Vector3 착지점)` / `ConsumeCarried(Resident)` — 드래그로 들기(Resident.md §8 · §11.15 · §11.17). 통지 구독이 아니라 `ResidentDragCoordinator`가 **직접 부르는** 유일한 경로다(특정 개체를 지목하는 조작이라 상태 통지로 표현되지 않는다).
    ⚠ **`ConsumeCarried`는 `despawnEffectPrefab`("뿅")을 재생하지 않는다 — 그 파티클은 패널 +1 전용이다.** 설명하는 바가 다르다: 패널 +1은 *플레이어가 손댄 적 없는 자리에서 주민이 갑자기 사라지는 것*이고, 드롭은 플레이어가 직접 집어 넣은 것이다. 드래그 연출은 #466에서 들어왔지만(§11.17) 배치 성공의 소멸 표현은 여전히 비어 있다 — 있는 것을 임시로 돌려쓰면 나중에 어느 쪽 연출인지 구분이 안 된다.
...
- `CameraController2.FocusViewCenterForTutorial(Vector3, float groundY, bool changeZoom, float orthographicSize)` / `CancelTutorialFocus()` / `TutorialFocusCompleted` — 튜토리얼 입력 게이트와 게임 정지에 막히지 않는 강제 화면 중앙 이동 계약. 위치와 선택한 줌 목표가 모두 끝났을 때만 완료 이벤트를 보내며, 조건 종료·튜토리얼 중단은 취소 API로 이동을 정리한다.
...
- `TutorialInputGate.Allows` / `AllowsForDisplay` / `AllowsEndDay` — 16종 `TutorialAction` Flags로 행동 단계의 실제 실행과 UI 표시를 분리한다. 팝업에서는 실행 플래그를 `None`으로 닫되 다음 행동 버튼은 활성 색으로 표시할 수 있다. UI `interactable`만 믿지 않고 Undo·낮 종료·스킬·경영·타워 배치/합성 등 실제 진입점에서 다시 검사한다. `Clear`는 제한과 낮 종료 타워 요구치를 함께 초기화한다.
- Undo 버튼과 Ctrl+Z는 `UndoRequest.Submit()` 단일 진입점을 공유하며 둘 다 `TutorialAction.Undo`를 검사한다. 낮 종료 타워 수는 `Tower.Active`의 현재 상태를 세므로 `TowerPlaceCommand.OnUndo`가 Destroy 전에 타워를 비활성화해 같은 프레임에 목록에서 제거한다.
...
- `CameraFocusCondition`(#545) — 팝업 없는 연출 단계의 `Begin`에서 월드 위치 또는 전투 그리드 셀로 강제 카메라 이동을 시작하고, 위치·선택 줌이 모두 도착하면 `Satisfied`를 보낸다. `CameraMovedCondition`처럼 사용자 조작을 측정하는 조건이 아니다. 기본 단계 설정은 게임 정지 + 입력 전부 제한이며 이동은 `unscaledDeltaTime`으로 계속된다.
- `TutorialContext` — 조건과 단계 규칙이 재사용하는 씬 참조 주소록. 경영·카메라·몬스터 스폰·타워 배치/합성·건물 바로가기 등을 최초 접근 때 한 번 탐색해 캐시한다. `Tower.ActiveChanged`처럼 static인 대상과 명확한 `.Instance`가 있는 대상은 조건이 직접 잡는다.
- `TutorialOverlay.ShowPopup(string, string, Sprite)` / `ShowBubble(string)` / `HidePopup()` / `HideBubble()` / `HideAll()` / `PopupConfirmed` / `SkipRequested` — 표시 전용. 단계를 모르며, 확인·스킵 클릭을 이벤트로만 알린다. 팝업과 별도인 전체 화면 투명 `PopupInputBlocker`가 다른 Canvas 입력을 막고, 말풍선 루트는 `raycastTarget`을 꺼 뒤쪽 월드 클릭을 살린다. Bubble·Popup 루트가 새로 활성화될 때만 `Sfx` 안내음을 내고, 열린 상태의 언어 변경 재표시에는 반복하지 않는다.
...
| Combat ↔ EnemyStateUI(HealthUI)          | **표시 전용 단방향 읽기 계약**(#502). UI가 소비하는 것은 `StatusEffectHandler.ActiveKindMask`/`Has(EffectKind)`/`CountOf(EffectKind)`와 `Enemy.HasExecuteMark`(읽기 전용 파생 — `executeMarkRemaining`을 읽기만 하며 #318 `TakeDamage` 처형 판정 경로는 무수정) 넷뿐이다. `ActiveKindMask`는 **캐시하지 않고 매번 계산**한다(만료·사망·감속으로 덮임·전량해제 네 경로에서 갱신을 놓칠 위험 때문). 스택 수는 `CountOf`이고 **스턴은 항상 0/1**이다(`stunActive` 중 새 소스가 오면 `ApplySlow`가 `ExtendStun`으로 기존 `stunSource` 슬롯만 갱신하고 `slows`에 새 항목을 만들지 않음 — 이 불변식이 깨지면 스턴 아이콘에 겹 수가 붙는다). Combat 쪽에서 `EffectKind`를 늘리거나 `ApplyOrRefresh(effectId, kind, ...)` 시그니처를 바꾸면 아이콘 슬롯(상태이상 4 + 처형 표식 1)도 함께 늘어야 한다 |
...
| Management(Resource) ↔ PlayerSkill(마법 연구소) | **✅ 착지점 확정·구현 완료(#205, #465)**. `SkillManager`는 `ManagementController.GetUpgradeLevel(magicLabAsset)`으로 **레벨(int)만** 읽고, `magic_lab.asset`의 `Skill.UpgradeLevels`에서 데미지 `DamageValue`와 재충전 `CooldownSeconds`는 **절대값**, 사거리 `RadiusMultiplier`는 **배율**로 적용한다. Lv0 기본값은 GameScene의 `SkillManager`가 소유하며, 관리 컨트롤러는 스킬을 모른다. `BuildingInfoUI`도 선택된 건물의 같은 값을 읽어 실제 적용값과 동일한 미리보기를 표시하고, 사거리 곱셈만 `static SkillManager.Scale`을 공유한다. **기본 감전은 #465부터 `SkillStarEffect` 별 낙하 후 지연 착탄 + 중앙 100%/외곽 70% 피해**이며, 연구소 사거리 배율에 맞춰 연출과 인디케이터 크기가 증가한다. 과거 `SkillVisualSet` 기반 레벨별 착탄 프리팹 교체(#206)는 코드만 남고 GameScene에서는 미배선이라 현재 미사용이다. **보상 특수효과**(`SkillEffect.Level`/`SkillEffectManager.GetLevel`, #169)는 독립된 두 번째 축이며 동시에 스택된다. 상세: `BuildingUpgrade.md` §8, `PlayerSkill.md` §3.1~§3.2 |
...
| Command ↔ TowerFusion / MouseManager / DayNightManager / Management / KeyboardManager | **히스토리에 오르는 것은 항상 바깥쪽 커맨드 하나**다 — 합성 결과 타워도 `TowerPlaceCommand`로 만들어지지만 `PlacementOwner.Caller` + `AdoptResult`가 편입해 따로 오르지 않는다(연출 소유권도 `PlaysUndoDissolve`로 함께). 밤 확정은 `OnDayToNight` 지연 자기구독이며 **`PhasePanelSwitcher.ShowNight`와 구독 순서가 무관하다**(진행 중 커맨드와 스택의 것은 겹치지 않는 두 집합). 자원 환원은 `ManagementController.Grant` 단일 경로(WL-017 유지). **#444로 경영 조작(업그레이드 · 주민 증축)이 같은 스택에 합류했다** — 등록을 `ManagementController`가 직접 하므로(`PushSpendUndo`) `BuildingInfoUI`·`CastlePanelUI`·테스트 헬퍼 등 **호출부를 하나도 고치지 않고** 전부 되돌릴 수 있게 된다. 되돌리기는 세이브 복원 API를 재사용하므로 **경영 런타임 상태를 쓰는 경로가 늘지 않는다**(WL-016 유지). 단축키(Ctrl+Z)는 `UndoRequest`가 `KeyboardManager.Bind`로 스스로 등록하므로 **KeyboardManager는 되돌리기를 모른다**. 상세 `TowerMerge.md` §9.3, `BuildingUpgrade.md` §10 |
| Tutorial ↔ MouseManager / Management / Combat / PlayerSkill / DayNightManager | 전체 튜토리얼 여부는 `TutorialMode.IsActive` 하나만 읽는다. 작업 씬의 `startOnPlay`는 `TutorialController.Awake`에서 이 값을 먼저 켜는 입력일 뿐 소비 API가 아니다. `forceTutorialWaves`는 웨이브 구성 전용이라 초기 자원·적 HP·스킬 쿨다운·튜토리얼 UI에 전파하지 않는다. 월드 선택 제한은 대상의 `ITutorialSelectionGate.SelectionAction`으로 분류하며 MouseManager에 구체 타입 목록을 두지 않는다. 낮 종료 타워 요구치는 `TutorialInputGate`가 `Tower.ActiveChanged`를 소유해 `Changed`로 UI에 전달하므로 Management UI가 Combat의 `Tower`를 직접 구독하지 않는다. 상세 `Docs/Core/Tutorial.md` §1.6·§4 |
...
| GameSpeed ↔ 모달 UI / KeyboardManager    | 게임 진행을 멈춰야 하는 모달은 `Time.timeScale`을 직접 건드리지 않고 `SetPaused(GamePauseReason, bool)`로 요청하며 **자기가 켠 이유를 자기가 끈다**(`Docs/Core/UIZOrder.md`) — 이유 단위 참조 계수라 겹친 모달 중 먼저 닫힌 쪽이 나머지의 정지를 풀지 못한다. 새 모달은 전용 `GamePauseReason` 값을 받고, 켜고 끄지 않으면 게임이 영원히 멈춘 채 원인이 화면에 드러나지 않는다. 스페이스바 바인딩은 `GameSpeedController.OnEnable`/`OnDisable` 대칭이 유일한 방어다(바인딩 목록이 static이라 해제가 호출부 책임 — WL-199). **정지 중 `ToggleSpeed`가 무시되는 것이 버튼(`CanvasGroup` 잠금)과 단축키의 동작을 일치시키는 조건**이라, 한쪽만 바꾸면 모달을 닫는 순간 누른 기억이 없는 배속이 된다 |
...
1. **입력 단일 창구**: 포인터 입력은 MouseManager만 읽는다. **키보드 단축키(눌린 순간 1회)는
   ...
   **연속 입력**(카메라 WASD·줌 — 매 프레임 `isPressed`를 읽어 "눌린 순간 1회" 모델에 맞지 않는다,
   ...
   `GameManager.Instance`가 없는 씬에서 ESC를 처리하지 않는다.
   넷째 예외 — **포인터 입력의 부수 피드백**: `UiClickSfx`(#406)가 좌클릭 눌린 순간을 직접 읽어
   버튼 클릭음만 낸다. **조건은 "게임 상태를 바꾸지 않는다"** — 선택·배치·모드 전환을 일절 하지 않고
   `EventSystem` 레이캐스트로 눌린 `Selectable`을 확인해 소리만 낸다. MouseManager로 라우팅하지 않는
   이유는 그쪽이 `GameScene` 배치라 **`TitleScene`에는 아예 없고**(MouseManager.cs:125-130) 타이틀
   버튼에도 클릭음이 필요하기 때문이다. 상태를 바꾸는 새 폴링은 여전히 금지이며, 이 예외를 근거로
   삼으려면 "상태 불변"을 먼저 만족해야 한다. 같은 씬에 두 번째 ESC 소비처가 필요해지면 중앙 Cancel 라우터로
...
3. **자원 흐름** (GDD §3.2): 기본 자원(나무/철/식량) = 주민 배치 생산, 마나석 = 전투 보상.
   **자원은 이 4종뿐이다(#337)** — 영토 확장 보상 경로와 특수 자원 4종(금·루비·사파이어·다이아)은
   ...
   - **되돌리기 환원(#281 → #444)**: 위 규칙의 **명시적 예외 하나**가 `ManagementController.Grant(IReadOnlyList<ResourceCost>)`다. 지불과 환원이 다른 시점·다른 조작에서 일어나 "차감+지급이 한 트랜잭션" 형태에 담기지 않으므로, 대신 **호출 조건**으로 계약을 지킨다: 인자는 커맨드가 들고 있는 **실지불 비용**이어야 하고 임의 수량 지급에 쓰지 않는다. 부르는 곳은 `ReversibleCommandBase.Undo` **한 줄**뿐이며, #444로 대상이 타워 배치·합성에서 건물 업그레이드·주민 증축까지 늘었다. 구조적 차단(영수증 패턴)은 **WL-147**이 추적한다.
...
6. **책임 경계** (MouseManager.md §2): 배치 판정=그리드/검증 시스템, 자원 차감=경영 시스템,
   ...
   - **커서 전역 상태는 `CursorController` 단독 소유 — 다른 시스템의 기입 금지.** 대상은 `Cursor.visible`과
     `Cursor.SetCursor`(그리고 `Cursor.lockState`) 셋이며, 둘 다 **전역 값이라 마지막에 쓴 쪽이 이긴다.**
     `Placing`·`SkillAiming`에 `Hidden`이 켜지면서(#517 후속) 이 기제는 휴면이 아니라 **실가동**이 됐다 —
     본 저장소에서 기입자는 그 클래스뿐이고(`Assets/Scripts` 전수), Imported 데모 3종
     (`Sweet_Land`의 `PlayerCharacterInputBase`·`EditorLikeCameraController`, `Hovl` `HS_CameraHolder`)이
     같은 값을 건드리지만 정본 씬 미참조라 **지금만** 안전하다. 일시정지·컷신 등이 이 값을 쓰기 시작하면
     `CursorController`는 내부 캐시가 일치한다고 믿어 **되돌리지 않고**, 증상("커서가 안 보인다")이
     원인에서 가장 멀다. 커서를 숨기거나 바꿔야 하는 새 상태는 `CursorKind` 칸을 늘려 그 클래스를 경유할 것
     (CursorFeedback.md §2·§5).
...
8. **저장소 배치** (CLAUDE.md): 스크립트 정본은 `Assets/Scripts/`(공간/시스템 폴더), 씬 등 비-스크립트 WIP는 `Assets/Personal/<이름>/`, `Assets/Imported/` 수정 금지.
   ...
   - **커서 아트 동기화 계약(#517 후속)**: 상태별 커서 텍스처는 전부 `Assets/Imported/@NorthLand/UI/Cursor/`(32×32, `Texture Type = Cursor`)에 있고 `Assets/Resources/ScriptableObjects/CursorSet.asset`이 GUID로 참조한다. 미동기 환경에서는 **모든 칸이 조용히 OS 기본 화살표로 폴백**하는데 `Hidden`(`Placing`·`SkillAiming`)은 **아트가 필요 없어 그대로 동작**하므로, 화면이 "타일 위에선 커서 없음 / 타일 밖에선 평범한 화살표"가 되어 **`Hidden`을 켰다가 되돌렸던 상태와 구분되지 않는다.** 그래서 뱅크 로드 직후 `CursorSet.WarnIfArtMissing()`이 「숨김이 아닌데 그림이 빈 칸」을 한 줄로 모아 1회 경고한다(`Hidden` 칸과 `Pressed` 빈 칸은 설계라 제외 — 동기화된 환경에서는 무음). WL-040이 #445에서 굳힌 「§4 계약 등재 + null 시 1회 경고」 형태를 그대로 따랐다.
   ...
   - **결과창 아트 동기화 계약(#538)**: 승리/패배 로고(`@NorthLand/UI/Result/Victory.png`·`Defeat.png`, Sprite(2D and UI) / `alphaIsTransparency` 켬 / 밉맵 끔)와 결과창 버튼 스프라이트(`@NorthLand/Prefabs/UI/Button_Yellow_01 1.Png`·`Button_White_01 1.Png`)가 전부 Imported 소속이고, `ResultUI.prefab`이 GUID로 참조한다. 실행·리뷰·빌드 전에 `NorthLand-Imported`를 **`f3b246ef6`(결과창 Victory/Defeat 로고 + 버튼 스프라이트) 이상**으로 동기화해야 하며, sparse checkout을 쓰면 `@NorthLand/UI/Result/**` · `@NorthLand/Prefabs/UI/**`를 반드시 포함한다. 커밋 순서는 **Imported 선행**이다(WL-040). ⚠ **미동기 증상이 조용하다** — 컴포넌트 참조는 살아 있고 스프라이트만 끊기므로 **연출은 정상 재생되고 로고 자리만 빈다**. 그래서 `ResultPanelAnimator.Awake`가 `logoImage.sprite == null`일 때 「Imported 동기화 확인」을 1회 경고한다(커서 아트 계약의 `WarnIfArtMissing`과 같은 형태). 버튼 하이라이트는 런타임에 버튼 스프라이트를 그대로 빌려 쓰므로(`UIButtonHoverEffect.AdoptTargetSprite`) 버튼이 비면 호버 연출도 함께 사라진다 — 별도 경고는 두지 않았다(로고 경고가 같은 원인을 이미 가리킨다).
   ...
   - **몬스터 공격 스윙 동기화 계약(#452)**: 잡몹의 공격 모션이 공격속도에 맞춰 재생되는 근거는 **코드가 아니라 배선**이다 — `MonsterAnimation.attackClip`(프리팹)과 애니메이터 컨트롤러의 `SwingCadence` Float 파라미터 + 공격 상태의 Speed Parameter 연결이 전부 Imported 소속이다. 실행·리뷰·빌드 전에 `NorthLand-Imported`를 **`7b0169504`(AttackCadence → SwingCadence 파라미터 이름 변경) 이상**으로 동기화해야 하며, sparse checkout을 쓰면 `@NorthLand/Animations/Monster/**` · `@NorthLand/Prefabs/Monster/**`를 반드시 포함한다. 커밋 순서는 **Imported 선행**이다(WL-040).
     - ⚠ **실패 모드가 「조용한 폴백」이다.** `attackClip`이 비면 `CanScheduleSwing == false`가 되어 **예전 거동(공격 클립 루프 재생 + 쿨다운 만료 프레임에 즉발 피해)으로 되돌아간다** — 기능이 죽는 것이 아니라 #452 이전으로 돌아가므로, 증상은 "머지됐는데 여전히 휘두르는 중간에 때린다" 하나이고 미동기와 회귀가 구별되지 않는다. 그래서 `MonsterAnimation.PlaySwing`이 **인스턴스당 1회** 경고한다(스윙마다 짖으면 웨이브 후반에 콘솔이 죽는다 — 합성 후보 칸 계약의 `HasMergeSlotWiring`과 같은 처방).
     - ⚠ 파라미터만 빠진 부분 동기화도 별도로 경고한다. `Animator`는 없는 이름에 대해 **매 호출** 경고를 남기므로(`EnemyAgent.cs:205`가 같은 이유로 같은 가드를 둔다) `SetFloat`을 무가드로 부르면 스윙마다 찍힌다 → `Awake`에서 파라미터 유무를 1회 캐시하고, 없으면 1회 경고 후 배속 보정만 건너뛴다(스윙 스케줄링은 계속 돈다).
     - **파라미터 이름이 `AttackCadence`가 아니다.** 그 이름은 `BossAttackCadence`(#235)가 쓰고 있고 같은 질문("간격 > 클립일 때")에 반대로 답한다 — 보스는 하한 0.2로 배속을 낮춰 클립을 늘리고, 잡몹은 하한 1로 잘라 원속도 1회 재생 후 Idle로 뜸을 들인다. `Tank.prefab`은 **두 컴포넌트를 루트에 함께 얹고 있고** `Tank.controller`에 `AttackCadence`도 있어서, 이름을 공유하면 그 프리팹의 `attackClip` 칸이 채워지는 순간 둘이 같은 float를 매 프레임 번갈아 쓴다(증상 "보스 공격 모션 속도가 떨린다", 원인은 어느 스크립트에도 안 보인다). 이름을 갈라 그 함정을 없앴다 — **정책 통일은 보스 모션을 실제로 손보는 시점의 일이고, 그때까지 두 이름은 갈라진 채로 둔다.**
     - **타격 시점은 프리팹이 소유한다.** `hitNormalizedTime`(클립 안에서 피해가 나가는 지점) · `maxCadence` · `attackExitBlend`가 `MonsterAnimation`의 직렬화 필드다 — 다른 전투 수치는 전부 `EnemyAsset`(SO)에 있지만 이 셋은 **클립의 성질**이라 클립 옆에 둔다(클립이 벤더 `.anim`이라 AnimationEvent를 심는 길은 CLAUDE.md의 Imported 편집 금지와 부딪힌다). ⚠ 그래서 **`AttackInterval`만 보고 밸런싱하면 계약이 깨진다** — 간격이 `클립 길이 / maxCadence` 아래로 내려가면 스윙이 간격을 넘겨 「모션 1주기 = 공격 1회」가 성립하지 않는다(그러미 클립 1.167s · 상한 2 기준 0.584s). 그 순간을 `PlaySwing`이 1회 경고로 드러낸다(WL-027).
     - `attackExitBlend`는 컨트롤러의 「공격 → Idle」 전이 duration과 **이중 저작**이다. 런타임에 시작하지 않은 전이의 duration을 읽는 API가 없어(`GetAnimatorTransitionInfo`는 진행 중인 것만 준다) 직렬화 필드로 뺐다 — 한쪽만 바꾸면 페이드아웃 중 loop 클립이 두 번째 스윙을 시작해 겹침이 조용히 재발한다.
     - **`IsAttack` = 「스윙 1회 진행 중」이고, 애니메이터 기록은 `MonsterAnimation` 한 컴포넌트가 소유한다.** 예전 의미는 「사거리 안」이었고 `MonsterStateMachine.EnterState(Attack)`이 래치했는데, 그러면 사거리 안에 있는 동안 공격 클립이 자기 길이대로 무한 반복해 공격 간격과 어긋난다(#452가 고친 것). ⚠ **그래서 `MonsterState.Attack`은 이제 「교전 중 = 이동 정지」만 의미하고 애니메이션 의미가 없다** — 열거형 이름이 그대로라 상태 진입에 모션을 다시 매다는 것이 자연스러워 보이지만, 그것이 곧 재발이다. `Enemy`(공격 간격을 아는 유일한 쪽)가 `MonsterStateMachine.RequestAttackSwing(interval)` / `CancelAttackSwing()` 창구로 지시하고, `MonsterAnimation` 참조 해석도 `MonsterStateMachine.Awake` 한 곳이다(`Enemy`는 직접 들지 않는다 — 해석 경로가 둘이면 저작이 어긋날 때 한쪽만 null이 되고 로그가 없다).
     - 대상 프리팹은 **Red / Yellow / Blue_Grummy · Flying_Bat** 넷이다. `Bomb_Grummy`(자폭병)는 `TryAttack`이 곧바로 `Detonate`로 빠져 스윙이 없으므로 `attackClip`을 배선하지 않는 것이 정상이고, 보스·미사용 몬스터(Shadow·Phantom)도 대상이 아니다 — 그래서 위 경고들은 「클립이 배선된 프리팹」에서만 난다(무배선 프리팹에서 짖지 않는다).
   ...
   - **머지 패널 "선택 행" 프리팹 동기화 계약(#535 → #554로 하한 상향)**: 행 프리팹의 정본은 `Assets/Imported/@NorthLand/Prefabs/UI/SelectedRow.prefab`이다(위 `TowerTargetSlot.prefab`과 같은 저장소·같은 이유). 합성 패널의 **선택 리스트**를 실행·리뷰·빌드하기 전에 `NorthLand-Imported`를 **`4e04599f1`(SelectedRow 마스커블 수정) 이상**으로 동기화해야 하며(#535 시점 하한은 `aebbd691e` 「SelectedRow에 아이콘 추가」였다), sparse checkout을 쓰면 `@NorthLand/Prefabs/UI/**`를 반드시 포함한다(바로 위 #445 계약과 같은 경로 패턴이라 둘 중 하나만 챙기면 자동으로 둘 다 들어온다). 커밋 순서는 **Imported 선행**이다(WL-040). 미동기 환경의 실패 모드가 **하한에 따라 셋으로 갈린다** — ⓐ 프리팹 자체가 없으면 `TowerMergePanelView`가 이름만 채우는 폴백으로 내려가고, ⓑ 컴포넌트는 있는데 필드가 비면 `TowerMergeSelectedRowView`가 무엇이 안 배선됐는지 알린다(ⓐⓑ 모두 **1회** 경고 — #445가 굳힌 형태). ⓒ **`aebbd691e`~`4e04599f1` 사이면 아이콘 `Image`의 `m_Maskable`이 `0`이라 아이콘이 ScrollRect 뷰포트 `Mask`에 잘리지 않고 스크롤 밖에서도 계속 보인다**(#554). ⚠ **ⓒ만 로그가 한 줄도 없다** — `MaskableGraphic`은 `maskable`이 꺼져 있으면 스텐실 깊이를 0으로 두고 스텐실 머티리얼을 아예 만들지 않으므로, 잘못 그려질 뿐 경고도 예외도 없다. 같은 행의 `TowerName`(TMP)은 `m_Maskable: 1`이라 정상적으로 잘려서 **"글씨만 잘리고 아이콘은 안 잘리는" 비대칭**으로만 드러난다. Tank·`hitPosition` 계약이 남긴 "컴파일도 콘솔도 조용하다" 사각지대와 같은 계통이고, 런타임 감지가 불가능하므로(마스킹 플래그는 조용히 잘못 그릴 뿐이다) **이 계약 등재가 유일한 방어다.**
   ...
   - **타워 선택 표시 서브트리 동기화 계약(#563)**: 배치 팔레트 칸의 선택 표시는 `Assets/Imported/@NorthLand/Prefabs/UI/TowerButton.prefab`의 `Slot/Img_Bg/Fx_Selected`(`RectMask2D` + `UIParticle`, 기본 비활성) + 그 아래 `@NorthLand/Particles/Fx_SunShines 1.prefab` + **마스킹 가능한 머티리얼 2본**(`@NorthLand/Particles/Materials/Fx_SunShines_UI.mat`·`Fx_Spark_UI.mat`, 셰이더 `UI/Additive`)으로 이뤄진다. 실행·리뷰·빌드 전에 `NorthLand-Imported`를 **`aee42246c`(버튼 파티클 추가 및 타워 패널 버튼 수정) 이상**으로 동기화해야 하며, sparse checkout을 쓰면 `@NorthLand/Prefabs/UI/**` · `@NorthLand/Particles/**`를 반드시 포함한다. 커밋 순서는 **Imported 선행**이다(WL-040). ⚠ **머티리얼을 새로 만든 것이 이 계약의 핵심이다** — 원본 `Sun_Shines_03`(`Mobile/Particles/Additive`)·`sparkle_yellow`(`Legacy Shaders/Particles/Additive (Soft)`)는 `_ClipRect`도 `_Stencil`도 선언하지 않아 `RectMask2D`를 걸어도 **에러 없이 마스킹만 무시된다**(#554 ⓒ와 같은 계통 — 마스킹은 조용히 잘못 그릴 뿐 경고도 예외도 없다). 원본 머티리얼로 되돌리면 파티클이 칸 밖으로 새어 나온다. 미동기 실패는 두 갈래이고 **둘 다 1회 경고로 닫혀 있다** — 서브트리 자체가 없으면 `TowerButtonView.EnsureSelectedEffectWired`, 파티클 프리팹만 없으면 `CacheSelectedParticles`가 알린다(#445가 굳힌 형태).
   ...
   - **타워 전투음 클립 동기화 계약(#540)**: 타워의 발사음·착탄음·빔 루프 클립 14본이 전부 `@NorthLand/Sound/Towers/**`(Imported) 소속이고, 타워 SO 16개가 GUID로 참조한다. 실행·리뷰·빌드 전에 `NorthLand-Imported`를 **`c2df705a4`(타워 발사음 및 착탄음 전용 파일 커밋) 이상**으로 동기화해야 하며, sparse checkout을 쓰면 `@NorthLand/Sound/Towers/**`를 반드시 포함한다. 커밋 순서는 **Imported 선행**이다(WL-040). ⚠ **미동기 증상이 완전히 조용하다** — `CombatSfx.Play`가 `clip == null`을 조용히 무시하도록 설계돼 있어(`CombatSfx.cs:19-21`, 미저작 타워가 에러 없이 무음으로 발사하는 것이 의도다) 콘솔이 한 줄도 짖지 않고, **"아직 소리를 안 붙인 상태"와 구별되지 않는다.** 그래서 `BootWarmup.WarmTowerAssets`가 `Resources.LoadAll<TowerAsset>` 전수에서 **`ProjectilePrefab`이 있는 타워 중 `FireSfx`가 단 하나도 없으면** 「Imported 동기화 확인」을 1회 경고한다(결과창 아트 계약의 `logoImage.sprite == null` 경고와 같은 형태). ⚠ **타워별이 아니라 전수 0건 검사여야 한다** — 오라·빔 타워는 `FireSfx`가 비어 있는 것이 정상이라 개별 검사는 오탐이 된다.
...
9. **튜토리얼 모드와 입력 게이트** (#488): 전체 튜토리얼 런의 단일 정본은 `TutorialMode.IsActive`다.
   ...
   `MonsterSpawnWaveProvider.forceTutorialWaves`는 웨이브 구성 전용 테스트 옵션이며 전체 튜토리얼 규칙으로
   승격하지 않는다. 선택 제한은 대상이 `ITutorialSelectionGate`로 행동을 공개하고, 타워 수에 따른 낮 종료
   갱신은 `TutorialInputGate.Changed`가 소유한다. MouseManager의 구체 타입 분기나 Management UI의
   `Tower.ActiveChanged` 직접 구독으로 이 계약을 우회하지 않는다. (Docs/Core/Tutorial.md §1.6·§4)
...
- **씬 싱글톤 수명 규약(#554에서 전수 정렬)**: `static Instance`를 가진 MonoBehaviour는 **`Awake`의 중복 가드와 `OnDestroy`의 해제를 쌍으로** 갖는다 — `Awake`는 `if (Instance != null && Instance != this) { Destroy(gameObject); return; } Instance = this;`, `OnDestroy`는 **`if (Instance == this) Instance = null;`**. `Instance == this` 조건이 규약의 일부다: 중복 가드로 **사본**이 자멸할 때(그 시점 `Instance`는 원본, `this`는 사본) 원본 등록을 지우면 안 된다.
  **실측 현황(2026-08-30, `DontDestroyOnLoad` 호출부 기준 전수)**: 싱글톤 23종 중 해제 보유 19 / 미보유 4. 두 축이 **직교하지 않으므로 한쪽으로 규칙을 세우지 말 것** — ⓐ **씬 싱글톤(비-`DontDestroyOnLoad`) 14종은 전부 해제한다**(#554에서 `DayNightManager`·`SkillManager`·`BuffSkillManager`·`SkillEffectManager` 4종을 나머지에 맞춰 정렬). ⓑ **`DontDestroyOnLoad`는 9종**(`MouseManager`·`KeyboardManager`·`AudioManager`·`ResidentDragCoordinator`·`ResidentSelectionCoordinator`·`GameSceneManager`·`CombatSfxPool`·`PlayerSaveService`·`GameSettingsService`)이고 **그중 5종은 해제까지 한다** — 즉 `DontDestroyOnLoad`는 해제 면제의 근거가 **아니다**. 씬 전환으로 죽지 않을 뿐 앱 종료·도메인 리로드 비활성 상태의 플레이 중지에서는 파괴되기 때문이다. ⓒ 해제 미보유 4종은 전부 `DontDestroyOnLoad`지만 그 역은 성립하지 않는다.
  ⚠ **새 싱글톤은 `DontDestroyOnLoad` 여부와 무관하게 해제한다.** 특히 `DontDestroyOnLoad`라는 이유로 "씬 전환에 안 죽으니 안 해도 된다"고 판단하지 말 것 — 반대 방향의 수명주기 오해(살아 있다고 착각/죽었다고 착각)가 둘 다 #554와 같은 종류의 버그를 만든다. `MouseManager`처럼 `DontDestroyOnLoad`인 오브젝트를 "씬과 함께 죽는다"고 적은 주석은 실제로 이 PR에서 한 번 나왔다가 리뷰에서 잡혔다.
  ⚠ **`Instance?.`는 파괴 가드가 아니다.** `?.`는 순수 참조 검사라 Unity의 `==` 오버로드를 타지 않고, **파괴된 오브젝트를 그대로 통과시켜** `MissingReferenceException`이 난다. 파괴 가능성이 있는 곳에서는 `if (X.Instance != null)`로 검사할 것(`Tower.OnDeselected`가 사거리 원에 대해 같은 함정을 기록해 두었다). 위 `OnDestroy` 해제가 있으면 `Instance`가 진짜 null이 되어 `?.`도 함께 성립하지만, **해제에만 의존하지 말고 두 겹으로 둔다** — #554가 실측한 실패 모드가 정확히 "해제가 없어서 `?.`가 뚫린" 경우였다.
  ⚠ **게터 널 정규화(`get => _instance != null ? _instance : null;`)로 대체하지 않는다.** 기제 자체는 성립하지만(파괴 시 진짜 null 반환) ⓐ 저장소의 싱글톤 19종이 이미 `OnDestroy` 해제 규약으로 통일돼 있어 두 번째 패턴을 들이면 "어느 쪽이 정본인가"가 갈리고(WL-145 계열), ⓑ 해제와 달리 **참조를 놓지 않아** 파괴된 매니저의 관리 객체가 도메인 리로드까지 살아남으며, ⓒ 직접 접근(`X.Instance.Foo()`)은 예외 종류만 `MissingReference`→`NullReference`로 바뀔 뿐 **여전히 던진다**(#554 리뷰에서 "직접 접근도 공짜로 닫힌다"는 제안이 나왔으나 `SelectableTest.cs`로 실측해 반증했다).
...
- **연출의 시간축 판단 기준(#138에서 정리, 규칙 확정은 WL-100 대기)**: **월드의 일부인가**로 가른다.
  ...
  플레이어에게 주는 안내·피드백(건물 업그레이드 연출, 줌 힌트 파티클, `TowerSpawnEffect`/`TowerDissolveEffect`, 배속 활성 표시 `SpeedBoostEffect`)은 `unscaledDeltaTime`·`Use Unscaled Time = on` —
...
- **밸런스 수치 정본(2026-08-28)**: 현재 적용하는 타워 화력·레벨·코스트·해금과 경영 경제 수치는
  [BalanceV1.md](../Balance/BalanceV1.md)가 정본이다. 실제 에셋과 같은 PR에서 함께 갱신한다.
  [CombatBalance.md](../Core/CombatBalance.md)와 [EconomyBalance.md](../Core/EconomyBalance.md)는
  계산 방법·검산식·과거 조정 이력을 보존한다. 타워 저작 절차는
  ...
  ② **합성 결과 타워의 1회 통과 킬 수 > 최강 재료 타워 킬 수 × 1.3** — 3단 레벨 체계에서
  합성 승급감을 보장하는 현재 검산 기준이다. 구판의 "재료 킬 수 합 초과, 상한 합 ×1.3" 규칙은
  재료가 2~3개인 레시피와 레벨 밴드를 동시에 만족시키지 못해 폐기했다.
  ③ 화력 단위는 DPS가 아니라 **1회 통과 킬 수**이고, 레벨별 밴드(0.1킬 = 1눈금)에서 고른다.
  ✅ **①은 `TowerAsset.OnValidate`가 강제한다**(경계 3종 모두 저장 시 경고. 18 = 타일 6유닛 × 계수 3).
  ⚠ **②·③은 코드가 잡지 못한다**(WL-169) — 최강 재료의 킬 수와 결과 레벨 밴드를 알아야 해서 단일 SO 검사로는
