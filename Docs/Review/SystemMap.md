...
- `RewardCardView.Bind(WaveRewardData reward, SkillEffectSnapshot snapshot, int faceIndex, Action<WaveRewardData> onSelect)`(#320, 스냅샷 인자 #353, 카드면 인덱스 #356) — 보상 카드
  ...
  **등급 표현은 색 틴트가 아니라 등급별 스킨 스프라이트 교체다(#356)** — `faceIndex`가 가리키는 **스킨 한 벌**
  ...
  **슬롯별로 Image가 미배선이거나 스프라이트가 null이면 그 슬롯만 건너뛴다**(`ApplySprite`) — 아트가 아직 안 온
  ...
  `DefaultLevelColors`·`ResolveTint`는 함께 제거됐다. 상한 클램프와 "스킨이 모자람" 경고는 **배열을 가진
  `RewardCardView.ApplySkin`**이 하고, `WaveRewardSelectionUI.ResolveFaceIndex`는 하한만 막는다.
