...
- `Sfx`(정적 클래스) — 주인 없는 공용 효과음의 유일한 호출 진입점: `ButtonClick()` / `PanelOpen()` / `TowerInstalled()` / `Rejected()` / `BuildingUpgraded()` / `ResidentIncreased()` / `Undone()` / `Redone()`(호출부 없음). **같은 큐는 한 프레임에 한 번만 재생된다** — 선택 표시 소유자가 둘이라 `OnSelected`가 한 클릭에 두 번 불리는 구조를 소리 쪽에서 흡수한다.
