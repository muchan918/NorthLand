...
| Tutorial (튜토리얼 진행·진입, #408·#479·#488) | muchan | `Assets/Scripts/Tutorial`, `Assets/Prefabs/Tutorial`, `Assets/Scenes/GameScene.unity` | 단계 SO 24개 + 진행 컨트롤러 + 오버레이 + 완료 조건 추상화(`[SerializeReference]`)가 정본 `GameScene`에서 동작한다. `TutorialMode`를 씬 로드 전에 켜 전용 웨이브·초기 자원·적 HP·스킬 쿨다운을 적용하며, 슬롯별 `PlayerData.tutorialCompleted`가 최초 새로하기 자동 진입을 결정한다. 팝업은 전체 화면 투명 패널로 다른 Canvas 입력을 막고, 행동 단계는 Flags 기반 `TutorialInputGate`를 UI와 실제 시스템 진입점 양쪽에서 검사한다. 완료·스킵은 모든 단계 규칙을 초기화하고 일반 `GameScene`을 재로드한다. 절차·24단계 설정 정본은 `Docs/Core/Tutorial.md` |
