...
| Tutorial (튜토리얼 진행·진입, #408·#479) | muchan | `Assets/Scripts/Tutorial`, `Assets/Prefabs/Tutorial`, `Assets/Scenes/GameScene.unity` | 단계 SO 22개 + 진행 컨트롤러 + 오버레이 + 완료 조건 추상화(`[SerializeReference]`)가 정본 `GameScene`에서 동작한다. `TutorialMode`를 씬 로드 전에 켜 전용 웨이브와 UI를 활성화하며, 슬롯별 `PlayerData.tutorialCompleted`가 최초 새로하기 자동 진입을 결정한다. 튜토리얼 맵은 Inspector의 고정 Tutorial Master Seed를 사용한다. 완료·스킵은 일반 `GameScene`을 재로드하고, 시드 지정 시작은 같은 마스터 시드를 본 게임에 다시 전달한다. `TutorialRelayUI` 다시 보기는 경고 확인 후 현재 슬롯의 기존 Run을 삭제하며 슬롯이 없으면 건너뛰고, 튜토리얼 중 낮 시작 자동 저장은 차단한다. 딤·강조·말풍선 구간 입력 차단은 후속 범위다. 절차·규칙 정본은 `Docs/Core/Tutorial.md` |
...
- 마스터 시드 결정 우선순위는 **Inspector 개발용 override → 튜토리얼 실행 중 Tutorial Master Seed → 타이틀에서 전달된 시드 → 새 무작위 시드**다.
...
- `TutorialController.IsRunning` / `TutorialMasterSeed` / `UsesTutorialSeed` / `StartTutorial()` / `StopTutorial()` — 진행 소유. 단계 순서는 인스펙터 `steps` 리스트의 **등록 순서가 전부**이며 단계 에셋은 자기 인덱스를 갖지 않는다(`MonsterWaveAsset`과 같은 규칙, #294). 튜토리얼 실행 중에는 Inspector의 Tutorial Master Seed를 제공한다. 완료·스킵 시 슬롯별 완료 상태를 저장하고 일반 `GameScene`으로 전환하며, 시드 지정 진입은 복귀 시드를 필드에 보관했다가 본 게임에 전달한다. 저장 실패는 로그로 남기되 단계 규칙 정리와 전환은 계속한다.
