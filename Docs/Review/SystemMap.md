...
| CursorFeedback (상태별 커서 그림) | n0wst4ndup | `Assets/Scripts/GameManager/MouseManager/Cursor`(`CursorController`/`CursorSet`/`ICursorHint`), `Assets/Resources/ScriptableObjects/CursorSet.asset` | 하드웨어 커서(`Cursor.SetCursor` + `CursorMode.Auto`)를 상호작용 상태에 따라 교체한다. `OutlineInteractionDriver`와 같은 계보 — MouseManager는 통지만 하고 그림 결정은 구독자가 한다. **MouseManager 신규 통지 2종**: `OnPointerPressedChanged`(좌버튼 `isPressed` **변화**만, UI·모드 무관한 순수 입력 통지 — `wasReleasedThisFrame`을 쓰면 포커스 상실 시 뗌이 유실돼 눌린 그림에 갇힌다, WL-143과 같은 논리)와 `OnModeChanged`+`CurrentMode`(`SetMode` 단독 발행, 동일 모드 재진입은 통지 안 함 — `CancelInteractions`가 `SetMode(Idle)`을 두 번 때린다). 대상별 커서는 **대상이 스스로 답한다**(`ICursorHint`, `IOutlineKindFilter`와 같은 확장 규칙): `BuildingTooltipSource`=Door · `TowerGroupSelectable`=Inspect · `ResidentSelectable`=Hand. ⚠ 세 구현 모두 `IHoverable`과 **같은 컴포넌트**여야 한다(`TryGetComponent`가 GameObject당 하나만 잡음). **상태는 두 축이다** — 종류(`CursorKind`, 우선순위 `CameraPan > UIButton > Placing/SkillAiming > ResidentDrag > 호버(Building·Tower·Resident) > Default`) × 눌림(불리언). 어휘가 그림(Door/Inspect/Hand)이 아니라 **대상 기준**인 것은 아트 파일명(`Building-Hover-32` 등)과 1:1로 맞추기 위해서다 — 도메인 이름이 들어와도 원칙 2는 유지된다(값을 고르는 것은 대상 자신이고, 컨트롤러가 분기하는 것은 모드 3종뿐). 눌림이 별도 종류가 **아닌** 이유: "건물 위에서 누르면 열린 문"처럼 눌렸을 때의 그림이 무엇을 가리키느냐에 따라 다르므로, 전역 눌림 1장으로는 표현되지 않는다. 그래서 뱅크의 칸마다 `Normal`/`Pressed` 두 장을 갖고, `Pressed`가 비면 `Normal`로 폴백한다(=그 종류는 눌림 연출 없음). 텍스처는 자가 부팅 컨트롤러라 인스펙터 대신 Resources SO가 든다(`SfxBank` 선례). ⚠ 하드웨어 커서 제약: **애니메이션·파티클 부착·스케일 불가**, 텍스처는 `Texture Type = Cursor` 필수(아니면 `"not CPU accessible"`로 실패 → 컨트롤러가 사전 검사해 경고). **칸별 `Hidden`을 켜면 그 상태에서 커서를 감춘다**(`Cursor.visible=false`) — ⚠ **현재 켜진 칸은 없다**. `Placing`·`SkillAiming`에 켜려다 껐다(2026-08-27): 고스트·인디케이터가 **커서가 타일 위일 때만** 그려져, 타일 밖에서는 대신할 것도 없이 커서까지 사라져 화면에 아무것도 안 남는다. 필드와 아래 UI 예외는 휴면 상태로 살려 뒀다. ⚠ 단 (다시 켤 경우) **커서가 UI 위면 되살린다** — 배치 중에도 취소 버튼을 겨냥해야 하고, 고스트는 `overUI`면 갱신을 멈춰 대신할 것이 없다. 이 판정(`EventSystem.IsPointerOverGameObject()`)만 `LateUpdate`에서 1회/프레임 폴링하며, **`_overUI`가 쓰이는 곳은 이 예외 하나뿐이다**. 숨기는 종류여도 **그림은 계속 세팅한다** — 안 그러면 배치 직전 그림(문·돋보기)이 남아 UI 위에서 뜬다(숨김 칸은 비어 있어 `Default`로 폴백된다). ⚠ **"UI 위"가 아니라 "누를 수 있는 UI 위"에서만 그림이 바뀐다**(`UIButton`) — "UI에 올라가면 무조건 바뀜"은 좌우 한 쌍의 손 그림까지 넣었다가 "오히려 헷갈린다"로 걷어냈다(2026-08-27, `UI-Left-32`/`UI-Right-32` 삭제됨). **그 형태로 되살리지 말 것**. 판정은 `EventSystem.RaycastAll` 맨 앞 히트의 `Selectable`+`interactable`이며, 이것만 새 레이캐스트라 `IsPointerOverGameObject()`가 참일 때만 돈다(버퍼 재사용, 무할당). `CameraPan`은 **우버튼을 직접 폴링하지 않고** `CameraController2.IsDragging`(이번에 공개)을 읽는다 — 직접 읽으면 배치·조준 취소용 우클릭까지 팬으로 오인하고 "UI 위에서 시작한 우클릭은 팬이 아니다"라는 카메라 규칙을 재구현하게 된다. 같은 맥락으로 `CameraController2.Update`의 `Result != Playing` 조기 반환에 `_isDragging=false`를 추가했다(패널 분기와 달리 빠져 있어, 우버튼을 쥔 채 판이 끝나면 커서가 '쥔 손'으로 굳었다). `Cursor.visible`은 전역 값이라 **표시/숨김은 이 컨트롤러가 단독 소유**한다는 전제다. `CameraController2`는 씬 소유라 못 찾으면 **120프레임 백오프** 후 재탐색한다(`ResidentDragCoordinator`와 같은 값·같은 이유 — 이 컨트롤러는 `DontDestroyOnLoad`라 카메라가 없는 씬에도 상주한다, WL-002). 현재 배선: `Default`(Default-32/Default-Click-32, 손끝 14,5)·`UIButton`(Default-Hover-32, 13,2)·`CameraPan`(Default-Grab-32, 13,2)·`Resident`(Default-Hover-32, 13,2)·`Building`(Building-Hover-32/Building-Click-32, 핫스팟 16,16 — 두 그림 알파 bbox가 같아 눌러도 안 튐)·`Tower`(Tower-Hover-32, 12,12 = 렌즈 중심)·`ResidentDrag`(Resident-Drag-32, 16,8)·`Placing`(Tower-Placing-32, 0,0)·`SkillAiming`(**비어 있음** → `Default`로 폴백). ⚠ 미해결 3종은 `CursorFeedback.md` §6에 있다: `Placing` 핫스팟 (0,0)이 알파 bbox(3–28) 밖 / `CameraPan` 핫스팟 (13,2)가 주먹 실루엣(y 5–27) 밖 — 손 그림 값이 딸려 온 것으로 보임 / `Resident`와 `UIButton`이 같은 그림이라 주민 위와 버튼 위가 구분되지 않음. 아트는 `Assets/Imported/@NorthLand/UI/Cursor`(32×32, 파일명 `대상-상태-크기`, 별도 저장소) · 클릭 파티클 보류. 정본 `Docs/Core/CursorFeedback.md` |
