...
| TowerVfx (타워 등장 연출, #264)             | n0wst4ndup | `Assets/Scripts/CombatSystem/Vfx` | 배치 확정 시 입자 수렴 → 타워 등장 팝 → 바닥 링. **대상을 모르는 범용 연출** — `Transform` + 풋프린트 크기만 받고 `Renderer.bounds`/`localScale`만 읽어, 타워 에셋이 교체돼도 코드 수정이 필요 없다(에셋 교체 회귀 검증 통과). 수치 앵커는 둘로 나뉜다: **크기·바닥 링은 풋프린트(논리, 에셋 무관 불변) / 개수·구름 모양은 bounds(시각)**. ⚠ **재생 중 대상 루트 `localScale`을 배타적으로 소유**하므로 그 창(약 0.45초 + 과도기 0.28초) 동안 스케일을 쓰거나 캡처하는 시스템이 있으면 깨진다 — 이 계약 때문에 `RangeCircle`의 보정 시점이 함께 바뀌었다(2절). 시간은 `unscaledDeltaTime`(일시정지 중 타워가 투명하게 멈추는 것 방지, WL-100). **룩·수치는 전부 임시(아트 TBD)** — 타워 에셋이 임시이고 아트 방향이 미정이라, 검증 통과와 설계 확정은 별개다. #265 합성 소모 연출이 마지막 구간으로 재사용한다. 명세 `Docs/Core/TowerPlacement.md` §9.3 |
...
- `TowerSpawnEffect.PlayAsync(Transform, float, CancellationToken)`(`NorthLand.Combat`, #264) — 위와 동일하되 종료까지 대기. #265 합성 시퀀스의 마지막 구간
...
| TowerVfx ↔ MouseManager / TowerFusion    | 등장 연출(`TowerSpawnEffect`)의 **스케일 배타 소유 계약**(`TowerPlacement.md` §9.3.2). `TowerPlacer.PlaceTower`가 확정 말미에 무조건 `Play`를 부르므로, 합성(#265)이 결과 타워에 `PlayAsync`를 **또** 걸면 같은 대상에 두 번 재생된다 — 진입점 레지스트리가 인계 처리하지만, 의도가 "이중 재생"이 아니라면 합성 경로에서 `TowerPlacer` 쪽 호출을 억제하는 편이 맞다. 드래그 선택은 콜라이더가 아니라 위치 기반이라 **연출 중(스케일 0)에도 대상에 도달한다** |
