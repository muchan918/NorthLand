...
- **씬 싱글톤 수명 규약(#554에서 전수 정렬)**: `static Instance`를 가진 MonoBehaviour는 **`Awake`의 중복 가드와 `OnDestroy`의 해제를 쌍으로** 갖는다 — `Awake`는 `if (Instance != null && Instance != this) { Destroy(gameObject); return; } Instance = this;`, `OnDestroy`는 **`if (Instance == this) Instance = null;`**. `Instance == this` 조건이 규약의 일부다: 중복 가드로 **사본**이 자멸할 때(그 시점 `Instance`는 원본, `this`는 사본) 원본 등록을 지우면 안 된다. **현재 씬 싱글톤 19종 전부가 이 형태다**(#554에서 `MouseManager`·`SkillManager`·`BuffSkillManager`·`SkillEffectManager`·`DayNightManager` 5종을 나머지 14종에 맞춰 정렬했다). **예외는 정확히 `DontDestroyOnLoad` 4종**(`GameSceneManager`·`CombatSfxPool`·`PlayerSaveService`·`GameSettingsService`)이며, 이들은 씬 전환으로 죽지 않고 앱 종료 때만 파괴되므로 해제 대상이 아니다 — **새 싱글톤이 이 4종에 속하지 않는다면 해제는 필수다.**
