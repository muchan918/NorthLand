...
- `IDisabledClickFeedback.OnDisabledClick()` — #550. **비활성(회색) 버튼이 눌린 프레임에 `UiClickSfx`가 호출한다.** `Button.interactable == false`면 `onClick`이 아예 발화하지 않고 위 전역 훅도 `IsInteractable()`에서 빠지므로, **회색 버튼은 어느 소리 경로도 지나지 않는다** — 눌러도 아무 일도 안 일어난 것처럼 보인다. 무슨 소리를 낼지는 **버튼 자신이 판단한다**(`ICursorHint`와 같은 계보 — 전역 훅이 "이건 스킬 버튼인가"를 분기하기 시작하면 버튼 종류마다 조건이 하나씩 박힌다). 구현하지 않은 버튼은 조용히 넘어가고, 배선은 0이다. ⚠ **타입 필터(`Button`/`Toggle`)와 `UiClickSfxIgnore` 제외를 지나지 않는다** — 공용 클릭음이 아니라 버튼 자기 소리이므로 자기 규칙으로 낸다(`TowerUndoButtonView`가 `ApplyTo`로 클릭음을 뺀 상태에서도 이 피드백은 살아 있다).
  ...
  - 현재 구현체 **2개**: `SkillButtonView`(충전 소진 → `Sfx.SkillOutOfCharges`, 연타 흡수용 0.7초 게이트) · `TowerUndoButtonView`(되돌릴 것 없음 → `Sfx.Rejected`). 후자는 **아래 `CommandHistory.Undo` 항목이 계약으로 실은 「되돌릴 것 없음 → 거절음」이 버튼 경로에서 도달 불가였던 것**을 메운다(Ctrl+Z로만 들렸다).
