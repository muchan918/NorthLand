...
| KeyboardManager (단축키 입력)               | n0wst4ndup | `Assets/Scripts/GameManager/KeyboardManager`                         | 단축키 입력의 단일 창구(#444). `MouseManager`의 형제지만 **아는 것이 없다** — 기능 쪽이 `KeyboardManager.Bind(Key, KeyModifier, handler, label)`로 자기 단축키를 등록하고 매니저는 눌림만 감지해 부른다(단축키가 늘어도 이 파일은 안 바뀐다). 바인딩 목록은 static이고 펌프(MonoBehaviour)는 `AfterSceneLoad` 자가 부팅 + `DontDestroyOnLoad`라 **씬 배선이 없다**(`ResidentDragCoordinator` 계보). 보조키는 **정확히 일치** 판정(Ctrl+Shift+Z가 Ctrl+Z를 겸하지 않는다), 텍스트 입력 중(`TMP_InputField`/`InputField` 포커스)에는 삼킨다. 현재 등록: **Ctrl+Z → `UndoRequest.Submit` / Q → `SkillButtonView.HandleShortcut`(감전 스킬 조준)**. ⚠ Q도 `KeyModifier.None`의 정확 일치 판정을 받으므로 Shift·Ctrl·Alt 중 하나라도 누른 상태에서는 발화하지 않는다(WL-201) |
