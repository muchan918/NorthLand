...
| Tutorial (튜토리얼 진행·진입, #408·#479·#488·#490·#526) | muchan | `Assets/Scripts/Tutorial`, `Assets/Prefabs/Tutorial`, `Assets/Scenes/GameScene.unity` | 단계 SO 25개 + 진행 컨트롤러 + 오버레이 + 완료 조건 추상화(`[SerializeReference]`)가 정본 `GameScene`에서 동작한다. 전체 튜토리얼 여부의 단일 정본은 `TutorialMode.IsActive`다. 정식 진입은 씬 로드 전에, 작업 씬 직접 실행은 우선 실행되는 `TutorialController.Awake`에서 활성화해 전용 웨이브·초기 자원·적 HP·스킬 쿨다운을 함께 적용한다. `MonsterSpawnWaveProvider.forceTutorialWaves`는 웨이브 구성만 바꾸는 별도 테스트 옵션이다. 슬롯별 `PlayerData.tutorialCompleted`가 최초 새로하기 자동 진입을 결정한다. 팝업은 전체 화면 투명 패널로 다른 Canvas 입력을 막고, 행동 단계는 Flags 기반 `TutorialInputGate`를 UI와 실제 시스템 진입점 양쪽에서 검사한다. 말풍선은 문장 길이에 맞춰 한 번만 크기를 계산하며 텍스트 안전 영역은 프리팹의 `BubbleText` Anchor가 정본이다. `HideDuringTutorial`은 도감 버튼을 숨기고 도감 공개 API도 `TutorialMode.IsActive`를 재검사한다. 맵 재구성 시 `MonsterSpawn`은 기존 `PlayerBase.Instance`를 재사용해 본진 싱글톤과 튜토리얼 무적 구독을 유지한다. 완료 통지 단계는 입력을 막고 실제 시간 3초 뒤 일반 `GameScene`으로 전환하며, 완료·스킵은 모든 단계 규칙을 초기화한다. 절차·25단계 설정 정본은 `Docs/Core/Tutorial.md` |
...
- `Sfx`(정적 클래스) — 주인 없는 공용 효과음의 유일한 호출 진입점: `ButtonClick()` / `PanelOpen()` / `TowerInstalled()` / `Rejected()` / `BuildingUpgraded()` / `ResidentIncreased()` / `Undone()` / `Redone()`(호출부 없음). ⚠ **건물 사건(업그레이드·주민 증축)의 성공음은 호출부가 아니라 `ManagementController.OnBuildingAction` 구독(`InGameCue`)에서 난다** — 같은 사건의 파티클(`BuildingFeedback`)이 이미 그 이벤트를 쓰므로 트리거를 한 벌로 모은다. 실패는 이벤트로 오지 않아 거절음만 호출부 몫이다(WL-208). **같은 큐는 한 프레임에 한 번만 재생된다** — 선택 표시 소유자가 둘이라 `OnSelected`가 한 클릭에 두 번 불리는 구조를 소리 쪽에서 흡수한다.
...
- `TutorialOverlay.ShowPopup(string, string, Sprite)` / `ShowBubble(string)` / `HidePopup()` / `HideBubble()` / `HideAll()` / `PopupConfirmed` / `SkipRequested` — 표시 전용. 단계를 모르며, 확인·스킵 클릭을 이벤트로만 알린다. 팝업과 별도인 전체 화면 투명 `PopupInputBlocker`가 다른 Canvas 입력을 막고, 말풍선 루트는 `raycastTarget`을 꺼 뒤쪽 월드 클릭을 살린다.
