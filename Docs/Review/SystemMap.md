...
- `IReversibleCommand { bool Execute(); void Commit(); void Undo(); }`(#263) — 임시 실행 → 확정 또는 원복. 일반 Command의 2단이 아닌 이유는 "확정되어 더는 되돌릴 수 없음"을 표현해야 하기 때문(트랜잭션에 가깝다). **진행 중 커맨드는 항상 최대 1개**(배치 세션 1개 + `keepPlacing=0`)라 전역 Undo 스택을 두지 않는다
...
- `TowerFootprint.Release()` / `Reoccupy()`(#263) — 등록 목록은 유지한 채 `BattleTile.Occupied`만 임시 해제/복원. **`OnDestroy`는 `Release` 이후 타일을 건드리지 않는다** — 합성이 재료가 있던 자리에 결과를 놓으면 그 타일은 이미 결과의 것이라, 무조건 해제하면 결과가 서 있는 칸이 빈 칸으로 표시돼 그 위에 또 배치된다
