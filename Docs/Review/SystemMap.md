...
| Tutorial (튜토리얼 진행·진입, #408·#479·#488·#490·#526·#545) | muchan | `Assets/Scripts/Tutorial`, `Assets/Prefabs/Tutorial`, `Assets/Scenes/GameScene.unity` | `TutorialController.Steps`의 35개 등록 단계 + 진행 컨트롤러 + 오버레이 + 완료 조건 추상화(`[SerializeReference]`)가 정본 `GameScene`에서 동작한다. #545에서 시작 인트로·단계 사이 연출 지연·전투 공간 소개와 `CameraFocusCondition` 기반 강제 카메라 이동을 추가했다. 전체 튜토리얼 여부의 단일 정본은 `TutorialMode.IsActive`이며, 정식 진입은 씬 로드 전에 활성화해 전용 웨이브·초기 자원·적 HP·스킬 쿨다운을 함께 적용한다. 팝업은 전체 화면 투명 패널로 다른 Canvas 입력을 막고, 행동 단계는 Flags 기반 `TutorialInputGate`를 UI와 실제 시스템 진입점 양쪽에서 검사한다. 카메라 연출은 사용자 입력과 게임 시간을 막은 상태에서도 `unscaledDeltaTime`으로 이동한 뒤 다음 팝업으로 진행한다. 말풍선은 문장 길이에 맞춰 크기를 계산하고, Bubble·Popup 최초 표시에는 `SfxBank`의 전용 안내음을 1회 재생한다. 슬롯별 완료 상태·다시 보기 Run 삭제·도감 차단·본진 재사용·완료/스킵 규칙은 기존 계약을 유지한다. 절차·35개 등록 단계 설정 정본은 `Docs/Core/Tutorial.md` |
