...
| Management(Resource) ↔ PlayerSkill(마법 연구소) | **저결합 단방향 읽기(계약)**: 스킬 강화는 `ManagementController.GetUpgradeLevel(magicLabAsset)`으로 **레벨(int)만** 읽어 스킬 측이 효과를 정한다(레벨→수치 매핑=스킬 소유). 컨트롤러는 "스킬"을 모르고 `_upgradeBuildings`에 마법 연구소 SO만 갖는다. 레벨 변경은 `OnChanged`로 통지 → 스킬은 구독 후 재-pull. **현재 강화 효과는 미구현(TODO)** — 업그레이드 시스템(마나석 비용·레벨 추적)만 존재. BuildingUpgrade.md §8 |
