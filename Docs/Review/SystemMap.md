...
| MonsterHealthBar (몬스터 체력바 · HP 절대량 눈금, #447) | n0wst4ndup | `Assets/Scripts/UI/HealthUI`, `Assets/Resources/UI/MonsterHealthBar.prefab` | 바 **폭은 전 몬스터 공통 고정**이고 HP가 많을수록 눈금이 촘촘해진다(LoL 방식) — 얇은 줄 100 HP · 굵은 줄 1000 HP. 칸당 HP가 공통이라 **남은 칸 수 = `CurrentHp` ÷ 단위**가 되고, `CurrentHp` 절댓값으로 판정하는 「체력 높은/낮은 적」 조준 정책과 화면이 어긋나던 문제가 해소된다(빌드 6 노트 :137). **보스도 같은 바를 쓴다** — 굵은 줄 덕에 `tank` 2600도 같은 폭에 들어가므로 보스 전용 HP UI를 만들지 않는다(`BossDesign.md` 확정). 단위 계산의 기준은 SO 저작값이 아니라 **실효 HP**(= SO × 웨이브 배율, 보스는 배율 제외)다. 바는 몬스터 프리팹에 **없다** — `Enemy.Spawned`(static)를 받아 씬 배선 없는 `MonsterHealthBarLayer`가 런타임에 붙이고, 살아 있는 전 몬스터가 **World Space Canvas 하나**를 공유해 드로우콜 합계 1이 된다(마리당 캔버스는 배칭이 안 된다). 부착 높이는 렌더러 바운즈 상단에서 스폰 시 1회 계산하므로 프리팹에 앵커도 두지 않는다(WL-055 해소 — 손튜닝 스케일 오버라이드 0건, 제거 전 월드 폭 실측 편차 3.3배). 눈금은 텍스처가 아니라 `Graphic.OnPopulateMesh` 메시라 칸 수와 무관하게 선 두께가 고정이다. 명세 `Docs/Core/MonsterHealthBar.md` |
