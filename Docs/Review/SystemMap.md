...
| KeyboardManager (단축키 입력)               | n0wst4ndup | `Assets/Scripts/GameManager/KeyboardManager`                         | 단축키 입력의 단일 창구(#444). `MouseManager`의 형제지만 **아는 것이 없다** — 기능 쪽이 `KeyboardManager.Bind(Key, KeyModifier, handler, label, exactModifiers)`로 자기 단축키를 등록하고 매니저는 눌림만 감지해 부른다(단축키가 늘어도 이 파일은 안 바뀐다). 바인딩 목록은 static이고 펌프(MonoBehaviour)는 `AfterSceneLoad` 자가 부팅 + `DontDestroyOnLoad`라 **씬 배선이 없다**(`ResidentDragCoordinator` 계보). 텍스트 입력 중(`TMP_InputField`/`InputField` 포커스)에는 삼킨다. **보조키 판정은 바인딩별로 고른다(#537, WL-201 해소)**: 기본 `exactModifiers: true`는 정확히 일치라 Ctrl+Shift+Z가 Ctrl+Z를 겸하지 않고, `false`는 "요구 보조키 포함, 나머지 무시"다 — **무보조키 단축키는 후자를 써야 한다.** 이 게임에서 Shift는 그룹 선택으로 쥔 채 조작하는 키라 정확 일치로 등록하면 "가끔 안 먹는" 단축키가 된다. ⚠ **인스턴스 메서드를 등록하면 `Unbind`는 호출부 책임이다**(목록이 static이라 파괴된 오브젝트를 붙든 채 남는다) — 소유자 기반 자동 해제는 미구현(WL-199 PARTIAL). 현재 등록: **Ctrl+Z → `UndoRequest.Submit`**(static, 앱 수명, 정확 일치), **Space → `GameSpeedController.ToggleSpeed`**(인스턴스, `OnEnable`/`OnDisable` 대칭 + `exactModifiers: false`), **Q → `SkillButtonView.HandleShortcut`**(#501 감전 스킬 조준, 인스턴스, `Awake`/`OnDestroy` 대칭), **Digit1~9 → `BuildingShortcutBar.FocusEntry`**(항목 개수만큼, 인스턴스, `OnDestroy` 대칭). ⚠ **뒤 둘은 아직 기본값(정확 일치)으로 등록돼 있어 Shift·Ctrl·Alt 중 하나라도 쥐면 발화하지 않는다** — 둘 다 무보조키 바인딩이므로 `exactModifiers: false`가 맞고, WL-201 아카이브가 "재발감지"로 예고한 바로 그 지점이다(#537 ↔ #501 병합의 잔여) |
