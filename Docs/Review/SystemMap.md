...
- ⚠️ **#274 이름 변경 — 위 표(§1)와 아래 일부 서술에 남은 구 이름은 전부 삭제된 것들이다.**
  ...
  능력 질의는 `Has<IAttackAction>()`, 프리뷰 반경은 `TowerAsset.PreviewRadius`(구 `MagicRadius`).
  (§1 표 행은 4천 자짜리 단일 줄이라 고치면 diff를 읽을 수 없어 그대로 뒀다 — 상세는 `Tower.md` §3)
- ⚠️ **#252 이름 변경** — `HitscanAttackBehaviour`→`HitscanAttackAction`, `IAttackBehaviour`→`IAttackAction`.
  #252(체인 히트스캔)와 #274(액션 부품화)가 같은 파일들을 각자 고쳤고, 병합 시 #252의 결정을 #274 구조 위로
  이식했다. **결정 자체는 하나도 뒤집히지 않았다**(체인=히트스캔, 능력 판정=인터페이스, 체인 CC/DoT 미지원).
  구조상 바뀐 것: `ProjectileImpact.MakeChain`/`ImpactKind.Chain` **삭제**(투사체가 체인을 배달하지 않는다) ·
  체인 수치가 `ChainFields` 래퍼 → `TowerAsset` 최상위 평탄 필드(`ChainRadius`/`MaxChainTargets`/
  `ChainDamageFalloff`/`BeamPrefab`) · 체인 저작 경고가 구 `TowerAssetEditor` → `TowerAsset.OnValidate`
...
- `Tower.Has<T>()` / `Tower.Get<T>()`(`where T : class`) — **능력 질의 창구.** 소비처가 타워의 구상 타입이
  아니라 능력을 묻게 한다. 예: 보스 P3 마력 봉인 대상 = `Has<IAttackAction>()`(`EnemyNodeQuery.IsAttackTower`).
  제약이 `TowerAction`이 아니라 `class`인 것은 **능력 인터페이스로 물을 수 있어야** 하기 때문이다(#252).
  ⚠️ **구상 액션(`Has<AttackAction>()`)으로 능력을 묻지 말 것** — 전달 방식을 하나 늘릴 때마다 판정하는 모든 곳이
  조용히 그 타워를 제외한다. 컴파일 에러가 아니라 "버프가 안 걸리는 타워"·"공격 스탯이 안 뜨는 툴팁"으로
  나타나 발견이 늦다. 판정 기준:
  · **"이 타워는 공격하는가"** → `Has<IAttackAction>()`. 전달 방식과 무관해야 하는 판정(버프 오라 대상
  `BuffAuraAction`, 보스 봉인 `EnemyNodeQuery.IsAttackTower`, `Tower`의 `IAttacker` 4개, 배치 전 툴팁
  `TowerTooltipView`)은 전부 이쪽이다.
  · **"이 타워는 투사체를 쏘는가"** → `Has<AttackAction>()`. 전달 방식을 **일부러** 구분하는 판정.
  실제 사례는 `BurnBuff` 하나 — 체인은 번개 테마상 발화가 성립하지 않고 1발에 최대 `MaxChainTargets`명을
  때려 화상 처리량이 튜닝을 무너뜨리므로 **의도적으로** 구상 액션으로 거른다(근거 주석 필수)
- `IAttackAction`(`NorthLand.Combat`, #252) — **공격 능력을 가진 액션의 공통 계약**
  (`Damage`/`Range`/`Interval`/`TryAttack`). 전달 방식별 구현: `AttackAction`(투사체) / `HitscanAttackAction`(빔).
  **`TowerAction`을 상속하지 않는 순수 인터페이스다** — 구현체가 이미 `TowerAction`을 상속하고 있어
  클래스를 하나 더 끼울 수 없고, 공통 기반 클래스를 두면 액션 하나를 읽을 때 부모까지 따라가야 한다
- `HitscanAttackAction`(`NorthLand.Combat`, #252) — **즉시 판정(히트스캔) 공격 액션.** 체인 타워가 쓴다.
  발사와 명중이 같은 프레임에 끝나므로 "비행 중 첫 대상 사망" 창이 없다 — 투사체 체인이 그 창에서 홉 전체를
  잃던 경로(Homing 조기 파괴 / Ballistic 착탄 시 생존 검사)가 **구조적으로** 사라진다.
  대상 탐색·쿨다운·스탯 골격은 `AttackAction`과 같고 `TryAttack`만 다르다(공통 기반 클래스 없음 —
  BuffAura/DebuffAura가 독립 sealed인 것과 같은 관례. 공유하는 것은 능력 계약뿐).
  체인 여부의 정본은 `Impact`가 **아니라 프리팹의 이 액션 유무**다(#274의 "종류의 정본은 프리팹"과 같은 축).
  ⚠️ **명중 효과(`TowerAsset.Effects`)를 걸지 않는다** — 체인 CC/DoT 미지원은 #252의 확정 결정이고,
  지원하려면 `ChainResolver.Resolve`에 효과 축을 더하는 **기획 결정이 선행**한다.
  #274 Phase 4가 "세 명중 경로가 전부 `Projectile.Hit`을 지난다"로 닫은 표면 중 체인만 밖에 있으므로,
  그 어긋남이 조용해지지 않도록 `TowerAsset.OnValidate`가 저작 시점에 경고한다(`Effects`가 비어 있지 않으면)
- `ChainResolver`(`NorthLand.Combat`, #252) — **체인 명중 규칙의 단일 출처.** 최초 대상에서 인근 적으로
  튕기며 홉마다 `×ChainDamageFalloff`. 전달 방식을 모르는 **명중 축**이라 재사용 가능하고,
  `outPath`로 홉 좌표를 돌려줘 빔 연출이 경로를 다시 계산하지 않는다("선은 3명을 잇는데 데미지는 4명" 방지).
  ⚠️ **static이 아니라 타워별 인스턴스다** — 순회 중 `Projectile.DamageDealt`(public static, 구독자 확장 중)
  구독자가 같은 콜스택에서 다른 타워를 발사시킬 수 있고, 상태가 static이면 그 중첩 호출이 진행 중인 순회의
  `hitSet`을 비워 이미 때린 적을 다시 고른다 — 에러 없이 데미지 분포만 틀어져 재현이 어렵다.
  같은 이유로 **명중 통지는 순회 밖에서 일괄 발행**한다(인덱스 순회 + 사후 `Clear`로 재진입 안전)
- `ChainBeamVisual`(`NorthLand.Combat`, #252) — 홉 경로를 잇는 짧은 수명 빔. **순수 사후 표시**라
  데미지·명중과 어떤 인과도 없다. 좌표를 스냅샷으로 고정하고 대상을 추적하지 않는다(홉 대상이 연출 도중
  죽어도 파괴된 `Transform`을 역참조하지 않는다). `TowerAsset.BeamPrefab`이 있으면 아트 외형을 쓰고
  **코드가 색·폭을 덮지 않는다**(아트가 gradient로 페이드를 저작할 수 있어야 함). 없으면 최소 `LineRenderer`를
  런타임 생성 — 그 경로에서만 생성한 `Material`을 `OnDestroy`에서 파괴한다(`RangeCircle`의 PR#115 지적과 같은 사유).
  정식 빔 스펙(수명 0.15s·색)은 아트 협의 대상인 **임시값**이다(#252 열린 결정)
...
- `TowerAction`(`NorthLand.Combat`, #274) — 타워가 하는 일 한 조각. **MonoBehaviour가 아니라 프리팹에
  ...
  구현 4종: `AttackAction`(투사체 — Single/Area의 차이는 `ProjectileImpact` 전략뿐) ·
  `HitscanAttackAction`(빔, 체인 — #252) · `BuffAuraAction`(이벤트 구동, Always) ·
  `DebuffAuraAction`(Tick 폴링, NightOnly). 앞의 둘은 능력 계약 `IAttackAction`을 공유한다.
