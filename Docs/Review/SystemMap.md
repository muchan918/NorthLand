...
| Management(Resource) (자원 지갑·생산처)     | n0wst4ndup | `Assets/Scripts/ManagementSpace`                              | 지갑·생산처(#42) + 경영 패널 UI·DayNightManager 낮/밤 루프 연동(#43, #66). 정산+주민 배치 초기화=OnNightToDay(정산 먼저). **밤→낮 전환은 이제 밤 전용 임시 UI(`NightActionPanelView`)의 "웨이브 성공" 버튼이 트리거(WL-018)** — 경영 패널(`RequestAdvancePhase`)은 낮→밤(`EndDay`)만 담당. 주민 수는 placeholder(주민 시스템 부재). 소비처·마나석 생산 후속. **🔀 방향 전환 예고(GDD v0.3, #139)**: ①확장 자원 라인(영토 해금, 식량 소모 생산)으로 라인 목록 동적화 + 정산 시 식량 차감(WL-021 경로), ②생산 건물 3종 **업그레이드**(주민당량 증가, 라인별 상태로 분화 — WL-016/WL-021, BuildingUpgrade.md). 이번 이슈는 방향 문서화 + 업그레이드 구현 |
...
3. **자원 흐름** (GDD §3.2): 기본 자원(나무/철/식량) = 주민 배치 생산 **또는 영토 확장 보상**, 마나석 =
   영토 확장·전투 보상에서만.
   - **방향 전환(GDD v0.3)**: **미개척 영지 자원**(영토 해금)은 **식량만 소모해서** 생산하는 **의도된 변환
     경로**다(주민 배치 없음) — '식량 → 확장 자원' 변환은 계약 위반이 아니라 계약의 일부(WL-042 참고). 그 밖의
     우회 경로(마나석→기본 자원 교환 건물 등)는 여전히 금지/미결.
