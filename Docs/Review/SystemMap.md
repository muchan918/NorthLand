...
- `CombatSfx.Play(AudioClip, Vector3, bool loop = false, float volumeScale = 1, CombatSfxPriority priority = Normal)` — #522 전투 위치 SFX 공용 진입점. 16개 사전 생성·최대 32개 보이스 풀, 화면 좌표/줌 감쇠, 스테레오 팬, SFX 설정 실시간 반영. 핸들은 슬롯 세대로 재사용 오정지를 막는다. 상한에서는 화면 밖 무음·낮은 우선순위·오래된 보이스 순으로 회수한다. 향후 타워 타격음도 이 API를 사용한다(`AudioManager.md` §6.2).
