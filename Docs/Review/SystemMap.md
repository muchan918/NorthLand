...
- `TowerMergePanelView`(#183, UI) — 합성 패널. **코디네이터만 참조**(파사드). 상단 선택 리스트 + 하단 후보 버튼(레시피당 1개 미리 생성 후 기본 숨김, `CanMerge`면 `SetActive`, onClick→`RequestMerge`). 카탈로그=`Resources.LoadAll<TowerRecipe>("ScriptableObjects/Recipes")` + `Result.TowerID` 결정적 정렬
