...
| Management(Resource) (자원 지갑·생산처)     | n0wst4ndup | `Assets/Scripts/ManagementSpace`                              | 지갑·생산처(#42) + 경영 패널 UI·DayNightManager 낮/밤 루프 연동(#43, #66). 정산=OnNightToDay(**주민 배치 초기화는 #219에서 제거 — 전날 배치 유지**). **밤→낮 전환은 이제 밤 전용 임시 UI(`NightActionPanelView`)의 "웨이브 성공" 버튼이 트리거(WL-018)** — 경영 패널은 낮→밤(`EndDay`)만 담당. **✅ 낮 프로세스 강제 게이팅 해제(#219)**: 전원 배치·영토 확장 선행 조건을 없애고, 미충족 시 낮 종료 확인 팝업(`ManagementEndDayConfirmPopup`)으로 안내만 한다(WL-022/WL-035 해소). **✅ 주민 수 증가(#227)**: 본진 전용 패널(`CastlePanelUI`)에서 자원을 소모해 총 보유 주민 수를 늘린다(시작 2명/최대 10명, 상한=비용 테이블 행 수) — 주민 '수'의 placeholder는 해소, 주민 개체 시스템은 여전히 부재. 소비처·마나석 생산 후속. **✅ 확장 자원 라인 구현(#166)**: 미개척 영지(영토 해금) = 특수 자원(금/루비/사파이어/다이아) **매일 자동 수급** — `HandleNightToDay`가 Owned 노드에서 `SupplyDaily`만큼 `Add`(주민 배치 무관). 패널은 **고정 8행**(기본3+마나+특수4, 동적 등록 아님): 특수/마나는 +/- 숨김, 특수는 "+n"(일일 수급)·**미개방 시 회색**·활성 우선 재정렬, 마나 "+n"=`ManaPerWaveClear`. `ProductionLineView`에 Villager/Supply/Mana 모드. **지갑(보유량) 표기를 탑 바 → 각 행의 지갑 칸(`_balanceText`→ProdRow Wallet)으로 이관**(#166): 탑 바 `Wood/Iron/Food/Mana_hud` 비활성화, 주민 풀·페이즈만 탑 바 유지. **🔀 잔여 방향**: ②생산 건물 3종 업그레이드(#139 구현됨), ③탑 바 HUD 오브젝트 완전 삭제는 후속. **✅ 마법 연구소 업그레이드**: 생산 라인과 별개인 **업그레이드 전용 건물 트랙**(`_upgradeBuildings`)으로 구현 — 마나석 비용·레벨 추적 + 강화 효과(스킬 시스템이 `GetUpgradeLevel`로 레벨 참조, 결합도 최소)도 **구현 완료(#205)**. BuildingUpgrade.md §8 |
...
- `CastlePanelUI.Instance.Show(BuildingAsset)` / `Hide()` — 본진 전용 패널(#227). 위 둘과 **같은 계보의 세 번째 씬 싱글톤**.
  주민 수 현황·다음 증가 비용을 표시하고 버튼으로 `TryIncreaseVillagers`를 호출한다. 비용 행은 `BuildingInfoUI`와 같이
  매 `OnChanged`마다 재생성한다 — `StorePanelUI`가 재생성을 피한 건 *행 자체에 버튼이 있어서*이고, 여기 액션 버튼은
  패널의 고정 직렬화 필드라 그 위험이 없다. 업그레이드 버튼은 자리·레벨 표시만 있고 동작은 후속 이슈
- **패널 3개는 상호 배타**다. `BuildingInfo.OnSelected`가 **데이터 존재**로 분기한다 — `Villager.Levels` 유무 → `Exchange.Offers` 유무 →
  기본(`BuildingInfoUI`) 순. **`BuildingType`으로 분기하지 않는다**(타입은 인스펙터 authoring 분류일 뿐, 동작 게이트는
  '데이터 존재'로 건다는 기존 컨벤션 — `BuildingType.Castle`이 생긴 뒤에도 이 규칙은 그대로다).
  상호 배타는 `BuildingInfo.ShowOnly(Panel)`/`HideAll(except)` 헬퍼 한 곳에 모아 두어, 패널이 더 늘어도 '닫기 누락'이
  구조적으로 생기지 않는다. `OnDeselected`는 조건 없이 전부 닫는다
...
- `ManagementController` (경영 로직/모델, MonoBehaviour) — 지갑·생산처·주민 배치·업그레이드 상태 소유. `AssignVillager(int)`/
  `UnassignVillager(int)`, `EndDay()`(낮→밤 전용, #219에서 `RequestAdvancePhase`에서 개명 — **강제 게이트 없음**:
  영토 미확장·유휴 주민이어도 그대로 넘어가고, 확인은 UI(`ManagementEndDayConfirmPopup`)가 담당한다.
  판정 데이터만 `bool HasExpandedTerritory`·`bool HasIdleVillagers`로 노출. **밤→낮 `EndNight()`은 더 이상
  ...
  **주민 증가 게이트웨이**(#227, 본진): `int VillagerGrowthMax(BuildingAsset)`·`int VillagerGrowthCount`·`IReadOnlyList<ResourceCost> NextVillagerCost(BuildingAsset)`·
  `bool CanIncreaseVillagers(BuildingAsset)`·`bool TryIncreaseVillagers(BuildingAsset)` — 낮 전용, 비용은 `BuildingAsset.Villager.Levels`(SO),
  같은 `TrySpend` 게이트웨이 경유. 업그레이드 트랙과 달리 **별도 index 도메인을 만들지 않고 `BuildingAsset`을 그대로 받는다**(본진은 씬에 하나뿐 —
  `GetUpgradeLevel(BuildingAsset)`과 같은 저결합 창구 형태). 시작값(`_maxVillagers`, 씬 직렬화)과 런타임 증가분(`_bonusVillagers`)을
  분리해 공유 SO를 오염시키지 않으며(WL-016), `MaxVillagers`가 둘의 합이다. `_bonusVillagers`는 **늘린 횟수와 소진한 테이블 행 수를 겸한다**
  (별도 카운터를 두면 어긋날 수 있어 하나로 합쳤다 — 행을 다 쓰면 `NextVillagerCost`가 null=MAX),
...
| Management(Resource) ↔ DayNightManager   | 정산=`OnNightToDay` 구독, 낮→밤 전환=`ManagementController.EndDay()`가 `DayNightManager.EndDay()` 호출. **밤→낮(`EndNight`)은 이제 `NightActionPanelView`의 "웨이브 성공" 버튼이 임시 트리거 — 밤 종료 주체(Combat 웨이브 클리어 등)로 책임 이관 필요(WL-018)**. **밤→낮에 초기화하는 건 이제 없다(#219)** — 배치(`_villagerCounts`)도 전날 그대로 유지되고(매일 재배치 강제 제거), 본진에서 늘린 총 보유 주민 수(`_bonusVillagers`, #227)도 당연히 유지된다                |
...
| Management(Resource) ↔ 주민 수(#227)     | **placeholder 해소**. 총 보유 주민 수 = `_maxVillagers`(시작값, 씬 직렬화 `2`) + `_bonusVillagers`(본진에서 늘린 런타임 증가분) = `MaxVillagers`. 증가 진입점은 `CastlePanelUI` → `TryIncreaseVillagers(BuildingAsset)`, 상한은 `castle.asset`의 `Villager.Levels` **행 수**(8행 = 최대 10명). 밤→낮은 배치도 주민 수도 초기화하지 않으므로(#219) 늘어난 주민 수는 그대로 유지된다. 낮 도중 늘린 주민은 그날 바로 배치할 수 있고(`AssignVillager`의 상한이 `MaxVillagers`), **배치하지 않아도 밤으로 넘어갈 수 있다**(#219로 전원 배치 강제 해제) — 유휴로 남으면 낮 종료 확인 팝업이 `HasIdleVillagers`로 경고만 한다. ⚠ `HasIdleVillagers`의 기준은 시작값 `_maxVillagers`가 아니라 **`MaxVillagers`**여야 한다(#219+#227 병합 시 실제로 어긋났던 지점 — 팝업이 표시하는 유휴 인원수 `MaxVillagers - AssignedTotal`와 판정 기준이 달라 "2/5인데 경고가 안 뜨는" 누락이 생긴다). **주민 개체(엔티티) 시스템은 여전히 부재** — 여기서 다루는 건 '수'뿐이다 |
| Management(Resource) ↔ Territory         | `TerritoryController.HasExpandedToday`(하루 1회 확장 완료 여부, `OnDayStart`마다 초기화)를 `ManagementController.HasExpandedTerritory`로 노출 — **배치 게이트가 아니라 낮 종료 확인 팝업의 경고 판정용**이다(#219). 종전의 배치 선행 조건(`CanAssignVillagers`)은 **폐지**됐다: 영토를 확장하지 않아도 주민을 배치할 수 있고, `CanEditLine`은 밤 게이팅(`IsDay`)만 유지한다(WL-035 — 프론티어 소진 소프트락 경로 제거, 이슈 #67·GDD §6.1 재해석). `ManagementController`가 `TerritoryController.OnChanged` 구독해 확장/낮 시작 시 패널 즉시 갱신. `TerritoryController`가 씬에 없으면(null) `HasExpandedTerritory`는 permissive하게 true(WL-002와 동일 완화 패턴). **자원 수급(#166)**: `ManagementController.HandleNightToDay`가 매 정산 시 `Graph.Nodes`의 Owned+Definition 노드를 순회해 `ResourceWallet.Add(Definition.Kind, DailyYield)`로 **매일 자동 수급**(확보 즉시 지급·주민 배치 무관, GDD §3.2). 종전 `OnNodeClaimed` 즉시 효과 적용 경로는 제거됨 — muchan이 `TerritoryNode.Definition`/`DailyYield`·`Graph.Nodes` 구조를 바꾸면 수급 정산이 깨짐 |
...
| TowerFusion ↔ Combat                     | `Tower.Asset` 읽기로 재료 TowerID 매칭 + 재료 소모=`Destroy(Tower.gameObject)`(`OnDisable`로 `Tower.Active` 자동 해제). 결과=일반 `TowerAsset` 배치. SUNGSOO가 `Tower.data`/`Asset` 또는 `TowerAsset` 필드 그룹을 바꾸면 매칭·배치 깨짐(WL-001 축, 읽기 접근자는 muchan이 `Tower.cs`에 추가) |
| TowerFusion ↔ MouseManager/TowerPlacer   | `TowerPlacer` 신규 오버로드로 고스트 배치 재사용(확정 콜백에서 재료 소모, 비용은 기존 `TrySpend` 경로). tileSize·풋프린트(WL-034)·타일 종류 계약(WL-067)의 동일 전제를 그대로 상속 — TowerPlacer가 신맵 질의로 이관되면 합성 배치도 함께 따라감. **선택(#183)**: `TowerMergeCoordinator`가 `MouseManager.OnGroupSelectToggled`(Shift 토글)+`OnPrimarySelect`(평클릭/Esc/빈곳, 항상 발행)를 구독해 그룹을 만든다 — MouseManager는 마커 `IGroupSelectable`(타워는 `TowerGroupSelectable`, `TowerPlacer`가 배치 시 런타임 부착)만 보고 타워를 모름. n0wst4ndup이 MouseManager 선택 계약(Shift·Esc·`OnPrimarySelect`)을 확장 → 다른 선택 소비처(건물/영지)와 공존 확인. **밤 진입 배치 취소는 `PhasePanelSwitcher.ShowNight`로 이관**(페이즈 취소 일원화) |
