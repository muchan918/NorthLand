...
- `TowerAsset.IsUnlocked(DayNightManager dayNight = null)` — **타워 해금 판정 단일 출처**(#504). `dayNight.CurrentWave >= UnlockWave`, 참조가 없으면(`Instance`도 없는 타워 테스트 씬) 전부 해금으로 본다. 매번 계산한다 — 이벤트로 캐시하면 세이브 복원(`TryRestoreState`는 페이즈 이벤트를 발행하지 않는다) 후 잠긴 채 남는다. 소비처 둘: `TowerSelectPanelView`의 버튼 게이트(자물쇠·회색 처리)와 `TowerTooltipView`의 해금 웨이브 줄 — 각자 비교하면 게이트는 잠갔는데 툴팁은 열린 것처럼 보인다. 별도 static 헬퍼를 두지 않고 SO의 메서드로 둔 이유는 `UnlockWave`와 같다 — 게이트와 그 기준값을 한 곳에
