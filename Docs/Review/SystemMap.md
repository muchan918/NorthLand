...
| Tutorial (튜토리얼 진행 틀, #408) | muchan | `Assets/Scripts/Tutorial` | **틀만 구현**(#408) — 단계 SO + 진행 컨트롤러 + 오버레이 + 완료 조건 추상화(`[SerializeReference]`), 조건 2종(`BuildingActionCondition`/`PhaseChangedCondition`). 새 단계 비용은 "조건 클래스 1개 + 단계 에셋 1개"이며 컨트롤러·오버레이는 안 바뀐다. 완료 판정은 각 시스템의 **기존 이벤트 구독**으로만 한다(임의 타이머 금지). 딤·강조·입력 차단, 다국어, 자동 실행, 완료 기록은 **미구현** — 후속 이슈. 실제 안내 내용은 #271, 절차·규칙 정본은 `Docs/Core/Tutorial.md`. 현재 GameScene 복사본(`Assets/Personal/muchan/Scene/TutorialTest.unity`)에만 있고 정본 씬 미반영 |
...
- `TutorialController.IsRunning` / `StartTutorial()` / `StopTutorial()` — 진행 소유. 단계 순서는 인스펙터 `steps` 리스트의 **등록 순서가 전부**이며 단계 에셋은 자기 인덱스를 갖지 않는다(`MonsterWaveAsset`과 같은 규칙, #294). `startOnPlay`는 자동 실행 시점이 정해지기 전의 임시 스위치다.
...
- `TutorialOverlay.ShowPopup(string, string, Sprite)` / `ShowBubble(string)` / `HidePopup()` / `HideBubble()` / `HideAll()` / `PopupConfirmed` — 표시 전용. 단계를 모르며, 확인 클릭을 스스로 처리하지 않고 이벤트로만 알린다. **팝업 루트가 전체 화면 + `raycastTarget`이라 팝업 구간의 입력 차단이 여기 딸려 온다**; 반대로 말풍선 루트는 `raycastTarget`을 꺼야 뒤쪽 월드 클릭이 산다(`MouseManager`가 `IsPointerOverGameObject()`만 보기 때문).
