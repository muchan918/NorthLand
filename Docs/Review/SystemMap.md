...
- `Projectile.DamageDealt`(`static event Action<IAttacker, IDamageable>`, `NorthLand.Combat`, #169 muchan 추가) —
  타워의 **직접 타격** 데미지가 들어간 직후 발행(투사체 단일/스플래시 + 히트스캔 체인). **static이므로 구독 해제는
  구독자 책임**(파괴된 MonoBehaviour를 남기면 죽은 구독자 호출 버그). 현재 구독자: `BurnBuff`(버프 창 동안만).
  ⚠️ **투사체 밖에서 발행할 때는 `Projectile.RaiseDamageDealt(source, target)` 창구를 쓴다**(C# 이벤트는 선언
  클래스 밖에서 `Invoke` 불가) — `ChainResolver`가 이 경로다. **지속 효과(오라 DoT)의 틱에서는 부르지 않는다**:
  구독자가 명중마다 지속시간을 갱신하므로 틱마다 통지하면 갱신이 끊기지 않아 사실상 영구 화상이 된다.
  ⚠️ **전달 방식으로 거르는 것은 구독자 책임이다** — 이 통지 자체는 전달 방식을 구분하지 않으므로, 특정 방식만
  받고 싶으면 발행을 막지 말고 구독부에서 건다(발행을 막으면 앞으로 붙을 다른 구독자까지 그 명중을 못 본다).
  실제 사례: `BurnBuff`는 `Has<AttackBehaviour>()`로 **투사체 타워만** 통과시킨다 — 체인은 번개 테마상 발화가
  성립하지 않고, 1발에 최대 `MaxChainTargets`명을 때려 거르지 않으면 화상 처리량이 최대 4배가 된다(#252).
  이벤트 이름·소유가 `Projectile`에 남아 있는 것은 임시다(중립 위치 이동은 #252 미결)
...
- `Tower.Has<T>()` / `Tower.Get<T>()`(`where T : ITowerBehaviour`) — **능력 질의 창구.** 소비처가 타워의 구상 타입이
  아니라 능력을 묻게 한다. 예: 보스 P3 마력 봉인 대상 = `Has<IAttackBehaviour>()`(`EnemyNodeQuery.IsAttackTower`).
  ⚠️ **무엇을 묻는지에 따라 타입을 고른다** — 둘을 헷갈리면 조용히 틀린 집합을 얻는다(#252):
  · **"이 타워는 공격하는가"** → `Has<IAttackBehaviour>()`. 전달 방식과 무관해야 하는 판정(버프 오라 대상,
    보스 봉인 대상, `Tower`의 `IAttacker` 4개). 실제로 체인을 분리할 때 이 5곳이 구상 클래스로 묻고 있어
    전부 체인 타워를 제외했고, 컴파일 에러가 아니라 "버프가 안 걸리는 타워"로만 나타났다
  · **"이 타워는 투사체를 쏘는가"** → `Has<AttackBehaviour>()`. 전달 방식을 **일부러** 구분하는 판정
    (`BurnBuff`가 화상 대상을 투사체 타워로 좁히는 경로). 이 경우엔 구상 클래스가 정답이므로
    "인터페이스로 바꾸는" 리팩터를 하면 안 된다 — 호출부 주석에 의도를 명시해 둘 것
- `IAttackBehaviour : ITowerBehaviour`(`NorthLand.Combat`, #252) — **공격 능력을 가진 행동의 공통 계약**
  (`Damage`/`Range`/`Interval`/`TryAttack`). 전달 방식별 구현: `AttackBehaviour`(투사체) / `HitscanAttackBehaviour`
  (즉시 판정). "이 타워는 공격하는가"를 묻는 소비처는 이 인터페이스만 알면 되고, 전달 방식이 늘어도 판정하는 곳을
  손대지 않는다. `Tower`의 `IAttacker` 구현(`AttackDamage`/`AttackRange`/`AttackInterval`/`TryAttack`)이 이걸 위임한다
- `HitscanAttackBehaviour`(`NorthLand.Combat`, #252) — **즉시 판정(히트스캔) 공격 행동.** 현재 `TowerType.Chain`이
  쓴다(`TowerBehaviourFactory.Create:47`). 발사와 명중이 같은 프레임에 끝나므로 "비행 중 첫 대상 사망" 창이 없다 —
  구 투사체 체인은 `Homing` 조기 파괴/`Ballistic` 착탄 시 생존 검사 때문에 첫 대상이 죽으면 홉 전체를 잃었다.
...
- `ChainBeamVisual.Spawn(prefab, origin, path, lifetime, fallbackColor)`(`static`, `NorthLand.Combat`, #252) —
  ...
  **스냅샷으로 고정**하고 대상을 추적하지 않는다(순간 판정이라 자연스럽고, 홉 대상이 연출 중 죽어도 파괴된
  `Transform`을 역참조하지 않는다). `prefab`(`ChainFields.BeamPrefab`)이 있으면 아트 외형을 쓰고 **코드가 색·폭을
  건드리지 않는다**(페이드는 아트 책임) — `null`이면 `Sprites/Default`로 기본 빔을 만들고 알파 페이드까지 코드가 한다.
  런타임 `Material`은 코드 경로에서만 소유하고 `OnDestroy`에서 파괴한다(발사마다 생성 — `RangeCircle`과 같은 사유)
