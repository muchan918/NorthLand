...
| MouseManager (입력/선택/배치)               | n0wst4ndup | `Assets/Scripts/GameManager/MouseManager`                            | 3상태 머신 구현(Idle/Placement/SkillTargeting, #103에서 SkillTargeting 추가). Snap 항등·CanPlaceAt 항상 true (TODO). 스킬 타겟팅은 도로 타일 전용(`CombatMapTileView` 질의)                                                                                                                  |
...
- `MouseManager.Instance.BeginSkillTargeting(SkillTargetRequest)` / `CancelSkillTargeting()`(#103) —
  ...
  **도로 타일 전용**: `_placementMask` 히트에서 `CombatMapTileView.TileType`을 읽어 도로=초록·그 외 전투 타일=빨강(`SkillRangeIndicator.SetValid`),
  전투 타일 밖에선 인디케이터 숨김. **도로 위 좌클릭만** `OnConfirmed(hit.point)`로 확정(도로 게이팅은 MouseManager 소유 — 요청 타입엔 `CanPlaceAt` 훅 없음)
...
| PlayerSkill ↔ MouseManager               | 스킬 버튼 클릭 → `BeginSkillTargeting(SkillTargetRequest)` → **도로 타일 위에서만**(`CombatMapTileView.TileType==Road`) 확정, `OnConfirmed(Vector3)`로 `SkillManager.CastAt` 호출(#103). 인디케이터는 전투 타일 밖 숨김·도로=초록/그 외=빨강. `PlacementRequest`와 별개 타입 — 그리드 개념 없음                                                                                                            |
| MouseManager ↔ CombatSpace(맵)           | 스킬 타겟팅이 히트 타일의 `CombatMapTileView.TileType`으로 도로 여부 판정(#103 후속). MouseManager→CombatSpace 단방향 읽기(입력 매니저가 전투 공간 타일 데이터에 의존 — 지켜볼 커플링)                                            |
