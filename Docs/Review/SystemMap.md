...
- `AudioManager.PlaySfxExclusive(AudioClip, float volumeScale = 1f)` — 2D 전용 소스 1개. **직전 것을 끊고 처음부터 재생**하므로 연타에 겹치지 않고, `PlayOneShot`이 아니라 `clip`+`Play()`라 **매 프레임 볼륨을 다시 곱한다**(재생 중 슬라이더 반영). ⚠ 소스가 하나뿐이라 **이 경로의 소리들끼리도 서로를 끊는다** — 현재 소비처는 주민 증가음 1개. 길거나(수 초) 연타 가능한 버튼에 물린 소리 전용이다.
