...
| PlayerSkill (플레이어 스킬, #103)           | muchan     | `Assets/Scripts/Skill`                                                | 클릭 시전 감전 스킬(기본 스킬 1종). 밤 게이팅(`Tower.cs`와 동일하게 `DayNightManager.CurrentPhase` 직접 폴링)·쿨다운·범위 데미지(`IDamageable`/`DamageInfo` 재사용, 새 데미지 경로 없음). 수치는 CSV가 아니라 `SkillManager` 인스펙터 직접 입력(WL-015와 같은 축). 버프 스킬(2번째 스킬)·보상 기반 특수효과 업그레이드(감전→퍼지는→다단히트)는 미착수 — 후자는 WL-043(3택1 보상 구조 미착수)이 먼저 해결돼야 착수 가능 |
...
| TerritoryGraph (경영 영토 확장)             | n0wst4ndup | `Assets/Scripts/ManagementSpace/Territory`                    | 그래프 생성(Delaunay+프루닝)·클레임(`ISelectable`)·점진 공개·호버 하이라이트(`IHoverable`) 구현, `GameScene`에 씬 통합 완료(#18, #67). 하루 1회 확장 게이팅(`HasExpandedToday`, `DayNightManager.OnDayStart` 연동)도 #67에서 추가. 효과/마나석 지급(`OnNodeClaimed` 훅)은 여전히 미연결(WL-030) |
...
- `TerritoryGraph` (순수 C# 모델) — `Nodes`, `Frontier`, `OwnedCount`, `GetNode(id)`(null 반환 가능),
  ...
  `event OnNodeClaimed`(효과 적용 훅, 아직 미연결 WL-030), `event OnChanged`
...
| PlayerSkill ↔ Combat                     | 새 데미지 파이프라인 없이 `IDamageable`/`DamageInfo`/`Faction`을 그대로 소비(`Tower.FindTarget()`/`Projectile.ApplyArea()`와 동일한 `OverlapSphereNonAlloc`+Faction 필터링 패턴). `DamageInfo.Source`는 스킬 시전 시 `null`(IAttacker 개체가 아님 — 현재 아무도 역참조 안 해 안전)                       |
