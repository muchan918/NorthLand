...
- `ManagementController` (경영 로직/모델, MonoBehaviour) — 지갑·생산처·주민 배치·업그레이드 상태 소유. `AssignVillager(int)`/
  ...
  **업그레이드 전용 건물 트랙**(마법 연구소 등, 생산 라인과 별개 index 도메인): `int UpgradeIndexOf(BuildingAsset)`·`int UpgradeBuildingLevel/UpgradeBuildingMaxLevel(int)`·
  ...
  같은 `TrySpend` 게이트웨이 경유. **`int GetUpgradeLevel(BuildingAsset)`** = 소비 시스템(스킬 강화 등)이 레벨을 읽는 저결합 창구(효과 적용은 소비 측 소유, **TODO**) — BuildingUpgrade.md §8,
...
- `MouseManager.Instance` `event OnGroupSelectToggled(IGroupSelectable)`(#183) — Shift(추가 선택 키)+마커 클릭 시 토글 발행. 단일 `OnSelectionChanged`와 별개이며 이 경로에선 `_selected`를 건드리지 않음. Idle에 Esc→`Select(null)`(전체 해제) 추가 — **우클릭은 해제에 쓰지 않음**(카메라 드래그 이중 점유, WL-073)
...
- `TowerMergeGroup`(#183, 순수 C#) — 선택 재료 집합. `IReadOnlyList<Tower> Towers`/`Add`/`Remove`/`Clear`/`Prune` + `event Action OnChanged`(변경 시 발행, 코디네이터가 구독해 UI·하이라이트 갱신). **`TowerMergeCoordinator`가 소유**(씬 오브젝트 아님) — 구 임시 `TowerWallet`(#195) 대체·폐기
- `TowerMergeCoordinator`(#183, MonoBehaviour) — 합성 선택 두뇌·실행 오케스트레이터. `MouseManager.OnSelectionChanged`/`OnGroupSelectToggled`·`DayNightManager.OnDayToNight`·`Tower.ActiveChanged`·`TowerMergeGroup.OnChanged` 구독(낮 게이팅·그룹 관리·밤 리셋+`CancelPlacement`·stale `Prune`·하이라이트·우측 패널 스왑 1개=`TowerInfoUI`/2개↑=합성 패널). 파사드: `SelectedTowers`/`event OnGroupChanged`/`CanMerge(recipe)`/`RequestMerge(recipe)`. `OnDestroy`에서 구독 해제(F7)
- `IGroupSelectable { Tower Tower; OnGroupSelected(); OnGroupDeselected() }`(#183, 도메인 중립 마커) + `TowerGroupSelectable`(타워 구현, `TowerPlacer.PlaceTower`가 `TowerFootprint`와 같은 지점에서 런타임 `AddComponent`) — MouseManager가 마커 유무로만 그룹 선택 자격 판정(타워 무지 → 제네릭). 그룹 하이라이트 훅은 단일선택 `ISelectable`과 분리
- `TowerMergePanelView`(#183, UI) — 합성 패널. **코디네이터만 참조**(파사드). 상단 선택 리스트 + 하단 후보 버튼(레시피당 1개 미리 생성 후 기본 숨김, `CanMerge`면 `SetActive`, onClick→`RequestMerge`). 카탈로그=패널 인스펙터 직렬화 배열 `TowerRecipe[] _recipes`(등록 순서 결정적; 예시 SO는 `Resources/ScriptableObjects/Recipes/`)
...
| Management(Resource) ↔ PlayerSkill(마법 연구소) | **⚠ 제안 단계 — 확정 계약 아님(소비 측 muchan 사인오프 대기)**. 제안 메커니즘: 스킬 강화가 `ManagementController.GetUpgradeLevel(magicLabAsset)`으로 **레벨(int)만** 읽어 스킬 측이 효과를 정한다(레벨→수치 매핑=스킬 소유, 컨트롤러는 "스킬"을 모름, `OnChanged` 통지→재-pull). **현재 강화 효과 미구현(TODO) → 코드 충돌 없음.** ⚠ **중복 주의**: PlayerSkill은 이미 보상 기반 자체 레벨 축(`SkillEffect.Level`/`SkillEffectManager.GetLevel`, #169)을 갖고 GDD §5.5/§5.6은 강화 원천을 보상 트랙으로 명시 — 연구소 레벨의 **착지점**(기본 스킬 스탯 배율 vs 보상 특수효과 레벨)을 muchan과 확정 후 GDD §5.5 편입, 이 행에 합의 일자 명기할 것. 합의 전 코드 선반영 금지. BuildingUpgrade.md §8 |
...
| TowerFusion ↔ MouseManager/TowerPlacer   | `TowerPlacer` 신규 오버로드로 고스트 배치 재사용(확정 콜백에서 재료 소모, 비용은 기존 `TrySpend` 경로). tileSize·풋프린트(WL-034)·타일 종류 계약(WL-067)의 동일 전제를 그대로 상속 — TowerPlacer가 신맵 질의로 이관되면 합성 배치도 함께 따라감. **선택(#183)**: `TowerMergeCoordinator`가 `MouseManager.OnGroupSelectToggled`(신규)+`OnSelectionChanged`를 구독해 그룹을 만든다 — MouseManager는 마커 `IGroupSelectable`(타워는 `TowerGroupSelectable`, `TowerPlacer`가 배치 시 런타임 부착)만 보고 타워를 모름. n0wst4ndup이 MouseManager 선택 계약(Shift·Esc)을 확장 → 다른 선택 소비처(건물/영지)와 공존 확인 필요 |
