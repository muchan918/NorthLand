...
- `TowerMergeTargetSlot.Set(Sprite, string)`(UI) — 정보 패널 "합성 후보" 블록의 한 칸(아이콘+이름). **겉모습은 배치 팔레트 칸과 동일** — 프리팹은 `TowerButton.prefab`을 복제해 `Button`(누를 수 없는 칸)·`TowerButtonView`·`TowerLockOverlay`를 떼고 이 컴포넌트를 붙인 것. `TowerButtonView`를 그대로 안 쓰는 이유는 그쪽 `SetLocked`·해제 연출이 팔레트의 **해금** 개념과 한 몸이고 정보 패널엔 잠금이 없어서. 호버 툴팁은 `TowerInfoUI`가 `TowerTooltipSource`를 런타임 부착해 붙이므로 프리팹 배선 불필요 — 단 `Slot/Img_Bg`의 `Raycast Target`을 켠 채로 둬야 호버가 잡힌다(`Button`을 떼도 남는다)
