...
- `Tower.Has<T>()` / `Tower.Get<T>()`(`where T : ITowerBehaviour`) — **능력 질의 창구.** 소비처가 타워의 구상 타입이
  ...
  ⚠️ **구상 클래스가 아니라 인터페이스로 물을 것** — `Has<AttackBehaviour>()`로 물으면 전달 방식이 다른 공격 타워
  (히트스캔=체인)가 조용히 빠진다. 실제로 체인을 분리할 때 버프 오라 대상 선정·보스 봉인 대상 선정·`Tower`의
  `IAttacker` 4개가 전부 체인 타워를 제외했고, 컴파일 에러가 아니라 "버프가 안 걸리는 타워"로만 나타났다(#252)
