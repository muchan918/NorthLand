...
- **WL-019** | OPEN | PR#50 | Tower 페이즈 게이팅 미연결: Tower.Update()가 낮/밤 구분 없이 공격.
  DayNightManager(CurrentPhase/OnDayToNight/OnNightToDay) 머지로 연결 가능해짐. GDD §5.2 밤 전용 동작.
  | Tower가 DayNightManager 구독/조회로 밤에만 공격하도록 연결 (WL-018과 세트)