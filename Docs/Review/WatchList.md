...
- **WL-001** | OPEN | PR#29/53 | 스탯 SO 손입력 이원화 심화: TowerData/EnemyData/PlayerUnitData(SO 직접 입력) vs muchan 폴더 CSV 파이프라인. `TowerAsset`(CSV 기반 데이터 레이어)이 마련됐으나 Combat `Tower.cs`는 아직 미참조 — 마이그레이션은 SUNGSOO와 합의 후 별도 진행 | Tower/Enemy/PlayerUnit 스탯 단일 원본 합의 및 마이그레이션
