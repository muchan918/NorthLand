...
- **WL-028** | OPEN | PR#64 | 우산 UIManager 이름 선점: 팀이 4곳(MouseManager.md:99/TooltipUI.cs:8/TowerInfoUI.cs:4/SystemMap §2)에서 TowerInfoUI·BuildingInfoUI·TooltipUI를 흡수할 미래 우산 매니저 'UIManager'를 예약해 왔으나, PR#64가 결과패널 전용 클래스에 `NorthLand.UI.UIManager` 이름을 선점. asmdef 부재(전부 Assembly-CSharp)로 네임스페이스만 다른 두 UIManager 공존 시 개념 혼선 | n0wst4ndup과 이름 합의(결과패널=ResultUIManager로 양보 또는 이 클래스를 우산으로 승격)