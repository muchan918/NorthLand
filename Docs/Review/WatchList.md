...
- **WL-001** | 부분 착수(PR#80) | PR#29/53/59 | Combat Tower.cs/Enemy.cs가 TowerAsset/EnemyAsset 소비로 이관 완료(archer_tower/goblin_warrior). 옛 Combat SO(TowerData/EnemyData) 삭제로 동명 타입 혼동 해소됨. 잔여: 나머지 타워/적 종류 이관 + Soldier(PlayerUnitData) 이관 미착수. Combat이 이제 전역(네임스페이스 없음) TowerAsset/EnemyAsset 폴리모픽 구조에 하드결합 — muchan이 필드 그룹 변경 시 Combat 파손(SystemMap §2/§5 반영 필요) | 전 종류 이관 + PlayerUnit 스탯 원본 합의
...
- **WL-025** | OPEN(악화 방향) | PR#53/80 | Tower/Enemy FindTarget은 OverlapSphereNonAlloc로 통일됐으나 PR#80의 Projectile.ApplyArea/FindNearestUnhit이 할당형 Physics.OverlapSphere 재도입 | impact 경로도 NonAlloc 버퍼로 통일 콜라이더 배치 규약 확정
- **WL-026** | OPEN(심화) | PR#59/80 | 타워 택소노미가 데이터에 이어 전투 로직(Projectile Single/Area/Chain 분기, Magic no-op)까지 확대 | GDD §6.2 반영 + 팀 합의
...
- **WL-028** | OPEN(재발) | PR#61/80 | EditorBuildSettings 부트 씬이 다시 개인 씬(TestSpace-Sungsoo)으로 지정 | 정본 부트 씬 확정 + 개인 씬 부트 지정 금지
