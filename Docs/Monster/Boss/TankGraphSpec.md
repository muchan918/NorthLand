...
>
> ⚠ **블랙보드 값만 고치면 게임 동작이 바뀌지 않는다.** 컴파일된 그래프의 노드가 빌드 시점 복사본을
> 들고 있고 **런타임 바인딩이 블랙보드를 다시 읽지 않는다**(`BehaviorGraphAgent.Init` 후 실측). 값을
> 바꿀 때는 반드시 **Behavior 에디터**에서 하거나, 저작 블랙보드 · 런타임 블랙보드 · 컴파일된 노드
> 세 곳을 모두 맞춰야 한다. 에디터 표시만 바뀌고 실제 동작은 그대로인 상태가 조용히 만들어진다.
