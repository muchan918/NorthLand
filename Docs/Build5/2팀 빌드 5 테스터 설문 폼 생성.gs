/**
 * 2팀 빌드 5 테스터 설문 — 구글 폼 생성 스크립트
 *
 * ── 사용법 ────────────────────────────────────────────────
 * 1. https://script.google.com 접속 → "새 프로젝트"
 * 2. 기본으로 열린 Code.gs 내용을 전부 지우고 이 파일 내용을 붙여넣기
 * 3. 상단 함수 선택란에서 createForm 선택 → "실행"
 * 4. 최초 실행 시 권한 승인 (내 계정 선택 → 고급 → 안전하지 않음으로 이동 → 허용)
 * 5. 실행 로그(Ctrl+Enter)에 찍힌 편집 URL / 응답 URL 확인
 *    → 응답 URL을 `2팀 빌드 5 체크리스트.txt`에 저장한다 (빌드 3과 동일)
 *
 * ── 응답 취합 ─────────────────────────────────────────────
 * 폼 편집 화면 → "응답" 탭 → 스프레드시트 아이콘을 누르면
 * 응답이 구글 시트로 자동 누적된다.
 *
 * ── 문항 구성 ─────────────────────────────────────────────
 * 문항 15개(14개 + 마지막 자유 의견) + 각 문항마다 자유 의견(장문) 칸.
 * 문항을 추가하려면 choice(...) / text(...) 호출을 추가하면 된다.
 * ─────────────────────────────────────────────────────────
 */

var CONFIG = {
  title: '2팀 빌드 5 테스터 설문',
  description: [
    'NorthLand: Last Stand — Build 5 테스터 설문입니다.',
    '',
    '[플레이 안내]',
    '· zip 압축을 풀고 exe 실행 → 타이틀에서 세이브 슬롯을 하나 고른 뒤 "게임 시작"',
    '· 카메라: WASD 또는 우클릭 드래그로 이동, 휠로 줌',
    '· 목표: 낮에 자원을 모으고 방어를 준비 → 밤에 몰려오는 몬스터로부터 본진을 지킨다',
    '· 한 판은 15웨이브입니다. 끝까지 못 가도 괜찮습니다.',
    '',
    '설문은 5~10분 정도 걸립니다. "자유 의견" 칸은 비워도 됩니다.'
  ].join('\n'),
  // 각 문항 뒤에 자유 의견 칸(장문)을 붙일지 여부
  includeOpinionFields: true
};

function createForm() {
  var form = FormApp.create(CONFIG.title);
  form.setTitle(CONFIG.title);
  form.setDescription(CONFIG.description);
  form.setProgressBar(true);
  form.setCollectEmail(false);

  text(form, '1. 최대 몇 웨이브까지 갔나요? (숫자로 적어주세요 · 한 판은 15웨이브입니다)', true);
  opinion(form, '몇 번째 웨이브에서, 무엇 때문에 멈췄는지 자유롭게 적어주세요.');

  choice(form,
    '2. 매 판을 할 때마다 다른 조합(빌드)을 세워서 공략을 찾아가는 느낌이 있었나요?',
    [
      '판마다 다른 조합을 시도하게 됐다',
      '조금 달라지긴 했다',
      '결국 매번 같은 방식으로 하게 됐다',
      '한 판만 해봐서 모르겠다'
    ],
    true,
    '어떤 조합을 시도했는지, 왜 그 방식으로 굳어졌는지 자유롭게 적어주세요.');

  choice(form,
    '3. 경영 공간의 건물에 지금은 이펙트(파티클 · 강조 표시)만 들어가 있습니다. 어떤 것이 상호작용 가능한 건물인지 잘 보였나요? 건물 위에 아이콘 이미지를 넣으면 좋을까요?',
    [
      '잘 보였다 — 아이콘은 없어도 된다',
      '잘 보였지만 아이콘이 있으면 더 좋겠다',
      '잘 안 보였다 — 아이콘이 필요하다',
      '잘 안 보였지만 아이콘 말고 다른 방법이 좋겠다'
    ],
    true,
    '어느 건물이 특히 안 보였는지, 아이콘이라면 어떤 형태(항상 표시 · 마우스를 올렸을 때만 · 줌 아웃할 때만 등)가 좋을지 자유롭게 적어주세요.');

  choice(form,
    '4. 타워 합성(설치된 타워 여러 개를 재료로 더 강한 타워를 만드는 것)에 장점이 있다고 느꼈나요?',
    [
      '확실히 이득이라 자주 썼다',
      '이득인 것 같아서 가끔 썼다',
      '재료가 아까워서 잘 안 썼다',
      '이득인지 아닌지 모르겠다',
      '합성을 안 해봤다 / 몰랐다'
    ],
    true,
    '어떤 점이 좋았는지 / 왜 손해처럼 느껴졌는지 자유롭게 적어주세요.');

  choice(form,
    '5. 그날 타워를 하나도 배치하지 않고 밤으로 넘어가려 하면 확인 팝업이 뜹니다. 필요한 기능이라고 생각하나요?',
    [
      '필요하다 — 실수를 막아줬다',
      '있어도 그만 없어도 그만이다',
      '불필요하다 — 매번 뜨는 게 귀찮았다',
      '팝업을 못 봤다'
    ],
    true,
    '팝업이 뜬 상황과 그때 느낌을 자유롭게 적어주세요.');

  choice(form,
    '6. 공중 몬스터(박쥐)의 이동 경로가 어색하지 않았나요?',
    [
      '자연스러웠다',
      '조금 어색했다',
      '많이 어색했다',
      '공중 몬스터를 못 봤다'
    ],
    true,
    '어느 구간이 어색했는지(너무 높이 뜬다 · 지형을 통과한다 · 지상 몬스터와 경로가 겹친다 · 어디로 가는지 모르겠다 등) 자유롭게 적어주세요.');

  choice(form,
    '7. 경영 공간(낮에 건물을 만지는 마을) 크기에 비해 전투 공간(밤에 몬스터가 오는 맵) 크기가 적절했나요?',
    [
      '적절했다',
      '전투 공간이 너무 좁다',
      '전투 공간이 너무 넓다',
      '경영 공간이 너무 넓다',
      '경영 공간이 너무 좁다'
    ],
    true,
    '두 공간을 오가면서 크기 때문에 불편했던 점을 자유롭게 적어주세요.');

  choice(form,
    '8. 전투맵은 하루가 지날 때마다 경로가 조금씩 길어집니다. 늘어나는 정도가 적당했나요?',
    [
      '적당했다',
      '너무 조금씩 늘어나서 변화가 안 느껴졌다',
      '너무 많이 늘어나서 감당이 안 됐다',
      '길이가 늘어나는 줄 몰랐다'
    ],
    true,
    '몇 웨이브쯤부터 그렇게 느꼈는지 자유롭게 적어주세요.');

  choice(form,
    '9. 전투 공간에 타워를 강화해 주는 버프 타일이 있습니다(타워를 들면 아이콘으로 표시됩니다). 생성 빈도가 어땠나요?',
    [
      '적절했다',
      '적어서 활용할 기회가 별로 없었다',
      '너무 많아서 아무 데나 놔도 됐다',
      '버프 타일이 있는 줄 몰랐다'
    ],
    true,
    '버프 타일을 실제로 노리고 배치했는지 자유롭게 적어주세요.');

  choice(form,
    '10. 본진(무지개 포탈) 쪽 경로 초반 구간에는 버프 타일이 일부러 나오지 않습니다(타워가 본진 앞에만 몰리는 것을 막기 위해서입니다). 버프 타일이 없는 그 구간의 길이가 어떻게 느껴졌나요?',
    [
      '짧다',
      '적당하다',
      '길다',
      '그런 구간이 있는 줄 몰랐다'
    ],
    true,
    '그래서 타워를 어디에 놓게 됐는지 자유롭게 적어주세요.');

  choice(form,
    '11. 전투 공간에는 타워를 설치할 수 없는 장애물 타일이 있습니다. 생성 빈도가 어땠나요?',
    [
      '적절했다',
      '적어서 어디든 지을 수 있었다',
      '너무 많아서 지을 자리가 부족했다',
      '장애물 타일을 신경 쓰지 않았다'
    ],
    true,
    '장애물 때문에 배치가 막혔던 순간이 있으면 자유롭게 적어주세요.');

  choice(form,
    '12. 시작부터 직접 배치할 수 있는 타워가 7종 열려 있습니다. 처음에 고를 것이 많다고 느꼈나요?',
    [
      '적절했다 — 7종 다 열려 있는 게 좋다',
      '조금 많았다',
      '너무 많아서 뭘 지어야 할지 몰랐다',
      '오히려 더 많았으면 좋겠다'
    ],
    true,
    '처음에 2~3종만 열어 두고 판이 진행되면서 기본 타워가 늘어나는 방식은 어떨 것 같은지 자유롭게 적어주세요.');

  choice(form,
    '13. 웨이브를 깨면 스킬 강화 보상을 고릅니다(현재 5종 · 각 3레벨). 종류가 너무 많다고 느꼈나요?',
    [
      '적절했다',
      '많아서 한 판에 다 못 키웠다',
      '적어서 금방 다 봤다',
      '보상이 뭐였는지 잘 모르겠다'
    ],
    true,
    '한 판에 3종을 만렙까지 올리고 나머지는 조금씩 얻는 방향과, 1종만 집중해서 수치가 크게 뻥튀기되는 방향 중 어느 쪽이 좋을지 자유롭게 적어주세요.');

  choice(form,
    '14. 지금은 플레이어 스킬이 감전 1종이고, 보상으로 그 스킬에 특수효과를 붙이는 방식입니다. 스킬 종류가 적다고 느꼈나요?',
    [
      '1종이라도 충분했다',
      '조금 적게 느껴졌다',
      '너무 적어서 밤에 할 게 없었다',
      '스킬을 거의 안 썼다'
    ],
    true,
    '스킬을 2~3종으로 늘리고 특수효과도 그만큼 늘리는 방향은 어떨 것 같은지 자유롭게 적어주세요.');

  form.addParagraphTextItem()
    .setTitle('15. 플레이하면서 불편했던 점 · 그 외 자유 의견')
    .setHelpText('위 문항에 없던 내용도 좋습니다. 버그를 발견했다면 재현 방법과 함께 적어주세요.')
    .setRequired(false);

  // ── 결과 출력 ────────────────────────────────────────────
  Logger.log('폼이 생성되었습니다.');
  Logger.log('편집 URL : ' + form.getEditUrl());
  Logger.log('응답 URL : ' + form.getPublishedUrl());
  Logger.log('');
  Logger.log('응답 URL을 "2팀 빌드 5 체크리스트.txt"에 저장하세요.');
  Logger.log('응답을 스프레드시트로 받으려면: 편집 화면 → 응답 탭 → 스프레드시트 아이콘 클릭');

  return form.getEditUrl();
}

// ── 헬퍼 ──────────────────────────────────────────────────

/** 단답 문항을 추가한다. */
function text(form, title, required) {
  form.addTextItem().setTitle(title).setRequired(!!required);
}

/** 자유 의견(장문) 칸을 단독으로 추가한다. */
function opinion(form, helpText) {
  if (!CONFIG.includeOpinionFields) {
    return;
  }
  var item = form.addParagraphTextItem();
  item.setTitle('↳ 자유 의견 (선택)');
  if (helpText) {
    item.setHelpText(helpText);
  }
  item.setRequired(false);
}

/**
 * 객관식 문항 + 자유 의견 칸을 추가한다.
 * @param {Form} form 대상 폼
 * @param {string} title 질문
 * @param {string[]} choices 선택지
 * @param {boolean} required 필수 여부
 * @param {string=} opinionHelpText 자유 의견 칸에 붙일 안내문
 * @param {boolean=} withOpinion 자유 의견 칸을 붙일지 (기본 true)
 */
function choice(form, title, choices, required, opinionHelpText, withOpinion) {
  var item = form.addMultipleChoiceItem();
  item.setTitle(title);
  item.setChoiceValues(choices);
  item.setRequired(!!required);

  var attach = (withOpinion === undefined) ? true : withOpinion;
  if (attach) {
    opinion(form, opinionHelpText);
  }
}
