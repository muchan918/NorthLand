...
// 정확한 0~1 비율을 뽑는다.
//
// 이 두 값은 **메시의 로컬 치수**이지 월드 크기가 아니다 — 현재 소비처(SkillAura.prefab)가 유니티
// 기본 Cylinder라 머티리얼에 그 규격(반지름 0.5, 높이 2, Y[-1,1])을 그대로 적어 두고, 실제 스킬 사거리는
// SkillRangeIndicator가 트랜스폼 X/Z 스케일로만 반영한다(머티리얼 프로퍼티는 런타임에 건드리지 않는다).
// 다른 메시로 교체할 때만 이 두 값을 그 메시 규격에 맞춰 다시 적으면 된다.
