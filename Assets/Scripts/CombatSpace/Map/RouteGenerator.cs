    ...
    public sealed class RouteGenerator
    {
        ...
        public bool Generate( CombatMapData map, CombatMapGenerationSettings settings, System.Random random)
        {
            ...
            List<Vector2Int> targets =
                CreateRouteTargets(map);

            HashSet<Vector2Int> occupiedRoute =
        ...
        private List<Vector2Int> CreateRouteTargets(
            CombatMapData map)
        {
            List<Vector2Int> targets =
                new List<Vector2Int>(
                    map.MajorWaypoints.Count + 1);

            targets.Add(
                map.RouteStartPosition);
        ...
        // 노이즈 가중 A*로 한 구간 생성
        private List<Vector2Int> FindSegmentPath(
            CombatMapData map,
            ...
            HashSet<Vector2Int> occupiedRoute)
        {
            ...
            List<SearchState> openSet =
                new List<SearchState>
                {
                    startState
                };

            HashSet<SearchState> closedSet =
                new HashSet<SearchState>();

            Dictionary<SearchState, SearchState>
                cameFrom =
                    new Dictionary<
                        SearchState,
                        SearchState>();

            Dictionary<SearchState, float> gScore =
                new Dictionary<SearchState, float>
                {
                    [startState] = 0f
                };

            Dictionary<SearchState, float> fScore =
                new Dictionary<SearchState, float>
                {
                    [startState] =
                        GetHeuristic(
        ...
        private bool IsAdjacentToOccupiedRoute(
