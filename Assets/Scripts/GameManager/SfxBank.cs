...
public class SfxBank : ScriptableObject
{
    ...
    [Tooltip("충전이 없어 스킬을 쓸 수 없는데 시전을 시도한 순간(Q·비활성 버튼 클릭). " +
             "낮 페이즈·게임 종료로 못 쓰는 경우에는 울리지 않는다 — SkillManager.IsOutOfCharges 참고. " +
             "이름에 '쿨다운'을 쓰지 않는 이유는 GDD §5.6의 확정 어휘(감전은 충전을 쓴다)를 따르기 때문이다.")]
    [SerializeField] Cue skillOutOfCharges = new Cue();
    ...
    public Cue SkillOutOfCharges => skillOutOfCharges;
