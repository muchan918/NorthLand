...
namespace NorthLand.UI
{
    ...
    public sealed class EnemyStateUILayer : MonoBehaviour
    {
        // 정본은 `Assets/Resources/UI/EnemyStateUI.prefab`(메인 저장소)이다.
        //
        // **Resources로 집어오는 이유**: 씬·프리팹 어디에도 참조를 심지 않기 위해서다. 씬에 배선을 두면
        // "새 씬에서 빠뜨리면 무증상"이라는 WL-055의 실패 통로가 프리팹에서 씬으로 옮겨갈 뿐이다.
        const string k_BarResourcePath = "UI/EnemyStateUI";
        ...
        static EnemyStateUILayer instance;
        static EnemyStateUI barPrefab;
        static bool prefabLoadFailed;
        static bool quitting;

        readonly List<EnemyStateUI> live = new List<EnemyStateUI>();
        readonly Stack<EnemyStateUI> pool = new Stack<EnemyStateUI>();
        ...
        static void HandleEnemySpawned(Enemy enemy)
        {
            ...
            EnemyStateUILayer layer = EnsureInstance();
        ...
        static EnemyStateUILayer EnsureInstance()
        {
            ...
            if (barPrefab == null)
            {
                barPrefab = Resources.Load<EnemyStateUI>(k_BarResourcePath);
            ...
            var go = new GameObject(nameof(EnemyStateUILayer), typeof(RectTransform), typeof(Canvas));
            SetLayerRecursively(go, ResolveHiddenLayer());

            var canvas = go.GetComponent<Canvas>();
            canvas.renderMode = RenderMode.WorldSpace;
            canvas.sortingOrder = UILayer.EnemyStateUI;
            ...
            instance = go.AddComponent<EnemyStateUILayer>();
            return instance;
        }
        ...
        void Attach(Enemy enemy)
        {
            ...
            EnemyStateUI bar = spawned ? Instantiate(barPrefab) : pool.Pop();
