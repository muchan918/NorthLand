...
public static class SkillStatsFormatter
{
    ...
    // 구분자가 로컬라이즈 문자열 안으로 들어가 k_Arrow 단일 출처가 깨진다. 단위만 키로 빼면 둘 다 지킨다.
    ...
    // 구분자는 BuildingInfoUI의 업그레이드 표기(주민당 5 > 7)와 같은 문자를 쓴다.
    ...
    /// 폭탄 2줄: 폭발 데미지 / 반경. 반경은 레벨과 무관한 고정값이라 구분자 없이 한 값만 보여준다.
