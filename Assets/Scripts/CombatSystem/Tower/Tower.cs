...
namespace NorthLand.Combat
{
    public class Tower : MonoBehaviour, IAttacker, ISelectable
    {
        ...
        /// (예: 보스 마력 봉인은 `Has&lt;IAttackAction&gt;()`인 타워만 대상으로 한다).
        ///
        /// ⚠ **능력을 물을 때는 구상 액션이 아니라 `IAttackAction` 같은 능력 인터페이스를 쓸 것**(#252).
        /// `Has&lt;AttackAction&gt;()`로 물으면 전달 방식이 다른 공격 액션(히트스캔=`HitscanAttackAction`)이
        /// 조용히 빠진다 — 컴파일 에러가 아니라 "버프가 안 걸리는 타워"로 나타나 발견이 늦다.
        /// 구상 타입으로 묻는 것이 **의도**인 곳도 있다(BurnBuff는 투사체 명중만 받는다) — 그럴 때는 근거를 남긴다.
        ///
        /// 제약이 `TowerAction`이 아니라 `class`인 이유: 능력 인터페이스는 `TowerAction`을 상속하지 않는다
        /// (액션 구현체가 다중 상속을 못 하므로 `IAttackAction`은 순수 인터페이스여야 한다).
        ...
        /// 해당 액션을 반환한다. 없으면 null. 능력 인터페이스로도 질의할 수 있다(위 `Has` 주석 참조).
        ...
        public float AttackDamage => Get<IAttackAction>()?.Damage ?? 0f;

        public float AttackRange => Get<IAttackAction>()?.Range ?? 0f;

        public float AttackInterval => Get<IAttackAction>()?.Interval ?? 0f;
        ...
        public bool TryAttack(IDamageable target) => Get<IAttackAction>()?.TryAttack(target) ?? false;
