...
namespace NorthLand.Combat
{
    ...
    ///
    /// ⚠ **성장은 웨이브를 넘기지 않는다.** 어느 소비처든 웨이브가 끝나면 스택이 전부 풀린다
    /// (`TowerAction.OnWaveEnd`) — `DecaySeconds`는 **웨이브 안에서의** 감쇠만 정한다.
    ///
    ...
    public class RampProfile
    {
        ...
        [Tooltip("마지막 스택 획득 후 이 시간마다 스택이 1씩 빠진다. 0이면 웨이브 안에서는 식지 않는다 " +
                 "— 단 웨이브가 끝나면 이 값과 무관하게 전부 초기화된다(TowerAction.OnWaveEnd).")]
