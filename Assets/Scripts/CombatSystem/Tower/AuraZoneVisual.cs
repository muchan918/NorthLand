...
namespace NorthLand.Combat
{
    ...
    public sealed class AuraZoneVisual : MonoBehaviour
    {
        [Tooltip("텍스처가 쿼드 가장자리까지 차지 않는 만큼 곱한다. 1.15 = 바깥 13%가 비어 보인다는 뜻. " +
                 ...
                 "기본값 1.15는 바닥 장판 3종이 공유하는 경계선 링 에셋(AreaBounds)을 실측한 값이다 — " +
                 "다른 링 아트를 쓰는 이펙트라면 다시 재서 적을 것.")]
        // 기본값이 1f였을 때 바닥 장판 4종이 전부 그 값으로 저작돼 있었고, 그래서 전부 같은 크기로
        // 어긋나 있었다(#541). 어긋남의 원인이 프리팹 사정이 아니라 **공유 링 에셋의 성질**이기 때문이다 —
        // 링 텍스처의 밝은 밴드가 디스크 반경의 83~87% 지점에 그려져 있어, 기하학적으로 맞춰도 눈에
        // 보이는 링만 안쪽에서 끝난다(톱다운 픽셀 실측: 4종 모두 밝기 피크 0.83x · 가시 외곽 0.86~0.89x).
        // 기본값을 실측값으로 올려 새 장판 이펙트가 같은 버그로 시작하지 않게 한다.
        // ⚠ **이미 저작된 프리팹은 직렬화된 값이 이기므로 바뀌지 않는다** — 기존 4종은 프리팹에서 1.15로
        // 맞춰져 있고(NorthLand-Imported), 이 기본값은 신규 저작에만 적용된다.
        [SerializeField] float edgeCompensation = 1.15f;
