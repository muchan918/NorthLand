...
{
    // 투사체를 쏘는 액션. Single/Area는 별개 액션이 아니라 **명중 방식(ProjectileImpact)만**
    // 다르다 — 대상 탐색·쿨다운·발사 경로가 완전히 같아서, 이미 분리돼 있는 ProjectileImpact를 전략으로 쓴다.
    // 마찬가지로 유도/포격은 ProjectileFlight가 갈라낸다. 둘 다 SO가 정한다(#274).
    //
    // 체인은 여기서 빠진다 — 명중 방식이 아니라 **전달 방식**이 다르기 때문이다(히트스캔).
    // `HitscanAttackAction`이 담당하며, 능력 계약(`IAttackAction`)만 공유한다(#252).
    [Serializable]
    public sealed class AttackAction : TowerAction, IAttackAction
    {
        ...
        {
            // 체인 분기는 없다 — 투사체가 배달하지 않으므로 ImpactKind에서 빠졌다(#252).
