...
public enum EnemyUnitFilter
{
    ...
    // 배치된 **공격 타워**. Tower.Active 정적 리스트를 순회하므로 물리 질의가 필요 없다.
    // 오라·유틸 계열(Magic 타입 — haste / poison / 이동속도 감소)은 공격 스탯이 없어 빠진다
    // → 마력 봉인 중에도 감속이 살아남아 P1 파훼 수단이 유지된다.
    // 집합의 정의와 그 판정을 등록 여부가 아니라 카테고리로 둔 근거는 EnemyNodeQuery.IsAttackTower 참조.
    Tower,
